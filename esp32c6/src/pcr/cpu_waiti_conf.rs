///Register `CPU_WAITI_CONF` reader
pub type R = crate::R<CPU_WAITI_CONF_SPEC>;
///Register `CPU_WAITI_CONF` writer
pub type W = crate::W<CPU_WAITI_CONF_SPEC>;
///Field `CPUPERIOD_SEL` reader - Reserved. This filed has been replaced by PCR_CPU_HS_DIV_NUM and PCR_CPU_LS_DIV_NUM
pub type CPUPERIOD_SEL_R = crate::FieldReader;
///Field `PLL_FREQ_SEL` reader - Reserved. This filed has been replaced by PCR_CPU_HS_DIV_NUM and PCR_CPU_LS_DIV_NUM
pub type PLL_FREQ_SEL_R = crate::BitReader;
///Field `CPU_WAIT_MODE_FORCE_ON` reader - Set 1 to force cpu_waiti_clk enable.
pub type CPU_WAIT_MODE_FORCE_ON_R = crate::BitReader;
///Field `CPU_WAIT_MODE_FORCE_ON` writer - Set 1 to force cpu_waiti_clk enable.
pub type CPU_WAIT_MODE_FORCE_ON_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `CPU_WAITI_DELAY_NUM` reader - This field used to set delay cycle when cpu enter waiti mode, after delay waiti_clk will close
pub type CPU_WAITI_DELAY_NUM_R = crate::FieldReader;
///Field `CPU_WAITI_DELAY_NUM` writer - This field used to set delay cycle when cpu enter waiti mode, after delay waiti_clk will close
pub type CPU_WAITI_DELAY_NUM_W<'a, REG> = crate::FieldWriter<'a, REG, 4>;
impl R {
    ///Bits 0:1 - Reserved. This filed has been replaced by PCR_CPU_HS_DIV_NUM and PCR_CPU_LS_DIV_NUM
    #[inline(always)]
    pub fn cpuperiod_sel(&self) -> CPUPERIOD_SEL_R {
        CPUPERIOD_SEL_R::new((self.bits & 3) as u8)
    }
    ///Bit 2 - Reserved. This filed has been replaced by PCR_CPU_HS_DIV_NUM and PCR_CPU_LS_DIV_NUM
    #[inline(always)]
...
///`read()` method returns [`cpu_waiti_conf::R`](R) reader structure
impl crate::Readable for CPU_WAITI_CONF_SPEC {}
///`write(|w| ..)` method takes [`cpu_waiti_conf::W`](W) writer structure
