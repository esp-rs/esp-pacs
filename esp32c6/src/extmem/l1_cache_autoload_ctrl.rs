...
impl R {
    ///Bit 0 - The bit is used to enable and disable autoload operation on L1-Cache. 1: enable, 0: disable.
    #[inline(always)]
    pub fn l1_cache_autoload_ena(&self) -> L1_CACHE_AUTOLOAD_ENA_R {
        L1_CACHE_AUTOLOAD_ENA_R::new((self.bits & 1) != 0)
    }
    ///Bit 1 - The bit is used to indicate whether autoload operation on L1-Cache is finished or not. 0: not finished. 1: finished.
    #[inline(always)]
    pub fn l1_cache_autoload_done(&self) -> L1_CACHE_AUTOLOAD_DONE_R {
        L1_CACHE_AUTOLOAD_DONE_R::new(((self.bits >> 1) & 1) != 0)
    }
    ///Bit 2 - The bit is used to configure the direction of autoload operation on L1-Cache. 0: ascending. 1: descending.
    #[inline(always)]
    pub fn l1_cache_autoload_order(&self) -> L1_CACHE_AUTOLOAD_ORDER_R {
        L1_CACHE_AUTOLOAD_ORDER_R::new(((self.bits >> 2) & 1) != 0)
    }
    ///Bits 3:4 - The field is used to configure trigger mode of autoload operation on L1-Cache. 0/3: miss-trigger, 1: hit-trigger, 2: miss-hit-trigger.
    #[inline(always)]
