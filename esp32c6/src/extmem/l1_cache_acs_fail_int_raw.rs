    ...
    fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
        f.debug_struct("L1_CACHE_ACS_FAIL_INT_RAW")
            .field(
                ...
                &self.l1_icache0_fail_int_raw().bit(),
            )
            .field(
                "l1_icache1_fail_int_raw",
                &self.l1_icache1_fail_int_raw().bit(),
            )
            .field(
                "l1_icache2_fail_int_raw",
                &self.l1_icache2_fail_int_raw().bit(),
            )
            .field(
                "l1_icache3_fail_int_raw",
                &self.l1_icache3_fail_int_raw().bit(),
            )
            .field("l1_cache_fail_int_raw", &self.l1_cache_fail_int_raw().bit())
            .finish()
    }
}
#[cfg(feature = "impl-register-debug")]
