...
///Field `L1_CACHE_PRELOAD_ADDR` writer - Those bits are used to configure the start virtual address of preload on L1-Cache, which should be used together with L1_CACHE_PRELOAD_SIZE_REG
pub type L1_CACHE_PRELOAD_ADDR_W<'a, REG> = crate::FieldWriter<'a, REG, 32, u32>;
impl R {
    ///Bits 0:31 - Those bits are used to configure the start virtual address of preload on L1-Cache, which should be used together with L1_CACHE_PRELOAD_SIZE_REG
    #[inline(always)]
...
impl W {
    ...
    pub fn l1_cache_preload_addr(
        &mut self,
    ) -> L1_CACHE_PRELOAD_ADDR_W<L1_DCACHE_PRELOAD_ADDR_SPEC> {
