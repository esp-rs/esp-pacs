...
///Field `PRESCALE` reader - period of PT0_clk = Period of PWM_clk * (PWM_TIMER0_PRESCALE + 1)
pub type PRESCALE_R = crate::FieldReader;
///Field `PRESCALE` writer - period of PT0_clk = Period of PWM_clk * (PWM_TIMER0_PRESCALE + 1)
pub type PRESCALE_W<'a, REG> = crate::FieldWriter<'a, REG, 8>;
///Field `PERIOD` reader - period shadow register of PWM timer0
pub type PERIOD_R = crate::FieldReader<u16>;
///Field `PERIOD` writer - period shadow register of PWM timer0
pub type PERIOD_W<'a, REG> = crate::FieldWriter<'a, REG, 16, u16>;
///Field `PERIOD_UPMETHOD` reader - Update method for active register of PWM timer0 period, 0: immediate, 1: TEZ, 2: sync, 3: TEZ | sync. TEZ here and below means timer equal zero event
pub type PERIOD_UPMETHOD_R = crate::FieldReader;
///Field `PERIOD_UPMETHOD` writer - Update method for active register of PWM timer0 period, 0: immediate, 1: TEZ, 2: sync, 3: TEZ | sync. TEZ here and below means timer equal zero event
