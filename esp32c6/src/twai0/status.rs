...
///Field `RX_BUF_ST` reader - 1: full, one or more complete messages are available in the RXFIFO. 0: empty, no message is available
...
///Field `TX_BUF_ST` reader - 1: released, the CPU may write a message into the transmit buffer. 0: locked, the CPU cannot access the transmit buffer, a message is either waiting for transmission or is in the process of being transmitted
...
///Field `BUS_OFF_ST` reader - 1: bus-off, the TWAI controller is not involved in bus activities. 0: bus-on, the TWAI controller is involved in bus activities
pub type BUS_OFF_ST_R = crate::BitReader;
///Field `MISS_ST` reader - 1: current message is destroyed because of FIFO overflow.
