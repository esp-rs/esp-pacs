...
///Field `CD` reader - These bits are used to define the frequency at the external CLKOUT pin.
pub type CD_R = crate::FieldReader;
///Field `CD` writer - These bits are used to define the frequency at the external CLKOUT pin.
pub type CD_W<'a, REG> = crate::FieldWriter<'a, REG, 8>;
///Field `CLOCK_OFF` reader - 1: Disable the external CLKOUT pin. 0: Enable the external CLKOUT pin. Software has R/W permission in reset mode and RO in operation mode.
pub type CLOCK_OFF_R = crate::BitReader;
///Field `CLOCK_OFF` writer - 1: Disable the external CLKOUT pin. 0: Enable the external CLKOUT pin. Software has R/W permission in reset mode and RO in operation mode.
pub type CLOCK_OFF_W<'a, REG> = crate::BitWriter<'a, REG>;
impl R {
    ///Bits 0:7 - These bits are used to define the frequency at the external CLKOUT pin.
    #[inline(always)]
    pub fn cd(&self) -> CD_R {
        CD_R::new((self.bits & 0xff) as u8)
    }
    ///Bit 8 - 1: Disable the external CLKOUT pin. 0: Enable the external CLKOUT pin. Software has R/W permission in reset mode and RO in operation mode.
    #[inline(always)]
