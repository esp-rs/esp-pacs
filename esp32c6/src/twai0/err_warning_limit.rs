...
///Field `ERR_WARNING_LIMIT` reader - The threshold that trigger error warning interrupt when this interrupt is enabled. Software has R/W permission in reset mode and RO in operation mode.
pub type ERR_WARNING_LIMIT_R = crate::FieldReader;
///Field `ERR_WARNING_LIMIT` writer - The threshold that trigger error warning interrupt when this interrupt is enabled. Software has R/W permission in reset mode and RO in operation mode.
...
///`read()` method returns [`err_warning_limit::R`](R) reader structure
impl crate::Readable for ERR_WARNING_LIMIT_SPEC {}
///`write(|w| ..)` method takes [`err_warning_limit::W`](W) writer structure
