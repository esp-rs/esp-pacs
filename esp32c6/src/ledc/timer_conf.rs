...
impl W {
    ...
    #[inline(always)]
    #[must_use]
    pub fn clk_div(&mut self) -> CLK_DIV_W<TIMER_CONF_SPEC, 5> {
        CLK_DIV_W::new(self)
    }
    #[doc = "Bit 23 - This bit is used to suspend the counter in timer %s."]
    #[inline(always)]
    #[must_use]
    pub fn pause(&mut self) -> PAUSE_W<TIMER_CONF_SPEC, 23> {
        PAUSE_W::new(self)
    }
    #[doc = "Bit 24 - This bit is used to reset timer %s. The counter will show 0 after reset."]
    #[inline(always)]
    #[must_use]
    pub fn rst(&mut self) -> RST_W<TIMER_CONF_SPEC, 24> {
        RST_W::new(self)
    }
    #[doc = "Bit 25 - This bit is used to select clock for timer %s. When this bit is set to 1 LEDC_APB_CLK_SEL\\[1:0\\] should be 1, otherwise the timer clock may be not accurate. 1'h0: SLOW_CLK 1'h1: REF_TICK"]
    #[inline(always)]
    #[must_use]
    pub fn tick_sel(&mut self) -> TICK_SEL_W<TIMER_CONF_SPEC, 25> {
        TICK_SEL_W::new(self)
    }
    #[doc = "Bit 26 - Set this bit to update LEDC_CLK_DIV_TIMER%s and LEDC_TIMER%s_DUTY_RES."]
    #[inline(always)]
    #[must_use]
    pub fn para_up(&mut self) -> PARA_UP_W<TIMER_CONF_SPEC, 26> {
        PARA_UP_W::new(self)
    }
