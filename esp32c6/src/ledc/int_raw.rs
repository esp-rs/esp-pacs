...
///Field `DUTY_CHNG_END_CH(0-5)` reader - Interrupt raw bit for channel %s. Triggered when the gradual change of duty has finished.
pub type DUTY_CHNG_END_CH_R = crate::BitReader;
///Field `DUTY_CHNG_END_CH(0-5)` writer - Interrupt raw bit for channel %s. Triggered when the gradual change of duty has finished.
pub type DUTY_CHNG_END_CH_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `OVF_CNT_CH(0-5)` reader - Interrupt raw bit for channel %s. Triggered when the OVF_CNT has reached the value specified by LEDC.CHx.CONF0.OVF_NUM.
