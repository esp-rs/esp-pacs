///Register `CLK_CONF1` reader
pub type R = crate::R<CLK_CONF1_SPEC>;
///Register `CLK_CONF1` writer
pub type W = crate::W<CLK_CONF1_SPEC>;
///Field `CLK_WIFIBB_22M_EN` reader -
pub type CLK_WIFIBB_22M_EN_R = crate::BitReader;
///Field `CLK_WIFIBB_22M_EN` writer -
pub type CLK_WIFIBB_22M_EN_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `CLK_WIFIBB_40M_EN` reader -
pub type CLK_WIFIBB_40M_EN_R = crate::BitReader;
///Field `CLK_WIFIBB_40M_EN` writer -
pub type CLK_WIFIBB_40M_EN_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `CLK_WIFIBB_44M_EN` reader -
pub type CLK_WIFIBB_44M_EN_R = crate::BitReader;
///Field `CLK_WIFIBB_44M_EN` writer -
pub type CLK_WIFIBB_44M_EN_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `CLK_WIFIBB_80M_EN` reader -
pub type CLK_WIFIBB_80M_EN_R = crate::BitReader;
///Field `CLK_WIFIBB_80M_EN` writer -
pub type CLK_WIFIBB_80M_EN_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `CLK_WIFIBB_40X_EN` reader -
pub type CLK_WIFIBB_40X_EN_R = crate::BitReader;
///Field `CLK_WIFIBB_40X_EN` writer -
pub type CLK_WIFIBB_40X_EN_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `CLK_WIFIBB_80X_EN` reader -
pub type CLK_WIFIBB_80X_EN_R = crate::BitReader;
///Field `CLK_WIFIBB_80X_EN` writer -
pub type CLK_WIFIBB_80X_EN_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `CLK_WIFIBB_40X1_EN` reader -
pub type CLK_WIFIBB_40X1_EN_R = crate::BitReader;
///Field `CLK_WIFIBB_40X1_EN` writer -
pub type CLK_WIFIBB_40X1_EN_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `CLK_WIFIBB_80X1_EN` reader -
pub type CLK_WIFIBB_80X1_EN_R = crate::BitReader;
///Field `CLK_WIFIBB_80X1_EN` writer -
pub type CLK_WIFIBB_80X1_EN_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `CLK_WIFIBB_160X1_EN` reader -
pub type CLK_WIFIBB_160X1_EN_R = crate::BitReader;
///Field `CLK_WIFIBB_160X1_EN` writer -
pub type CLK_WIFIBB_160X1_EN_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `CLK_WIFIMAC_EN` reader -
pub type CLK_WIFIMAC_EN_R = crate::BitReader;
///Field `CLK_WIFIMAC_EN` writer -
pub type CLK_WIFIMAC_EN_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `CLK_WIFI_APB_EN` reader -
pub type CLK_WIFI_APB_EN_R = crate::BitReader;
///Field `CLK_WIFI_APB_EN` writer -
pub type CLK_WIFI_APB_EN_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `CLK_FE_20M_EN` reader -
pub type CLK_FE_20M_EN_R = crate::BitReader;
///Field `CLK_FE_20M_EN` writer -
pub type CLK_FE_20M_EN_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `CLK_FE_40M_EN` reader -
pub type CLK_FE_40M_EN_R = crate::BitReader;
///Field `CLK_FE_40M_EN` writer -
pub type CLK_FE_40M_EN_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `CLK_FE_80M_EN` reader -
pub type CLK_FE_80M_EN_R = crate::BitReader;
///Field `CLK_FE_80M_EN` writer -
pub type CLK_FE_80M_EN_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `CLK_FE_160M_EN` reader -
pub type CLK_FE_160M_EN_R = crate::BitReader;
///Field `CLK_FE_160M_EN` writer -
pub type CLK_FE_160M_EN_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `CLK_FE_CAL_160M_EN` reader -
pub type CLK_FE_CAL_160M_EN_R = crate::BitReader;
///Field `CLK_FE_CAL_160M_EN` writer -
pub type CLK_FE_CAL_160M_EN_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `CLK_FE_APB_EN` reader -
pub type CLK_FE_APB_EN_R = crate::BitReader;
///Field `CLK_FE_APB_EN` writer -
pub type CLK_FE_APB_EN_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `CLK_BT_APB_EN` reader -
pub type CLK_BT_APB_EN_R = crate::BitReader;
///Field `CLK_BT_APB_EN` writer -
pub type CLK_BT_APB_EN_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `CLK_BT_EN` reader -
pub type CLK_BT_EN_R = crate::BitReader;
///Field `CLK_BT_EN` writer -
pub type CLK_BT_EN_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `CLK_WIFIBB_480M_EN` reader -
pub type CLK_WIFIBB_480M_EN_R = crate::BitReader;
///Field `CLK_WIFIBB_480M_EN` writer -
pub type CLK_WIFIBB_480M_EN_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `CLK_FE_480M_EN` reader -
pub type CLK_FE_480M_EN_R = crate::BitReader;
///Field `CLK_FE_480M_EN` writer -
pub type CLK_FE_480M_EN_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `CLK_FE_ANAMODE_40M_EN` reader -
pub type CLK_FE_ANAMODE_40M_EN_R = crate::BitReader;
///Field `CLK_FE_ANAMODE_40M_EN` writer -
pub type CLK_FE_ANAMODE_40M_EN_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `CLK_FE_ANAMODE_80M_EN` reader -
pub type CLK_FE_ANAMODE_80M_EN_R = crate::BitReader;
///Field `CLK_FE_ANAMODE_80M_EN` writer -
pub type CLK_FE_ANAMODE_80M_EN_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `CLK_FE_ANAMODE_160M_EN` reader -
pub type CLK_FE_ANAMODE_160M_EN_R = crate::BitReader;
///Field `CLK_FE_ANAMODE_160M_EN` writer -
pub type CLK_FE_ANAMODE_160M_EN_W<'a, REG> = crate::BitWriter<'a, REG>;
impl R {
    ///Bit 0
    #[inline(always)]
    pub fn clk_wifibb_22m_en(&self) -> CLK_WIFIBB_22M_EN_R {
        CLK_WIFIBB_22M_EN_R::new((self.bits & 1) != 0)
    }
    ///Bit 1
    #[inline(always)]
    pub fn clk_wifibb_40m_en(&self) -> CLK_WIFIBB_40M_EN_R {
        CLK_WIFIBB_40M_EN_R::new(((self.bits >> 1) & 1) != 0)
    }
    ///Bit 2
    #[inline(always)]
    pub fn clk_wifibb_44m_en(&self) -> CLK_WIFIBB_44M_EN_R {
        CLK_WIFIBB_44M_EN_R::new(((self.bits >> 2) & 1) != 0)
    }
    ///Bit 3
    #[inline(always)]
    pub fn clk_wifibb_80m_en(&self) -> CLK_WIFIBB_80M_EN_R {
        CLK_WIFIBB_80M_EN_R::new(((self.bits >> 3) & 1) != 0)
    }
    ///Bit 4
    #[inline(always)]
    pub fn clk_wifibb_40x_en(&self) -> CLK_WIFIBB_40X_EN_R {
        CLK_WIFIBB_40X_EN_R::new(((self.bits >> 4) & 1) != 0)
    }
    ///Bit 5
    #[inline(always)]
    pub fn clk_wifibb_80x_en(&self) -> CLK_WIFIBB_80X_EN_R {
        CLK_WIFIBB_80X_EN_R::new(((self.bits >> 5) & 1) != 0)
    }
    ///Bit 6
    #[inline(always)]
    pub fn clk_wifibb_40x1_en(&self) -> CLK_WIFIBB_40X1_EN_R {
        CLK_WIFIBB_40X1_EN_R::new(((self.bits >> 6) & 1) != 0)
    }
    ///Bit 7
    #[inline(always)]
    pub fn clk_wifibb_80x1_en(&self) -> CLK_WIFIBB_80X1_EN_R {
        CLK_WIFIBB_80X1_EN_R::new(((self.bits >> 7) & 1) != 0)
    }
    ///Bit 8
    #[inline(always)]
    pub fn clk_wifibb_160x1_en(&self) -> CLK_WIFIBB_160X1_EN_R {
        CLK_WIFIBB_160X1_EN_R::new(((self.bits >> 8) & 1) != 0)
    }
    ///Bit 9
    #[inline(always)]
    pub fn clk_wifimac_en(&self) -> CLK_WIFIMAC_EN_R {
        CLK_WIFIMAC_EN_R::new(((self.bits >> 9) & 1) != 0)
    }
    ///Bit 10
    #[inline(always)]
    pub fn clk_wifi_apb_en(&self) -> CLK_WIFI_APB_EN_R {
        CLK_WIFI_APB_EN_R::new(((self.bits >> 10) & 1) != 0)
    }
    ///Bit 11
    #[inline(always)]
    pub fn clk_fe_20m_en(&self) -> CLK_FE_20M_EN_R {
        CLK_FE_20M_EN_R::new(((self.bits >> 11) & 1) != 0)
    }
    ///Bit 12
    #[inline(always)]
    pub fn clk_fe_40m_en(&self) -> CLK_FE_40M_EN_R {
        CLK_FE_40M_EN_R::new(((self.bits >> 12) & 1) != 0)
    }
    ///Bit 13
    #[inline(always)]
    pub fn clk_fe_80m_en(&self) -> CLK_FE_80M_EN_R {
        CLK_FE_80M_EN_R::new(((self.bits >> 13) & 1) != 0)
    }
    ///Bit 14
    #[inline(always)]
    pub fn clk_fe_160m_en(&self) -> CLK_FE_160M_EN_R {
        CLK_FE_160M_EN_R::new(((self.bits >> 14) & 1) != 0)
    }
    ///Bit 15
    #[inline(always)]
    pub fn clk_fe_cal_160m_en(&self) -> CLK_FE_CAL_160M_EN_R {
        CLK_FE_CAL_160M_EN_R::new(((self.bits >> 15) & 1) != 0)
    }
    ///Bit 16
    #[inline(always)]
    pub fn clk_fe_apb_en(&self) -> CLK_FE_APB_EN_R {
        CLK_FE_APB_EN_R::new(((self.bits >> 16) & 1) != 0)
    }
    ///Bit 17
    #[inline(always)]
    pub fn clk_bt_apb_en(&self) -> CLK_BT_APB_EN_R {
        CLK_BT_APB_EN_R::new(((self.bits >> 17) & 1) != 0)
    }
    ///Bit 18
    #[inline(always)]
    pub fn clk_bt_en(&self) -> CLK_BT_EN_R {
        CLK_BT_EN_R::new(((self.bits >> 18) & 1) != 0)
    }
    ///Bit 19
    #[inline(always)]
    pub fn clk_wifibb_480m_en(&self) -> CLK_WIFIBB_480M_EN_R {
        CLK_WIFIBB_480M_EN_R::new(((self.bits >> 19) & 1) != 0)
    }
    ///Bit 20
    #[inline(always)]
    pub fn clk_fe_480m_en(&self) -> CLK_FE_480M_EN_R {
        CLK_FE_480M_EN_R::new(((self.bits >> 20) & 1) != 0)
    }
    ///Bit 21
    #[inline(always)]
    pub fn clk_fe_anamode_40m_en(&self) -> CLK_FE_ANAMODE_40M_EN_R {
        CLK_FE_ANAMODE_40M_EN_R::new(((self.bits >> 21) & 1) != 0)
    }
    ///Bit 22
    #[inline(always)]
    pub fn clk_fe_anamode_80m_en(&self) -> CLK_FE_ANAMODE_80M_EN_R {
        CLK_FE_ANAMODE_80M_EN_R::new(((self.bits >> 22) & 1) != 0)
    }
    ///Bit 23
    #[inline(always)]
    pub fn clk_fe_anamode_160m_en(&self) -> CLK_FE_ANAMODE_160M_EN_R {
        CLK_FE_ANAMODE_160M_EN_R::new(((self.bits >> 23) & 1) != 0)
    }
}
#[cfg(feature = "impl-register-debug")]
impl core::fmt::Debug for R {
    fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
        f.debug_struct("CLK_CONF1")
            .field("clk_wifibb_22m_en", &self.clk_wifibb_22m_en())
            .field("clk_wifibb_40m_en", &self.clk_wifibb_40m_en())
            .field("clk_wifibb_44m_en", &self.clk_wifibb_44m_en())
            .field("clk_wifibb_80m_en", &self.clk_wifibb_80m_en())
            .field("clk_wifibb_40x_en", &self.clk_wifibb_40x_en())
            .field("clk_wifibb_80x_en", &self.clk_wifibb_80x_en())
            .field("clk_wifibb_40x1_en", &self.clk_wifibb_40x1_en())
            .field("clk_wifibb_80x1_en", &self.clk_wifibb_80x1_en())
            .field("clk_wifibb_160x1_en", &self.clk_wifibb_160x1_en())
            .field("clk_wifimac_en", &self.clk_wifimac_en())
            .field("clk_wifi_apb_en", &self.clk_wifi_apb_en())
            .field("clk_fe_20m_en", &self.clk_fe_20m_en())
            .field("clk_fe_40m_en", &self.clk_fe_40m_en())
            .field("clk_fe_80m_en", &self.clk_fe_80m_en())
            .field("clk_fe_160m_en", &self.clk_fe_160m_en())
            .field("clk_fe_cal_160m_en", &self.clk_fe_cal_160m_en())
            .field("clk_fe_apb_en", &self.clk_fe_apb_en())
            .field("clk_bt_apb_en", &self.clk_bt_apb_en())
            .field("clk_bt_en", &self.clk_bt_en())
            .field("clk_wifibb_480m_en", &self.clk_wifibb_480m_en())
            .field("clk_fe_480m_en", &self.clk_fe_480m_en())
            .field("clk_fe_anamode_40m_en", &self.clk_fe_anamode_40m_en())
            .field("clk_fe_anamode_80m_en", &self.clk_fe_anamode_80m_en())
            .field("clk_fe_anamode_160m_en", &self.clk_fe_anamode_160m_en())
            .finish()
    }
}
impl W {
    ///Bit 0
    #[inline(always)]
    #[must_use]
    pub fn clk_wifibb_22m_en(&mut self) -> CLK_WIFIBB_22M_EN_W<CLK_CONF1_SPEC> {
        CLK_WIFIBB_22M_EN_W::new(self, 0)
    }
    ///Bit 1
    #[inline(always)]
    #[must_use]
    pub fn clk_wifibb_40m_en(&mut self) -> CLK_WIFIBB_40M_EN_W<CLK_CONF1_SPEC> {
        CLK_WIFIBB_40M_EN_W::new(self, 1)
    }
    ///Bit 2
    #[inline(always)]
    #[must_use]
    pub fn clk_wifibb_44m_en(&mut self) -> CLK_WIFIBB_44M_EN_W<CLK_CONF1_SPEC> {
        CLK_WIFIBB_44M_EN_W::new(self, 2)
    }
    ///Bit 3
    #[inline(always)]
    #[must_use]
    pub fn clk_wifibb_80m_en(&mut self) -> CLK_WIFIBB_80M_EN_W<CLK_CONF1_SPEC> {
        CLK_WIFIBB_80M_EN_W::new(self, 3)
    }
    ///Bit 4
    #[inline(always)]
    #[must_use]
    pub fn clk_wifibb_40x_en(&mut self) -> CLK_WIFIBB_40X_EN_W<CLK_CONF1_SPEC> {
        CLK_WIFIBB_40X_EN_W::new(self, 4)
    }
    ///Bit 5
    #[inline(always)]
    #[must_use]
    pub fn clk_wifibb_80x_en(&mut self) -> CLK_WIFIBB_80X_EN_W<CLK_CONF1_SPEC> {
        CLK_WIFIBB_80X_EN_W::new(self, 5)
    }
    ///Bit 6
    #[inline(always)]
    #[must_use]
    pub fn clk_wifibb_40x1_en(&mut self) -> CLK_WIFIBB_40X1_EN_W<CLK_CONF1_SPEC> {
        CLK_WIFIBB_40X1_EN_W::new(self, 6)
    }
    ///Bit 7
    #[inline(always)]
    #[must_use]
    pub fn clk_wifibb_80x1_en(&mut self) -> CLK_WIFIBB_80X1_EN_W<CLK_CONF1_SPEC> {
        CLK_WIFIBB_80X1_EN_W::new(self, 7)
    }
    ///Bit 8
    #[inline(always)]
    #[must_use]
    pub fn clk_wifibb_160x1_en(&mut self) -> CLK_WIFIBB_160X1_EN_W<CLK_CONF1_SPEC> {
        CLK_WIFIBB_160X1_EN_W::new(self, 8)
    }
    ///Bit 9
    #[inline(always)]
    #[must_use]
    pub fn clk_wifimac_en(&mut self) -> CLK_WIFIMAC_EN_W<CLK_CONF1_SPEC> {
        CLK_WIFIMAC_EN_W::new(self, 9)
    }
    ///Bit 10
    #[inline(always)]
    #[must_use]
    pub fn clk_wifi_apb_en(&mut self) -> CLK_WIFI_APB_EN_W<CLK_CONF1_SPEC> {
        CLK_WIFI_APB_EN_W::new(self, 10)
    }
    ///Bit 11
    #[inline(always)]
    #[must_use]
    pub fn clk_fe_20m_en(&mut self) -> CLK_FE_20M_EN_W<CLK_CONF1_SPEC> {
        CLK_FE_20M_EN_W::new(self, 11)
    }
    ///Bit 12
    #[inline(always)]
    #[must_use]
    pub fn clk_fe_40m_en(&mut self) -> CLK_FE_40M_EN_W<CLK_CONF1_SPEC> {
        CLK_FE_40M_EN_W::new(self, 12)
    }
    ///Bit 13
    #[inline(always)]
    #[must_use]
    pub fn clk_fe_80m_en(&mut self) -> CLK_FE_80M_EN_W<CLK_CONF1_SPEC> {
        CLK_FE_80M_EN_W::new(self, 13)
    }
    ///Bit 14
    #[inline(always)]
    #[must_use]
    pub fn clk_fe_160m_en(&mut self) -> CLK_FE_160M_EN_W<CLK_CONF1_SPEC> {
        CLK_FE_160M_EN_W::new(self, 14)
    }
    ///Bit 15
    #[inline(always)]
    #[must_use]
    pub fn clk_fe_cal_160m_en(&mut self) -> CLK_FE_CAL_160M_EN_W<CLK_CONF1_SPEC> {
        CLK_FE_CAL_160M_EN_W::new(self, 15)
    }
    ///Bit 16
    #[inline(always)]
    #[must_use]
    pub fn clk_fe_apb_en(&mut self) -> CLK_FE_APB_EN_W<CLK_CONF1_SPEC> {
        CLK_FE_APB_EN_W::new(self, 16)
    }
    ///Bit 17
    #[inline(always)]
    #[must_use]
    pub fn clk_bt_apb_en(&mut self) -> CLK_BT_APB_EN_W<CLK_CONF1_SPEC> {
        CLK_BT_APB_EN_W::new(self, 17)
    }
    ///Bit 18
    #[inline(always)]
    #[must_use]
    pub fn clk_bt_en(&mut self) -> CLK_BT_EN_W<CLK_CONF1_SPEC> {
        CLK_BT_EN_W::new(self, 18)
    }
    ///Bit 19
    #[inline(always)]
    #[must_use]
    pub fn clk_wifibb_480m_en(&mut self) -> CLK_WIFIBB_480M_EN_W<CLK_CONF1_SPEC> {
        CLK_WIFIBB_480M_EN_W::new(self, 19)
    }
    ///Bit 20
    #[inline(always)]
    #[must_use]
    pub fn clk_fe_480m_en(&mut self) -> CLK_FE_480M_EN_W<CLK_CONF1_SPEC> {
        CLK_FE_480M_EN_W::new(self, 20)
    }
    ///Bit 21
    #[inline(always)]
    #[must_use]
    pub fn clk_fe_anamode_40m_en(&mut self) -> CLK_FE_ANAMODE_40M_EN_W<CLK_CONF1_SPEC> {
        CLK_FE_ANAMODE_40M_EN_W::new(self, 21)
    }
    ///Bit 22
    #[inline(always)]
    #[must_use]
    pub fn clk_fe_anamode_80m_en(&mut self) -> CLK_FE_ANAMODE_80M_EN_W<CLK_CONF1_SPEC> {
        CLK_FE_ANAMODE_80M_EN_W::new(self, 22)
    }
    ///Bit 23
    #[inline(always)]
    #[must_use]
    pub fn clk_fe_anamode_160m_en(&mut self) -> CLK_FE_ANAMODE_160M_EN_W<CLK_CONF1_SPEC> {
        CLK_FE_ANAMODE_160M_EN_W::new(self, 23)
    }
}
/**

You can [`read`](crate::generic::Reg::read) this register and get [`clk_conf1::R`](R).  You can [`reset`](crate::generic::Reg::reset), [`write`](crate::generic::Reg::write), [`write_with_zero`](crate::generic::Reg::write_with_zero) this register using [`clk_conf1::W`](W). You can also [`modify`](crate::generic::Reg::modify) this register. See [API](https://docs.rs/svd2rust/#read--modify--write-api).*/
pub struct CLK_CONF1_SPEC;
impl crate::RegisterSpec for CLK_CONF1_SPEC {
    type Ux = u32;
}
///`read()` method returns [`clk_conf1::R`](R) reader structure
impl crate::Readable for CLK_CONF1_SPEC {}
///`write(|w| ..)` method takes [`clk_conf1::W`](W) writer structure
impl crate::Writable for CLK_CONF1_SPEC {
    type Safety = crate::Unsafe;
    const ZERO_TO_MODIFY_FIELDS_BITMAP: u32 = 0;
    const ONE_TO_MODIFY_FIELDS_BITMAP: u32 = 0;
}
///`reset()` method sets CLK_CONF1 to value 0
impl crate::Resettable for CLK_CONF1_SPEC {
    const RESET_VALUE: u32 = 0;
}
