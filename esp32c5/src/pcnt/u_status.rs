...
#[doc = "Field `STEP_LIM` reader - The latched value of step counter limit event of PCNT_U%s when step counter event interrupt is valid. 1: the current pulse counter equals to reg_cnt_step_lim and step counter event is valid. 0: others"]
pub type STEP_LIM_R = crate::BitReader;
#[doc = "Field `STEP` reader - The latched value of step counter event of PCNT_U%s when step counter event interrupt is valid. 1: the current pulse counter increment equals to reg_cnt_step and step counter event is valid. 0: others"]
...
impl R {
    ...
    #[inline(always)]
    pub fn step_lim(&self) -> STEP_LIM_R {
        STEP_LIM_R::new(((self.bits >> 7) & 1) != 0)
    }
