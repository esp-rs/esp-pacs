...
#[doc = "Field `OUT_W1TS` writer - Configures whether or not to enable the output register LP_IO_OUT_REG of GPIO0 ~ GPIO7.\\\\ - bit0 ~ bit7 are corresponding to GPIO0 ~ GPIO7. - If the value 1 is written to a bit here, the corresponding bit in LP_IO_OUT_REG will be set to 1. - Recommended operation: use this register to set LP_IO_OUT_REG."]
pub type OUT_W1TS_W<'a, REG> = crate::FieldWriter<'a, REG, 8>;
...
impl W {
    #[doc = "Bits 0:7 - Configures whether or not to enable the output register LP_IO_OUT_REG of GPIO0 ~ GPIO7.\\\\ - bit0 ~ bit7 are corresponding to GPIO0 ~ GPIO7. - If the value 1 is written to a bit here, the corresponding bit in LP_IO_OUT_REG will be set to 1. - Recommended operation: use this register to set LP_IO_OUT_REG."]
    #[inline(always)]
