...
///Field `MEM_WADDR_EX` reader - This register records the memory address offset when receiver of CHANNEL%s is using the RAM.
...
///Field `MEM_OWNER_ERR` reader - This status bit will be set when the ownership of memory block is wrong.
pub type MEM_OWNER_ERR_R = crate::BitReader;
///Field `MEM_FULL` reader - This status bit will be set if the receiver receives more data than the memory size.
...
///Field `APB_MEM_RD_ERR` reader - This status bit will be set if the offset address out of memory size when reads via APB bus.
...
impl R {
    ...
    ///Bits 10:18 - This register records the memory address offset when transmitter of CHANNEL%s is using the RAM.
    #[inline(always)]
    pub fn mem_raddr_ex(&self) -> MEM_RADDR_EX_R {
        MEM_RADDR_EX_R::new(((self.bits >> 10) & 0x01ff) as u16)
    }
    ///Bits 20:22 - This register records the FSM status of CHANNEL%s.
    #[inline(always)]
    pub fn state(&self) -> STATE_R {
        STATE_R::new(((self.bits >> 20) & 7) as u8)
    }
    ///Bit 23 - This status bit will be set when the ownership of memory block is wrong.
    #[inline(always)]
    pub fn mem_owner_err(&self) -> MEM_OWNER_ERR_R {
        MEM_OWNER_ERR_R::new(((self.bits >> 23) & 1) != 0)
    }
    ///Bit 24 - This status bit will be set if the receiver receives more data than the memory size.
    #[inline(always)]
