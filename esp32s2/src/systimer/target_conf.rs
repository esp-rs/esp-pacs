...
///Field `PERIOD` reader - Set alarm period for system timer target 0, only valid in periodic alarms mode.
pub type PERIOD_R = crate::FieldReader<u32>;
///Field `PERIOD` writer - Set alarm period for system timer target 0, only valid in periodic alarms mode.
pub type PERIOD_W<'a, REG> = crate::FieldWriter<'a, REG, 30, u32>;
///Field `PERIOD_MODE` reader - Set work mode for system timer target 0. 0: work in a timedelay alarm mode; 1: work in periodic alarms mode.
pub type PERIOD_MODE_R = crate::BitReader;
///Field `PERIOD_MODE` writer - Set work mode for system timer target 0. 0: work in a timedelay alarm mode; 1: work in periodic alarms mode.
pub type PERIOD_MODE_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `WORK_EN` reader - System timer target 0 work enable.
pub type WORK_EN_R = crate::BitReader;
///Field `WORK_EN` writer - System timer target 0 work enable.
pub type WORK_EN_W<'a, REG> = crate::BitWriter<'a, REG>;
impl R {
    ///Bits 0:29 - Set alarm period for system timer target 0, only valid in periodic alarms mode.
    #[inline(always)]
    pub fn period(&self) -> PERIOD_R {
        PERIOD_R::new(self.bits & 0x3fff_ffff)
    }
    ///Bit 30 - Set work mode for system timer target 0. 0: work in a timedelay alarm mode; 1: work in periodic alarms mode.
    #[inline(always)]
    pub fn period_mode(&self) -> PERIOD_MODE_R {
        PERIOD_MODE_R::new(((self.bits >> 30) & 1) != 0)
    }
    ///Bit 31 - System timer target 0 work enable.
    #[inline(always)]
