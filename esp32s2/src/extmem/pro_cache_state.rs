...
///Field `PRO_ICACHE_STATE` reader - The bit is used to indicate icache main fsm is in idle state or not. 1: in idle state, 0: not in idle state
pub type PRO_ICACHE_STATE_R = crate::FieldReader<u16>;
///Field `PRO_DCACHE_STATE` reader - The bit is used to indicate dcache main fsm is in idle state or not. 1: in idle state, 0: not in idle state
...
///`read()` method returns [`pro_cache_state::R`](R) reader structure
impl crate::Readable for PRO_CACHE_STATE_SPEC {}
///`reset()` method sets PRO_CACHE_STATE to value 0
