...
///Field `IC_PRELOAD_CNT` reader - The bits are used to count the number of issued pre-load which include manual pre-load and conditional pre-load.
...
///`read()` method returns [`ic_preload_cnt::R`](R) reader structure
impl crate::Readable for IC_PRELOAD_CNT_SPEC {}
///`reset()` method sets IC_PRELOAD_CNT to value 0
