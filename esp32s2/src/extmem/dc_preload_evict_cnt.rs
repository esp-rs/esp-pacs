...
///Field `DC_PRELOAD_EVICT_CNT` reader - The bits are used to count the number of cache evictions by pre-load which include manual pre-load and conditional pre-load.
pub type DC_PRELOAD_EVICT_CNT_R = crate::FieldReader<u16>;
impl R {
    ///Bits 0:15 - The bits are used to count the number of cache evictions by pre-load which include manual pre-load and conditional pre-load.
    #[inline(always)]
...
///`read()` method returns [`dc_preload_evict_cnt::R`](R) reader structure
impl crate::Readable for DC_PRELOAD_EVICT_CNT_SPEC {}
///`reset()` method sets DC_PRELOAD_EVICT_CNT to value 0
