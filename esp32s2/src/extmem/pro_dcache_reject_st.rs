...
///Field `PRO_DCACHE_TAG_ATTR` reader - The bits are used to indicate the attribute of data from external memory when authentication fail. 0: invalidate, 1: execute-able, 2: read-able, 4: write-able.
pub type PRO_DCACHE_TAG_ATTR_R = crate::FieldReader;
///Field `PRO_DCACHE_CPU_ATTR` reader - The bits are used to indicate the attribute of CPU access dcache when authentication fail. 0: invalidate, 1: execute-able, 2: read-able, 4: write-able.
...
///`read()` method returns [`pro_dcache_reject_st::R`](R) reader structure
impl crate::Readable for PRO_DCACHE_REJECT_ST_SPEC {}
///`reset()` method sets PRO_DCACHE_REJECT_ST to value 0
