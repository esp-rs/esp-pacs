...
///Register `PRO_DCACHE_LOCK0_SIZE` writer
pub type W = crate::W<PRO_DCACHE_LOCK0_SIZE_SPEC>;
///Field `PRO_DCACHE_LOCK0_SIZE` reader - The bits are used to configure the first length of data locking, which is combined with PRO_DCACHE_LOCK0_ADDR_REG
pub type PRO_DCACHE_LOCK0_SIZE_R = crate::FieldReader<u16>;
///Field `PRO_DCACHE_LOCK0_SIZE` writer - The bits are used to configure the first length of data locking, which is combined with PRO_DCACHE_LOCK0_ADDR_REG
pub type PRO_DCACHE_LOCK0_SIZE_W<'a, REG> = crate::FieldWriter<'a, REG, 16, u16>;
impl R {
    ///Bits 0:15 - The bits are used to configure the first length of data locking, which is combined with PRO_DCACHE_LOCK0_ADDR_REG
    #[inline(always)]
...
///`read()` method returns [`pro_dcache_lock0_size::R`](R) reader structure
impl crate::Readable for PRO_DCACHE_LOCK0_SIZE_SPEC {}
///`write(|w| ..)` method takes [`pro_dcache_lock0_size::W`](W) writer structure
impl crate::Writable for PRO_DCACHE_LOCK0_SIZE_SPEC {
    type Safety = crate::Unsafe;
    const ZERO_TO_MODIFY_FIELDS_BITMAP: u32 = 0;
    const ONE_TO_MODIFY_FIELDS_BITMAP: u32 = 0;
}
///`reset()` method sets PRO_DCACHE_LOCK0_SIZE to value 0
