...
///Field `DC_PRELOAD_MISS_CNT` reader - The bits are used to count the number of missed pre-load which include manual pre-load and conditional pre-load.
...
impl core::fmt::Debug for R {
    fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
        f.debug_struct("DC_PRELOAD_MISS_CNT")
            .field("dc_preload_miss_cnt", &self.dc_preload_miss_cnt())
            .finish()
    }
}
...
///`read()` method returns [`dc_preload_miss_cnt::R`](R) reader structure
impl crate::Readable for DC_PRELOAD_MISS_CNT_SPEC {}
///`reset()` method sets DC_PRELOAD_MISS_CNT to value 0
