...
///Field `PRO_DCACHE_PRELOAD_ADDR` reader - The bits are used to configure the start virtual address for manual pre-load operation. It should be combined with PRO_DCACHE_PRELOAD_SIZE_REG.
pub type PRO_DCACHE_PRELOAD_ADDR_R = crate::FieldReader<u32>;
///Field `PRO_DCACHE_PRELOAD_ADDR` writer - The bits are used to configure the start virtual address for manual pre-load operation. It should be combined with PRO_DCACHE_PRELOAD_SIZE_REG.
...
///`read()` method returns [`pro_dcache_preload_addr::R`](R) reader structure
impl crate::Readable for PRO_DCACHE_PRELOAD_ADDR_SPEC {}
///`write(|w| ..)` method takes [`pro_dcache_preload_addr::W`](W) writer structure
impl crate::Writable for PRO_DCACHE_PRELOAD_ADDR_SPEC {
    type Safety = crate::Unsafe;
    const ZERO_TO_MODIFY_FIELDS_BITMAP: u32 = 0;
    const ONE_TO_MODIFY_FIELDS_BITMAP: u32 = 0;
}
///`reset()` method sets PRO_DCACHE_PRELOAD_ADDR to value 0
