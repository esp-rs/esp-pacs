...
///Register `PRO_DCACHE_PRELOAD_SIZE` writer
pub type W = crate::W<PRO_DCACHE_PRELOAD_SIZE_SPEC>;
///Field `PRO_DCACHE_PRELOAD_SIZE` reader - The bits are used to configure the length for manual pre-load operation. It should be combined with PRO_DCACHE_PRELOAD_ADDR_REG..
pub type PRO_DCACHE_PRELOAD_SIZE_R = crate::FieldReader<u16>;
///Field `PRO_DCACHE_PRELOAD_SIZE` writer - The bits are used to configure the length for manual pre-load operation. It should be combined with PRO_DCACHE_PRELOAD_ADDR_REG..
pub type PRO_DCACHE_PRELOAD_SIZE_W<'a, REG> = crate::FieldWriter<'a, REG, 10, u16>;
///Field `PRO_DCACHE_PRELOAD_ORDER` reader - The bits are used to configure the direction of manual pre-load operation. 1: descending, 0: ascending.
pub type PRO_DCACHE_PRELOAD_ORDER_R = crate::BitReader;
///Field `PRO_DCACHE_PRELOAD_ORDER` writer - The bits are used to configure the direction of manual pre-load operation. 1: descending, 0: ascending.
pub type PRO_DCACHE_PRELOAD_ORDER_W<'a, REG> = crate::BitWriter<'a, REG>;
impl R {
    ///Bits 0:9 - The bits are used to configure the length for manual pre-load operation. It should be combined with PRO_DCACHE_PRELOAD_ADDR_REG..
    #[inline(always)]
