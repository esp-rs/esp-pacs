///Register `RTC_GPIO_STATUS_W1TS` writer
pub type W = crate::W<RTC_GPIO_STATUS_W1TS_SPEC>;
///Field `GPIO_STATUS_INT_W1TS` writer - GPIO0 ~ 21 interrupt set register. If the value 1 is written to a bit here, the corresponding bit in RTCIO_GPIO_STATUS_INT will be set to 1. Recommended operation: use this register to set RTCIO_GPIO_STATUS_INT.
...
impl W {
    ///Bits 10:31 - GPIO0 ~ 21 interrupt set register. If the value 1 is written to a bit here, the corresponding bit in RTCIO_GPIO_STATUS_INT will be set to 1. Recommended operation: use this register to set RTCIO_GPIO_STATUS_INT.
    #[inline(always)]
    #[must_use]
...
///`write(|w| ..)` method takes [`rtc_gpio_status_w1ts::W`](W) writer structure
