...
pub type VAL_W<'a, REG, const O: u8> = crate::BitWriter<'a, REG, O>;
...
pub type OUT_EN_W<'a, REG, const O: u8> = crate::BitWriter<'a, REG, O>;
...
pub type OUT_TIME_W<'a, REG, const O: u8> = crate::FieldWriter<'a, REG, 3, O>;
...
pub type DMA_SEG_TRANS_DONE_W<'a, REG, const O: u8> = crate::BitWriter<'a, REG, O>;
impl R {
...
impl W {
    ...
    #[inline(always)]
    #[must_use]
    pub fn int_hold_ena(&mut self) -> INT_HOLD_ENA_W<HOLD_SPEC, 0> {
        INT_HOLD_ENA_W::new(self)
    }
    #[doc = "Bit 2 - spi hold output value, which should be used with SPI_HOLD_OUT_EN. Can be configured in CONF state."]
    #[inline(always)]
    #[must_use]
    pub fn val(&mut self) -> VAL_W<HOLD_SPEC, 2> {
        VAL_W::new(self)
    }
    #[doc = "Bit 3 - Enable set spi output hold value to spi_hold_reg. It can be used to hold spi state machine with SPI_EXT_HOLD_EN and other usr hold signals. Can be configured in CONF state."]
    #[inline(always)]
    #[must_use]
    pub fn out_en(&mut self) -> OUT_EN_W<HOLD_SPEC, 3> {
        OUT_EN_W::new(self)
    }
    #[doc = "Bits 4:6 - set the hold cycles of output spi_hold signal when SPI_HOLD_OUT_EN is enable. Can be configured in CONF state."]
    #[inline(always)]
    #[must_use]
    pub fn out_time(&mut self) -> OUT_TIME_W<HOLD_SPEC, 4> {
        OUT_TIME_W::new(self)
    }
    #[doc = "Bit 7 - 1: spi master DMA full-duplex/half-duplex seg-trans ends or slave half-duplex seg-trans ends. And data has been pushed to corresponding memory. 0: seg-trans is not ended or not occurred. Can not be changed by CONF_buf."]
    #[inline(always)]
    #[must_use]
    pub fn dma_seg_trans_done(&mut self) -> DMA_SEG_TRANS_DONE_W<HOLD_SPEC, 7> {
