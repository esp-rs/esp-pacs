...
impl R {
    ///Bits 0:7 - It is the vertical blank front porch of a frame. Can be configured in CONF state.
    #[inline(always)]
    pub fn lcd_vb_front(&self) -> LCD_VB_FRONT_R {
        LCD_VB_FRONT_R::new((self.bits & 0xff) as u8)
    }
    ///Bits 8:19 - It is the horizontal active width of a frame. Can be configured in CONF state.
    #[inline(always)]
    pub fn lcd_ha_width(&self) -> LCD_HA_WIDTH_R {
        LCD_HA_WIDTH_R::new(((self.bits >> 8) & 0x0fff) as u16)
    }
    ///Bits 20:31 - It is the horizontal total width of a frame. Can be configured in CONF state.
    #[inline(always)]
