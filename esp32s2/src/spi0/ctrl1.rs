...
impl W {
    ...
    #[inline(always)]
    #[must_use]
    pub fn clk_mode_13(&mut self) -> CLK_MODE_13_W<CTRL1_SPEC> {
        CLK_MODE_13_W::new(self, 2)
    }
    ///Bit 3 - It saves half a cycle when tsck is the same as rsck. 1: output data at rsck posedge 0: output data at tsck posedge
    #[inline(always)]
    #[must_use]
    pub fn rsck_data_out(&mut self) -> RSCK_DATA_OUT_W<CTRL1_SPEC> {
        RSCK_DATA_OUT_W::new(self, 3)
    }
    ///Bit 4 - 1:SPI_BUF16~SPI_BUF17 can be written 0:SPI_BUF16~SPI_BUF17 can not be written. Can be configured in CONF state.
    #[inline(always)]
    #[must_use]
    pub fn w16_17_wr_ena(&mut self) -> W16_17_WR_ENA_W<CTRL1_SPEC> {
...
///`reset()` method sets CTRL1 to value 0x4010
