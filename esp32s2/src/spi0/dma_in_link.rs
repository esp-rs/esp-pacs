...
///Field `INLINK_AUTO_RET` reader - when the bit is set, the inlink descriptor returns to the first link node when a packet is error.
pub type INLINK_AUTO_RET_R = crate::BitReader;
///Field `INLINK_AUTO_RET` writer - when the bit is set, the inlink descriptor returns to the first link node when a packet is error.
...
///Field `DMA_RX_ENA` reader - SPI DMA read data status bit.
pub type DMA_RX_ENA_R = crate::BitReader;
///Field `DMA_RX_ENA` writer - SPI DMA read data status bit.
...
impl R {
    ...
    ///Bit 20 - when the bit is set, the inlink descriptor returns to the first link node when a packet is error.
    #[inline(always)]
    ...
    ///Bit 31 - SPI DMA read data status bit.
    #[inline(always)]
