...
///Field `GPIO(0-7)` reader - This is the status bit for DEDIC_GPIO%s_INT_RAW when DEDIC_GPIO7_INT_ENA is set to 1.
pub type GPIO_R = crate::BitReader;
impl R {
    ///This is the status bit for DEDIC_GPIO(0-7)_INT_RAW when DEDIC_GPIO7_INT_ENA is set to 1.
    ///
...
///`reset()` method sets INTR_ST to value 0
