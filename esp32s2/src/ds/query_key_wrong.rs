...
///Field `QUERY_KEY_WRONG` reader - 1-15: HMAC was activated, but the DS peripheral did not successfully receive the DS_KEY value from the HMAC peripheral. The biggest value is 15. 0: HMAC is not activated.
pub type QUERY_KEY_WRONG_R = crate::FieldReader;
impl R {
    ///Bits 0:3 - 1-15: HMAC was activated, but the DS peripheral did not successfully receive the DS_KEY value from the HMAC peripheral. The biggest value is 15. 0: HMAC is not activated.
    #[inline(always)]
...
///`read()` method returns [`query_key_wrong::R`](R) reader structure
