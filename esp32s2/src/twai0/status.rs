...
///Field `OVERRUN_ST` reader - 1: The RX FIFO is full and data overrun has occurred.
...
///Field `TX_ST` reader - 1: The TWAI Controller is transmitting a message to the bus.
pub type TX_ST_R = crate::BitReader;
///Field `ERR_ST` reader - 1: At least one of the RX/TX error counter has reached or exceeded the value set in register TWAI_ERR_WARNING_LIMIT_REG.
pub type ERR_ST_R = crate::BitReader;
///Field `BUS_OFF_ST` reader - 1: In bus-off status, the TWAI Controller is no longer involved in bus activities.
