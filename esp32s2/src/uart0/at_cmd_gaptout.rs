...
///Field `RX_GAP_TOUT` reader - This register is used to configure the duration time between the AT_CMD characters. It will not take the data as continuous AT_CMD characters when the duration time is less than this register's value.
pub type RX_GAP_TOUT_R = crate::FieldReader<u16>;
///Field `RX_GAP_TOUT` writer - This register is used to configure the duration time between the AT_CMD characters. It will not take the data as continuous AT_CMD characters when the duration time is less than this register's value.
