...
///Field `ACTIVE_THRESHOLD` reader - The UART is activated from Light-sleep mode when the input RXD edge changes more times than this register's value.
pub type ACTIVE_THRESHOLD_R = crate::FieldReader<u16>;
///Field `ACTIVE_THRESHOLD` writer - The UART is activated from Light-sleep mode when the input RXD edge changes more times than this register's value.
pub type ACTIVE_THRESHOLD_W<'a, REG> = crate::FieldWriter<'a, REG, 10, u16>;
impl R {
    ///Bits 0:9 - The UART is activated from Light-sleep mode when the input RXD edge changes more times than this register's value.
    #[inline(always)]
