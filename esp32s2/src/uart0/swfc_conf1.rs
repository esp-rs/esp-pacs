...
///Field `XON_THRESHOLD` reader - When the number of data bytes in RX FIFO is less than this register's value with UART_SW_FLOW_CON_EN set to 1, the transmitter sends an XON character.
pub type XON_THRESHOLD_R = crate::FieldReader<u16>;
///Field `XON_THRESHOLD` writer - When the number of data bytes in RX FIFO is less than this register's value with UART_SW_FLOW_CON_EN set to 1, the transmitter sends an XON character.
pub type XON_THRESHOLD_W<'a, REG> = crate::FieldWriter<'a, REG, 9, u16>;
///Field `XON_CHAR` reader - This register stores the XON flow control character.
pub type XON_CHAR_R = crate::FieldReader;
///Field `XON_CHAR` writer - This register stores the XON flow control character.
...
///`reset()` method sets SWFC_CONF1 to value 0x2200
