...
///Field `APB_TX_WADDR` reader - This register stores the offset address in TX FIFO when software writes TX FIFO via APB.
pub type APB_TX_WADDR_R = crate::FieldReader<u16>;
///Field `TX_RADDR` reader - This register stores the offset address in TX FIFO when TX FSM reads data via Tx_FIFO_Ctrl.
...
impl R {
    ...
    ///Bits 11:20 - This register stores the offset address in TX FIFO when TX FSM reads data via Tx_FIFO_Ctrl.
    #[inline(always)]
