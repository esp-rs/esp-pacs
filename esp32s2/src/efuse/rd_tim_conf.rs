...
///Field `THR_A` reader - Configures the hold time of read operation.
pub type THR_A_R = crate::FieldReader;
///Field `THR_A` writer - Configures the hold time of read operation.
pub type THR_A_W<'a, REG> = crate::FieldWriter<'a, REG, 8>;
///Field `TRD` reader - Configures the length of pulse of read operation.
pub type TRD_R = crate::FieldReader;
///Field `TRD` writer - Configures the length of pulse of read operation.
pub type TRD_W<'a, REG> = crate::FieldWriter<'a, REG, 8>;
///Field `TSUR_A` reader - Configures the setup time of read operation.
pub type TSUR_A_R = crate::FieldReader;
///Field `TSUR_A` writer - Configures the setup time of read operation.
...
impl R {
    ///Bits 0:7 - Configures the hold time of read operation.
    #[inline(always)]
    pub fn thr_a(&self) -> THR_A_R {
        THR_A_R::new((self.bits & 0xff) as u8)
    }
    ///Bits 8:15 - Configures the length of pulse of read operation.
    #[inline(always)]
    pub fn trd(&self) -> TRD_R {
        TRD_R::new(((self.bits >> 8) & 0xff) as u8)
    }
    ///Bits 16:23 - Configures the setup time of read operation.
    #[inline(always)]
...
///`reset()` method sets RD_TIM_CONF to value 0x1201_0101
