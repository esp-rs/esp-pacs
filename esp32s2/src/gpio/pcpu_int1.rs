...
///Field `PROCPU1_INT` reader - GPIO32 ~ 53 PRO_CPU interrupt status. This interrupt status is corresponding to the bit in GPIO_STATUS1_REG when assert (high) enable signal (bit 13 of GPIO_PINn_REG).
pub type PROCPU1_INT_R = crate::FieldReader<u32>;
impl R {
    ///Bits 0:21 - GPIO32 ~ 53 PRO_CPU interrupt status. This interrupt status is corresponding to the bit in GPIO_STATUS1_REG when assert (high) enable signal (bit 13 of GPIO_PINn_REG).
    #[inline(always)]
