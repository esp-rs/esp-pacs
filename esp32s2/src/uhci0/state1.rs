...
///Field `OUTLINK_DSCR_ADDR` reader - This register stores the current transmit descriptor's address.
...
///Field `OUTFIFO_CNT` reader - This register stores the number of data bytes in TX FIFO.
...
impl R {
    ///Bits 0:17 - This register stores the current transmit descriptor's address.
    #[inline(always)]
    ...
    ///Bits 23:27 - This register stores the number of data bytes in TX FIFO.
    #[inline(always)]
    pub fn outfifo_cnt(&self) -> OUTFIFO_CNT_R {
        OUTFIFO_CNT_R::new(((self.bits >> 23) & 0x1f) as u8)
    }
    ///Bits 28:30 - UHCI encoder status.
    #[inline(always)]
