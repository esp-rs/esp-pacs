...
///Field `INLINK_DSCR_ADDR` reader - This register stores the current receive descriptor's address.
...
///Field `INFIFO_CNT_DEBUG` reader - This register stores the number of data bytes in RX FIFO.
...
impl R {
    ///Bits 0:17 - This register stores the current receive descriptor's address.
    #[inline(always)]
    ...
    ///Bits 23:27 - This register stores the number of data bytes in RX FIFO.
    #[inline(always)]
    pub fn infifo_cnt_debug(&self) -> INFIFO_CNT_DEBUG_R {
        INFIFO_CNT_DEBUG_R::new(((self.bits >> 23) & 0x1f) as u8)
    }
    ///Bits 28:30 - UHCI decoder status.
    #[inline(always)]
