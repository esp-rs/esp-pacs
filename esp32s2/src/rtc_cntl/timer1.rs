...
///Field `CPU_STALL_EN` reader - Enables CPU stalling.
pub type CPU_STALL_EN_R = crate::BitReader;
///Field `CPU_STALL_EN` writer - Enables CPU stalling.
pub type CPU_STALL_EN_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `CPU_STALL_WAIT` reader - Sets the CPU stall waiting cycle (using the RTC fast clock).
pub type CPU_STALL_WAIT_R = crate::FieldReader;
///Field `CPU_STALL_WAIT` writer - Sets the CPU stall waiting cycle (using the RTC fast clock).
pub type CPU_STALL_WAIT_W<'a, REG> = crate::FieldWriter<'a, REG, 5>;
///Field `CK8M_WAIT` reader - Sets the 8 MHz clock waiting (using the RTC slow clock).
pub type CK8M_WAIT_R = crate::FieldReader;
///Field `CK8M_WAIT` writer - Sets the 8 MHz clock waiting (using the RTC slow clock).
pub type CK8M_WAIT_W<'a, REG> = crate::FieldWriter<'a, REG, 8>;
///Field `XTL_BUF_WAIT` reader - Sets the XTAL waiting cycle (using the RTC slow clock).
pub type XTL_BUF_WAIT_R = crate::FieldReader<u16>;
///Field `XTL_BUF_WAIT` writer - Sets the XTAL waiting cycle (using the RTC slow clock).
pub type XTL_BUF_WAIT_W<'a, REG> = crate::FieldWriter<'a, REG, 10, u16>;
///Field `PLL_BUF_WAIT` reader - Sets the PLL waiting cycle (using the RTC slow clock).
pub type PLL_BUF_WAIT_R = crate::FieldReader;
///Field `PLL_BUF_WAIT` writer - Sets the PLL waiting cycle (using the RTC slow clock).
...
///`read()` method returns [`timer1::R`](R) reader structure
impl crate::Readable for TIMER1_SPEC {}
///`write(|w| ..)` method takes [`timer1::W`](W) writer structure
