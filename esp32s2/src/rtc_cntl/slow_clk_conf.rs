///Register `SLOW_CLK_CONF` reader
pub type R = crate::R<SLOW_CLK_CONF_SPEC>;
///Register `SLOW_CLK_CONF` writer
pub type W = crate::W<SLOW_CLK_CONF_SPEC>;
///Field `ANA_CLK_DIV_VLD` reader - Synchronizes the reg_rtc_ana_clk_div bus. Note that you have to invalidate the bus before switching clock, and validate the new clock.
pub type ANA_CLK_DIV_VLD_R = crate::BitReader;
///Field `ANA_CLK_DIV_VLD` writer - Synchronizes the reg_rtc_ana_clk_div bus. Note that you have to invalidate the bus before switching clock, and validate the new clock.
pub type ANA_CLK_DIV_VLD_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `ANA_CLK_DIV` reader - Set the rtc_clk divider.
pub type ANA_CLK_DIV_R = crate::FieldReader;
///Field `ANA_CLK_DIV` writer - Set the rtc_clk divider.
pub type ANA_CLK_DIV_W<'a, REG> = crate::FieldWriter<'a, REG, 8>;
///Field `SLOW_CLK_NEXT_EDGE` reader -
pub type SLOW_CLK_NEXT_EDGE_R = crate::BitReader;
///Field `SLOW_CLK_NEXT_EDGE` writer -
...
///`read()` method returns [`slow_clk_conf::R`](R) reader structure
impl crate::Readable for SLOW_CLK_CONF_SPEC {}
///`write(|w| ..)` method takes [`slow_clk_conf::W`](W) writer structure
