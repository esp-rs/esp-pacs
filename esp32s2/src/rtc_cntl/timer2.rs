///Register `TIMER2` reader
pub type R = crate::R<TIMER2_SPEC>;
///Register `TIMER2` writer
pub type W = crate::W<TIMER2_SPEC>;
///Field `ULPCP_TOUCH_START_WAIT` reader - Sets the waiting cycle (using the RTC slow clock) before the ULP co-processor / touch controller start to work.
pub type ULPCP_TOUCH_START_WAIT_R = crate::FieldReader<u16>;
///Field `ULPCP_TOUCH_START_WAIT` writer - Sets the waiting cycle (using the RTC slow clock) before the ULP co-processor / touch controller start to work.
pub type ULPCP_TOUCH_START_WAIT_W<'a, REG> = crate::FieldWriter<'a, REG, 9, u16>;
///Field `MIN_TIME_CK8M_OFF` reader - Sets the minimal cycle for 8 MHz clock (using the RTC slow clock) when powered down.
pub type MIN_TIME_CK8M_OFF_R = crate::FieldReader;
///Field `MIN_TIME_CK8M_OFF` writer - Sets the minimal cycle for 8 MHz clock (using the RTC slow clock) when powered down.
pub type MIN_TIME_CK8M_OFF_W<'a, REG> = crate::FieldWriter<'a, REG, 8>;
impl R {
    ///Bits 15:23 - Sets the waiting cycle (using the RTC slow clock) before the ULP co-processor / touch controller start to work.
    #[inline(always)]
    pub fn ulpcp_touch_start_wait(&self) -> ULPCP_TOUCH_START_WAIT_R {
        ULPCP_TOUCH_START_WAIT_R::new(((self.bits >> 15) & 0x01ff) as u16)
    }
    ///Bits 24:31 - Sets the minimal cycle for 8 MHz clock (using the RTC slow clock) when powered down.
    #[inline(always)]
...
///`read()` method returns [`timer2::R`](R) reader structure
impl crate::Readable for TIMER2_SPEC {}
///`write(|w| ..)` method takes [`timer2::W`](W) writer structure
