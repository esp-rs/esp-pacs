...
///Field `ADC_EOF_NUM` reader - Generate dma_in_suc_eof when sample cnt = spi_eof_num.
pub type ADC_EOF_NUM_R = crate::FieldReader<u16>;
///Field `ADC_EOF_NUM` writer - Generate dma_in_suc_eof when sample cnt = spi_eof_num.
pub type ADC_EOF_NUM_W<'a, REG> = crate::FieldWriter<'a, REG, 16, u16>;
///Field `ADC_RESET_FSM` reader - Reset DIG ADC CTRL status.
pub type ADC_RESET_FSM_R = crate::BitReader;
///Field `ADC_RESET_FSM` writer - Reset DIG ADC CTRL status.
pub type ADC_RESET_FSM_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `ADC_TRANS` reader - Set this bit, DIG ADC CTRL uses SPI DMA.
pub type ADC_TRANS_R = crate::BitReader;
///Field `ADC_TRANS` writer - Set this bit, DIG ADC CTRL uses SPI DMA.
