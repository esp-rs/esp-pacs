///Register `SAR_MEAS1_CTRL2` reader
pub type R = crate::R<SAR_MEAS1_CTRL2_SPEC>;
///Register `SAR_MEAS1_CTRL2` writer
...
///Field `MEAS1_DONE_SAR` reader - Indicate SAR ADC1 conversion is done.
pub type MEAS1_DONE_SAR_R = crate::BitReader;
///Field `MEAS1_START_SAR` reader - SAR ADC1 controller (in RTC) starts conversion, active only when SENS_MEAS1_START_FORCE = 1.
pub type MEAS1_START_SAR_R = crate::BitReader;
///Field `MEAS1_START_SAR` writer - SAR ADC1 controller (in RTC) starts conversion, active only when SENS_MEAS1_START_FORCE = 1.
pub type MEAS1_START_SAR_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `MEAS1_START_FORCE` reader - 1: SAR ADC1 controller (in RTC) is started by software. 0: SAR ADC1 controller is started by ULP coprocessor.
pub type MEAS1_START_FORCE_R = crate::BitReader;
///Field `MEAS1_START_FORCE` writer - 1: SAR ADC1 controller (in RTC) is started by software. 0: SAR ADC1 controller is started by ULP coprocessor.
pub type MEAS1_START_FORCE_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `SAR1_EN_PAD` reader - SAR ADC1 pad enable bitmap, active only when SENS_SAR1_EN_PAD_FORCE = 1.
pub type SAR1_EN_PAD_R = crate::FieldReader<u16>;
///Field `SAR1_EN_PAD` writer - SAR ADC1 pad enable bitmap, active only when SENS_SAR1_EN_PAD_FORCE = 1.
pub type SAR1_EN_PAD_W<'a, REG> = crate::FieldWriter<'a, REG, 12, u16>;
///Field `SAR1_EN_PAD_FORCE` reader - 1: SAR ADC1 pad enable bitmap is controlled by software. 0: SAR ADC1 pad enable bitmap is controlled by ULP coprocessor.
pub type SAR1_EN_PAD_FORCE_R = crate::BitReader;
///Field `SAR1_EN_PAD_FORCE` writer - 1: SAR ADC1 pad enable bitmap is controlled by software. 0: SAR ADC1 pad enable bitmap is controlled by ULP coprocessor.
...
impl R {
    ...
    ///Bit 16 - Indicate SAR ADC1 conversion is done.
    #[inline(always)]
    pub fn meas1_done_sar(&self) -> MEAS1_DONE_SAR_R {
        MEAS1_DONE_SAR_R::new(((self.bits >> 16) & 1) != 0)
    }
    ///Bit 17 - SAR ADC1 controller (in RTC) starts conversion, active only when SENS_MEAS1_START_FORCE = 1.
    #[inline(always)]
    pub fn meas1_start_sar(&self) -> MEAS1_START_SAR_R {
        MEAS1_START_SAR_R::new(((self.bits >> 17) & 1) != 0)
    }
    ///Bit 18 - 1: SAR ADC1 controller (in RTC) is started by software. 0: SAR ADC1 controller is started by ULP coprocessor.
    #[inline(always)]
    pub fn meas1_start_force(&self) -> MEAS1_START_FORCE_R {
        MEAS1_START_FORCE_R::new(((self.bits >> 18) & 1) != 0)
    }
    ///Bits 19:30 - SAR ADC1 pad enable bitmap, active only when SENS_SAR1_EN_PAD_FORCE = 1.
    #[inline(always)]
    pub fn sar1_en_pad(&self) -> SAR1_EN_PAD_R {
        SAR1_EN_PAD_R::new(((self.bits >> 19) & 0x0fff) as u16)
    }
    ///Bit 31 - 1: SAR ADC1 pad enable bitmap is controlled by software. 0: SAR ADC1 pad enable bitmap is controlled by ULP coprocessor.
    #[inline(always)]
...
///`read()` method returns [`sar_meas1_ctrl2::R`](R) reader structure
impl crate::Readable for SAR_MEAS1_CTRL2_SPEC {}
///`write(|w| ..)` method takes [`sar_meas1_ctrl2::W`](W) writer structure
