///Register `TX_CONF1` reader
pub type R = crate::R<TX_CONF1_SPEC>;
///Register `TX_CONF1` writer
pub type W = crate::W<TX_CONF1_SPEC>;
///Field `TX_TDM_WS_WIDTH` reader - The width of tx_ws_out at idle level in TDM mode is (I2S_TX_TDM_WS_WIDTH\[8:0\] +1) * T_bck
pub type TX_TDM_WS_WIDTH_R = crate::FieldReader<u16>;
///Field `TX_TDM_WS_WIDTH` writer - The width of tx_ws_out at idle level in TDM mode is (I2S_TX_TDM_WS_WIDTH\[8:0\] +1) * T_bck
...
///Field `TX_BITS_MOD` writer - Set the bits to configure the valid data bit length of I2S transmitter channel. 7: all the valid channel data is in 8-bit-mode. 15: all the valid channel data is in 16-bit-mode. 23: all the valid channel data is in 24-bit-mode. 31:all the valid channel data is in 32-bit-mode.
pub type TX_BITS_MOD_W<'a, REG> = crate::FieldWriter<'a, REG, 5>;
///Field `TX_HALF_SAMPLE_BITS` reader - I2S Tx half sample bits -1.
pub type TX_HALF_SAMPLE_BITS_R = crate::FieldReader;
///Field `TX_HALF_SAMPLE_BITS` writer - I2S Tx half sample bits -1.
pub type TX_HALF_SAMPLE_BITS_W<'a, REG> = crate::FieldWriter<'a, REG, 8>;
///Field `TX_TDM_CHAN_BITS` reader - The Tx bit number for each channel minus 1in TDM mode.
pub type TX_TDM_CHAN_BITS_R = crate::FieldReader;
///Field `TX_TDM_CHAN_BITS` writer - The Tx bit number for each channel minus 1in TDM mode.
pub type TX_TDM_CHAN_BITS_W<'a, REG> = crate::FieldWriter<'a, REG, 5>;
impl R {
    ///Bits 0:8 - The width of tx_ws_out at idle level in TDM mode is (I2S_TX_TDM_WS_WIDTH\[8:0\] +1) * T_bck
    #[inline(always)]
    pub fn tx_tdm_ws_width(&self) -> TX_TDM_WS_WIDTH_R {
        TX_TDM_WS_WIDTH_R::new((self.bits & 0x01ff) as u16)
    }
    ///Bits 14:18 - Set the bits to configure the valid data bit length of I2S transmitter channel. 7: all the valid channel data is in 8-bit-mode. 15: all the valid channel data is in 16-bit-mode. 23: all the valid channel data is in 24-bit-mode. 31:all the valid channel data is in 32-bit-mode.
    #[inline(always)]
    pub fn tx_bits_mod(&self) -> TX_BITS_MOD_R {
        TX_BITS_MOD_R::new(((self.bits >> 14) & 0x1f) as u8)
    }
    ///Bits 19:26 - I2S Tx half sample bits -1.
    #[inline(always)]
    pub fn tx_half_sample_bits(&self) -> TX_HALF_SAMPLE_BITS_R {
        TX_HALF_SAMPLE_BITS_R::new(((self.bits >> 19) & 0xff) as u8)
    }
    ///Bits 27:31 - The Tx bit number for each channel minus 1in TDM mode.
    #[inline(always)]
...
///`read()` method returns [`tx_conf1::R`](R) reader structure
impl crate::Readable for TX_CONF1_SPEC {}
///`write(|w| ..)` method takes [`tx_conf1::W`](W) writer structure
