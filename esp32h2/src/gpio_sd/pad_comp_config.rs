///Register `PAD_COMP_CONFIG` reader
pub type R = crate::R<PAD_COMP_CONFIG_SPEC>;
///Register `PAD_COMP_CONFIG` writer
pub type W = crate::W<PAD_COMP_CONFIG_SPEC>;
///Field `XPD_COMP` reader - Pad compare enable bit.
pub type XPD_COMP_R = crate::BitReader;
///Field `XPD_COMP` writer - Pad compare enable bit.
pub type XPD_COMP_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `MODE_COMP` reader - 1 to enable external reference from PAD\[0\]. 0 to enable internal reference, meanwhile PAD\[0\] can be used as a regular GPIO.
pub type MODE_COMP_R = crate::BitReader;
///Field `MODE_COMP` writer - 1 to enable external reference from PAD\[0\]. 0 to enable internal reference, meanwhile PAD\[0\] can be used as a regular GPIO.
pub type MODE_COMP_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `DREF_COMP` reader - internal reference voltage tuning bit. 0V to 0.7*VDDPST step 0.1*VDDPST.
pub type DREF_COMP_R = crate::FieldReader;
///Field `DREF_COMP` writer - internal reference voltage tuning bit. 0V to 0.7*VDDPST step 0.1*VDDPST.
pub type DREF_COMP_W<'a, REG> = crate::FieldWriter<'a, REG, 3>;
///Field `ZERO_DET_MODE` reader - Zero Detect mode select.
pub type ZERO_DET_MODE_R = crate::FieldReader;
///Field `ZERO_DET_MODE` writer - Zero Detect mode select.
pub type ZERO_DET_MODE_W<'a, REG> = crate::FieldWriter<'a, REG, 2>;
impl R {
    ///Bit 0 - Pad compare enable bit.
    #[inline(always)]
    pub fn xpd_comp(&self) -> XPD_COMP_R {
        XPD_COMP_R::new((self.bits & 1) != 0)
    }
    ///Bit 1 - 1 to enable external reference from PAD\[0\]. 0 to enable internal reference, meanwhile PAD\[0\] can be used as a regular GPIO.
    #[inline(always)]
    pub fn mode_comp(&self) -> MODE_COMP_R {
        MODE_COMP_R::new(((self.bits >> 1) & 1) != 0)
    }
    ///Bits 2:4 - internal reference voltage tuning bit. 0V to 0.7*VDDPST step 0.1*VDDPST.
    #[inline(always)]
    pub fn dref_comp(&self) -> DREF_COMP_R {
        DREF_COMP_R::new(((self.bits >> 2) & 7) as u8)
    }
    ///Bits 5:6 - Zero Detect mode select.
    #[inline(always)]
...
///`read()` method returns [`pad_comp_config::R`](R) reader structure
impl crate::Readable for PAD_COMP_CONFIG_SPEC {}
///`write(|w| ..)` method takes [`pad_comp_config::W`](W) writer structure
