///Register `TX_ST0` reader
pub type R = crate::R<TX_ST0_SPEC>;
///Field `TX_CNT` reader - Indicates the cycle number of reading Tx FIFO.
pub type TX_CNT_R = crate::FieldReader;
///Field `TX_FIFO_RD_BIT_CNT` reader - Indicates the current read bit number from Tx FIFO.
pub type TX_FIFO_RD_BIT_CNT_R = crate::FieldReader<u32>;
impl R {
    ///Bits 6:12 - Indicates the cycle number of reading Tx FIFO.
    #[inline(always)]
    pub fn tx_cnt(&self) -> TX_CNT_R {
        TX_CNT_R::new(((self.bits >> 6) & 0x7f) as u8)
    }
    ///Bits 13:31 - Indicates the current read bit number from Tx FIFO.
    #[inline(always)]
...
///`read()` method returns [`tx_st0::R`](R) reader structure
