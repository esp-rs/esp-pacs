...
///Field `TX_FIFO_REMPTY` reader - The masked interrupt status of TX_FIFO_REMPTY_INT.
pub type TX_FIFO_REMPTY_R = crate::BitReader;
///Field `RX_FIFO_WOVF` reader - The masked interrupt status of RX_FIFO_WOVF_INT.
pub type RX_FIFO_WOVF_R = crate::BitReader;
///Field `TX_EOF` reader - The masked interrupt status of TX_EOF_INT.
pub type TX_EOF_R = crate::BitReader;
impl R {
    ///Bit 0 - The masked interrupt status of TX_FIFO_REMPTY_INT.
    #[inline(always)]
    pub fn tx_fifo_rempty(&self) -> TX_FIFO_REMPTY_R {
        TX_FIFO_REMPTY_R::new((self.bits & 1) != 0)
    }
    ///Bit 1 - The masked interrupt status of RX_FIFO_WOVF_INT.
    #[inline(always)]
    pub fn rx_fifo_wovf(&self) -> RX_FIFO_WOVF_R {
        RX_FIFO_WOVF_R::new(((self.bits >> 1) & 1) != 0)
    }
    ///Bit 2 - The masked interrupt status of TX_EOF_INT.
    #[inline(always)]
