///Register `SPI_SMEM_PMS%s_ADDR` reader
pub type R = crate::R<SPI_SMEM_PMS_ADDR_SPEC>;
///Register `SPI_SMEM_PMS%s_ADDR` writer
pub type W = crate::W<SPI_SMEM_PMS_ADDR_SPEC>;
///Field `S` reader - SPI1 external RAM ACE section %s start address value
pub type S_R = crate::FieldReader<u32>;
///Field `S` writer - SPI1 external RAM ACE section %s start address value
pub type S_W<'a, REG> = crate::FieldWriter<'a, REG, 26, u32>;
impl R {
    ///Bits 0:25 - SPI1 external RAM ACE section %s start address value
    #[inline(always)]
    pub fn s(&self) -> S_R {
...
///`read()` method returns [`spi_smem_pms_addr::R`](R) reader structure
impl crate::Readable for SPI_SMEM_PMS_ADDR_SPEC {}
///`write(|w| ..)` method takes [`spi_smem_pms_addr::W`](W) writer structure
