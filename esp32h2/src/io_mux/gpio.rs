...
impl R {
    ...
    ///Bits 5:6 - Select the drive strength of the pad during sleep mode. 0: ~5 mA. 1: ~10mA. 2: ~20mA. 3: ~40mA.
    #[inline(always)]
    ...
    ///Bit 15 - Enable filter for pin input signals. 1: Filter enabled. 0: Filter disabled.
    #[inline(always)]
    pub fn filter_en(&self) -> FILTER_EN_R {
        FILTER_EN_R::new(((self.bits >> 15) & 1) != 0)
    }
    ///Bit 16 - Software enables hysteresis function for the pad. 1: Hysteresis enabled. 0: Hysteresis disabled.
    #[inline(always)]
    pub fn hys_en(&self) -> HYS_EN_R {
        HYS_EN_R::new(((self.bits >> 16) & 1) != 0)
    }
    ///Bit 17 - Select enabling signals of the pad from software and efuse hardware. 1: Select enabling siganl from slftware. 0: Select enabling signal from efuse hardware.
    #[inline(always)]
