...
///Field `A_CBC_D` reader - Cycle-by-cycle mode action on PWM0A when fault event occurs and timer is decreasing. 0: do nothing, 1: force low, 2: force high, 3: toggle
pub type A_CBC_D_R = crate::FieldReader;
///Field `A_CBC_D` writer - Cycle-by-cycle mode action on PWM0A when fault event occurs and timer is decreasing. 0: do nothing, 1: force low, 2: force high, 3: toggle
pub type A_CBC_D_W<'a, REG> = crate::FieldWriter<'a, REG, 2>;
///Field `A_CBC_U` reader - Cycle-by-cycle mode action on PWM0A when fault event occurs and timer is increasing. 0: do nothing, 1: force low, 2: force high, 3: toggle
pub type A_CBC_U_R = crate::FieldReader;
///Field `A_CBC_U` writer - Cycle-by-cycle mode action on PWM0A when fault event occurs and timer is increasing. 0: do nothing, 1: force low, 2: force high, 3: toggle
pub type A_CBC_U_W<'a, REG> = crate::FieldWriter<'a, REG, 2>;
///Field `A_OST_D` reader - One-shot mode action on PWM0A when fault event occurs and timer is decreasing. 0: do nothing, 1: force low, 2: force high, 3: toggle
pub type A_OST_D_R = crate::FieldReader;
///Field `A_OST_D` writer - One-shot mode action on PWM0A when fault event occurs and timer is decreasing. 0: do nothing, 1: force low, 2: force high, 3: toggle
pub type A_OST_D_W<'a, REG> = crate::FieldWriter<'a, REG, 2>;
///Field `A_OST_U` reader - One-shot mode action on PWM0A when fault event occurs and timer is increasing. 0: do nothing, 1: force low, 2: force high, 3: toggle
pub type A_OST_U_R = crate::FieldReader;
///Field `A_OST_U` writer - One-shot mode action on PWM0A when fault event occurs and timer is increasing. 0: do nothing, 1: force low, 2: force high, 3: toggle
pub type A_OST_U_W<'a, REG> = crate::FieldWriter<'a, REG, 2>;
///Field `B_CBC_D` reader - Cycle-by-cycle mode action on PWM0B when fault event occurs and timer is decreasing. 0: do nothing, 1: force low, 2: force high, 3: toggle
pub type B_CBC_D_R = crate::FieldReader;
///Field `B_CBC_D` writer - Cycle-by-cycle mode action on PWM0B when fault event occurs and timer is decreasing. 0: do nothing, 1: force low, 2: force high, 3: toggle
pub type B_CBC_D_W<'a, REG> = crate::FieldWriter<'a, REG, 2>;
///Field `B_CBC_U` reader - Cycle-by-cycle mode action on PWM0B when fault event occurs and timer is increasing. 0: do nothing,1: force low, 2: force high, 3: toggle
pub type B_CBC_U_R = crate::FieldReader;
///Field `B_CBC_U` writer - Cycle-by-cycle mode action on PWM0B when fault event occurs and timer is increasing. 0: do nothing,1: force low, 2: force high, 3: toggle
pub type B_CBC_U_W<'a, REG> = crate::FieldWriter<'a, REG, 2>;
///Field `B_OST_D` reader - One-shot mode action on PWM0B when fault event occurs and timer is decreasing. 0: do nothing, 1: force low, 2: force high, 3: toggle
pub type B_OST_D_R = crate::FieldReader;
///Field `B_OST_D` writer - One-shot mode action on PWM0B when fault event occurs and timer is decreasing. 0: do nothing, 1: force low, 2: force high, 3: toggle
pub type B_OST_D_W<'a, REG> = crate::FieldWriter<'a, REG, 2>;
///Field `B_OST_U` reader - One-shot mode action on PWM0B when fault event occurs and timer is increasing. 0: do nothing, 1: force low, 2: force high, 3: toggle
pub type B_OST_U_R = crate::FieldReader;
///Field `B_OST_U` writer - One-shot mode action on PWM0B when fault event occurs and timer is increasing. 0: do nothing, 1: force low, 2: force high, 3: toggle
...
impl R {
    ...
    ///Bits 8:9 - Cycle-by-cycle mode action on PWM0A when fault event occurs and timer is decreasing. 0: do nothing, 1: force low, 2: force high, 3: toggle
    #[inline(always)]
    pub fn a_cbc_d(&self) -> A_CBC_D_R {
        A_CBC_D_R::new(((self.bits >> 8) & 3) as u8)
    }
    ///Bits 10:11 - Cycle-by-cycle mode action on PWM0A when fault event occurs and timer is increasing. 0: do nothing, 1: force low, 2: force high, 3: toggle
    #[inline(always)]
    pub fn a_cbc_u(&self) -> A_CBC_U_R {
        A_CBC_U_R::new(((self.bits >> 10) & 3) as u8)
    }
    ///Bits 12:13 - One-shot mode action on PWM0A when fault event occurs and timer is decreasing. 0: do nothing, 1: force low, 2: force high, 3: toggle
    #[inline(always)]
    pub fn a_ost_d(&self) -> A_OST_D_R {
        A_OST_D_R::new(((self.bits >> 12) & 3) as u8)
    }
    ///Bits 14:15 - One-shot mode action on PWM0A when fault event occurs and timer is increasing. 0: do nothing, 1: force low, 2: force high, 3: toggle
    #[inline(always)]
    pub fn a_ost_u(&self) -> A_OST_U_R {
        A_OST_U_R::new(((self.bits >> 14) & 3) as u8)
    }
    ///Bits 16:17 - Cycle-by-cycle mode action on PWM0B when fault event occurs and timer is decreasing. 0: do nothing, 1: force low, 2: force high, 3: toggle
    #[inline(always)]
    pub fn b_cbc_d(&self) -> B_CBC_D_R {
        B_CBC_D_R::new(((self.bits >> 16) & 3) as u8)
    }
    ///Bits 18:19 - Cycle-by-cycle mode action on PWM0B when fault event occurs and timer is increasing. 0: do nothing,1: force low, 2: force high, 3: toggle
    #[inline(always)]
    pub fn b_cbc_u(&self) -> B_CBC_U_R {
        B_CBC_U_R::new(((self.bits >> 18) & 3) as u8)
    }
    ///Bits 20:21 - One-shot mode action on PWM0B when fault event occurs and timer is decreasing. 0: do nothing, 1: force low, 2: force high, 3: toggle
    #[inline(always)]
    pub fn b_ost_d(&self) -> B_OST_D_R {
        B_OST_D_R::new(((self.bits >> 20) & 3) as u8)
    }
    ///Bits 22:23 - One-shot mode action on PWM0B when fault event occurs and timer is increasing. 0: do nothing, 1: force low, 2: force high, 3: toggle
    #[inline(always)]
