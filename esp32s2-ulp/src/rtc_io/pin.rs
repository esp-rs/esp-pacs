...
///Field `GPIO_PIN_PAD_DRIVER` reader - Pad driver selection. 0: normal output. 1: open drain.
pub type GPIO_PIN_PAD_DRIVER_R = crate::BitReader;
///Field `GPIO_PIN_PAD_DRIVER` writer - Pad driver selection. 0: normal output. 1: open drain.
pub type GPIO_PIN_PAD_DRIVER_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `GPIO_PIN_INT_TYPE` reader - GPIO interrupt type selection. 0: GPIO interrupt disabled. 1: rising edge trigger. 2: falling edge trigger. 3: any edge trigger. 4: low level trigger. 5: high level trigger.
pub type GPIO_PIN_INT_TYPE_R = crate::FieldReader;
///Field `GPIO_PIN_INT_TYPE` writer - GPIO interrupt type selection. 0: GPIO interrupt disabled. 1: rising edge trigger. 2: falling edge trigger. 3: any edge trigger. 4: low level trigger. 5: high level trigger.
pub type GPIO_PIN_INT_TYPE_W<'a, REG> = crate::FieldWriter<'a, REG, 3>;
///Field `GPIO_PIN_WAKEUP_ENABLE` reader - GPIO wake-up enable. This will only wake up ESP32-S2 from Light-sleep.
pub type GPIO_PIN_WAKEUP_ENABLE_R = crate::BitReader;
///Field `GPIO_PIN_WAKEUP_ENABLE` writer - GPIO wake-up enable. This will only wake up ESP32-S2 from Light-sleep.
...
impl core::fmt::Debug for R {
    fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
        f.debug_struct("PIN")
            .field("gpio_pin_pad_driver", &self.gpio_pin_pad_driver())
            .field("gpio_pin_int_type", &self.gpio_pin_int_type())
            .field("gpio_pin_wakeup_enable", &self.gpio_pin_wakeup_enable())
            .finish()
    }
}
impl W {
