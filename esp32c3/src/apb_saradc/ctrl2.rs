...
#[doc = "Field `MEAS_NUM_LIMIT` reader - enable max meas num"]
pub type MEAS_NUM_LIMIT_R = crate::BitReader;
#[doc = "Field `MEAS_NUM_LIMIT` writer - enable max meas num"]
...
impl core::fmt::Debug for R {
    fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
        f.debug_struct("CTRL2")
            .field("meas_num_limit", &self.meas_num_limit().bit())
            .field("max_meas_num", &self.max_meas_num().bits())
            .field("sar1_inv", &self.sar1_inv().bit())
            .field("sar2_inv", &self.sar2_inv().bit())
