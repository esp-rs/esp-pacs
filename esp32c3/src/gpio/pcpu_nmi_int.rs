...
///Field `PROCPU_NMI_INT` reader - GPIO PRO_CPU(not shielded) interrupt status register for GPIO0-25
pub type PROCPU_NMI_INT_R = crate::FieldReader<u32>;
impl R {
    ///Bits 0:25 - GPIO PRO_CPU(not shielded) interrupt status register for GPIO0-25
    #[inline(always)]
