...
///Field `FLASH_PES_COMMAND` reader - Program/Erase suspend command.
pub type FLASH_PES_COMMAND_R = crate::FieldReader;
///Field `FLASH_PES_COMMAND` writer - Program/Erase suspend command.
pub type FLASH_PES_COMMAND_W<'a, REG> = crate::FieldWriter<'a, REG, 8>;
///Field `WAIT_PESR_COMMAND` reader - Flash SUS/SUS1/SUS2 status bit read command. The command should be sent when SUS/SUS1/SUS2 bit should be checked to insure the suspend or resume status of flash.
...
///`reset()` method sets FLASH_SUS_CMD to value 0x0005_757a
