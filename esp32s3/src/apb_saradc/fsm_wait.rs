...
///Field `XPD_WAIT` reader - the cycle which saradc controller in xpd state
pub type XPD_WAIT_R = crate::FieldReader;
///Field `XPD_WAIT` writer - the cycle which saradc controller in xpd state
pub type XPD_WAIT_W<'a, REG> = crate::FieldWriter<'a, REG, 8>;
///Field `RSTB_WAIT` reader - the cycle which saradc controller in rst state
pub type RSTB_WAIT_R = crate::FieldReader;
///Field `RSTB_WAIT` writer - the cycle which saradc controller in rst state
pub type RSTB_WAIT_W<'a, REG> = crate::FieldWriter<'a, REG, 8>;
///Field `STANDBY_WAIT` reader - the cycle which saradc controller in standby state
pub type STANDBY_WAIT_R = crate::FieldReader;
///Field `STANDBY_WAIT` writer - the cycle which saradc controller in standby state
pub type STANDBY_WAIT_W<'a, REG> = crate::FieldWriter<'a, REG, 8>;
impl R {
    ///Bits 0:7 - the cycle which saradc controller in xpd state
    #[inline(always)]
    pub fn xpd_wait(&self) -> XPD_WAIT_R {
        XPD_WAIT_R::new((self.bits & 0xff) as u8)
    }
    ///Bits 8:15 - the cycle which saradc controller in rst state
    #[inline(always)]
    pub fn rstb_wait(&self) -> RSTB_WAIT_R {
        RSTB_WAIT_R::new(((self.bits >> 8) & 0xff) as u8)
    }
    ///Bits 16:23 - the cycle which saradc controller in standby state
    #[inline(always)]
