...
///Field `SET_RESULT_END` writer - After read result from upstream, then let hmac back to idle.
...
impl W {
    ///Bit 0 - After read result from upstream, then let hmac back to idle.
    #[inline(always)]
    #[must_use]
