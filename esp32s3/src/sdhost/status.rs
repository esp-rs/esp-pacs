...
///Field `FIFO_RX_WATERMARK` reader - FIFO reached Receive watermark level, not qualified with data transfer.
pub type FIFO_RX_WATERMARK_R = crate::BitReader;
///Field `FIFO_TX_WATERMARK` reader - FIFO reached Transmit watermark level, not qualified with data transfer.
pub type FIFO_TX_WATERMARK_R = crate::BitReader;
///Field `FIFO_EMPTY` reader - FIFO is empty status.
pub type FIFO_EMPTY_R = crate::BitReader;
///Field `FIFO_FULL` reader - FIFO is full status.
pub type FIFO_FULL_R = crate::BitReader;
///Field `COMMAND_FSM_STATES` reader - Command FSM states. 0: Idle; 1: Send init sequence; 2: Send cmd start bit; 3: Send cmd tx bit; 4: Send cmd index + arg; 5: Send cmd crc7; 6: Send cmd end bit; 7: Receive resp start bit; 8: Receive resp IRQ response; 9: Receive resp tx bit; 10: Receive resp cmd idx; 11: Receive resp data; 12: Receive resp crc7; 13: Receive resp end bit; 14: Cmd path wait NCC; 15: Wait, cmd-to-response turnaround.
pub type COMMAND_FSM_STATES_R = crate::FieldReader;
///Field `DATA_3_STATUS` reader - Raw selected sdhost_card_data\[3\], checks whether card is present. 0: card not present; 1: card present.
pub type DATA_3_STATUS_R = crate::BitReader;
///Field `DATA_BUSY` reader - Inverted version of raw selected sdhost_card_data\[0\]. 0: Card data not busy; 1: Card data busy.
pub type DATA_BUSY_R = crate::BitReader;
///Field `DATA_STATE_MC_BUSY` reader - Data transmit or receive state-machine is busy.
pub type DATA_STATE_MC_BUSY_R = crate::BitReader;
///Field `RESPONSE_INDEX` reader - Index of previous response, including any auto-stop sent by core.
pub type RESPONSE_INDEX_R = crate::FieldReader;
///Field `FIFO_COUNT` reader - FIFO count, number of filled locations in FIFO.
