...
///Field `DDR` reader - DDR mode selecton,1 bit for each card. 0-Non-DDR mdoe. 1-DDR mdoe.
pub type DDR_R = crate::FieldReader;
///Field `DDR` writer - DDR mode selecton,1 bit for each card. 0-Non-DDR mdoe. 1-DDR mdoe.
pub type DDR_W<'a, REG> = crate::FieldWriter<'a, REG, 2>;
impl R {
    ///Bits 16:17 - DDR mode selecton,1 bit for each card. 0-Non-DDR mdoe. 1-DDR mdoe.
    #[inline(always)]
...
///`read()` method returns [`uhs::R`](R) reader structure
impl crate::Readable for UHS_SPEC {}
///`write(|w| ..)` method takes [`uhs::W`](W) writer structure
