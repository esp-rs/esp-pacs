...
///Register `TIMING_CALI` writer
pub type W = crate::W<TIMING_CALI_SPEC>;
///Field `TIMING_CLK_ENA` reader - Set this bit to power on HCLK. When PLL is powered on, the frequency of HCLK equals to that of PLL. Otherwise, the frequency equals to that of XTAL.
pub type TIMING_CLK_ENA_R = crate::BitReader;
///Field `TIMING_CLK_ENA` writer - Set this bit to power on HCLK. When PLL is powered on, the frequency of HCLK equals to that of PLL. Otherwise, the frequency equals to that of XTAL.
pub type TIMING_CLK_ENA_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `TIMING_CALI` reader - Set this bit to add extra SPI_CLK cycles in DUMMY phase for all reading operations.
pub type TIMING_CALI_R = crate::BitReader;
///Field `TIMING_CALI` writer - Set this bit to add extra SPI_CLK cycles in DUMMY phase for all reading operations.
pub type TIMING_CALI_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `EXTRA_DUMMY_CYCLELEN` reader - Extra SPI_CLK cycles added in DUMMY phase for timing compensation, when SPI0 accesses to flash. Active when SPI_MEM_TIMING_CALI bit is set.
pub type EXTRA_DUMMY_CYCLELEN_R = crate::FieldReader;
///Field `EXTRA_DUMMY_CYCLELEN` writer - Extra SPI_CLK cycles added in DUMMY phase for timing compensation, when SPI0 accesses to flash. Active when SPI_MEM_TIMING_CALI bit is set.
...
///`write(|w| ..)` method takes [`timing_cali::W`](W) writer structure
