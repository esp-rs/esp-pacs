...
///Field `CACHE_SRAM_USR_WR_CMD_VALUE` reader - When SPI0 writes Ext_RAM, it is the command value of CMD phase.
pub type CACHE_SRAM_USR_WR_CMD_VALUE_R = crate::FieldReader<u16>;
///Field `CACHE_SRAM_USR_WR_CMD_VALUE` writer - When SPI0 writes Ext_RAM, it is the command value of CMD phase.
pub type CACHE_SRAM_USR_WR_CMD_VALUE_W<'a, REG> = crate::FieldWriter<'a, REG, 16, u16>;
///Field `CACHE_SRAM_USR_WR_CMD_BITLEN` reader - When SPI0 writes Ext_RAM, it is the length in bits of CMD phase. The register value shall be (bit_num-1).
pub type CACHE_SRAM_USR_WR_CMD_BITLEN_R = crate::FieldReader;
///Field `CACHE_SRAM_USR_WR_CMD_BITLEN` writer - When SPI0 writes Ext_RAM, it is the length in bits of CMD phase. The register value shall be (bit_num-1).
