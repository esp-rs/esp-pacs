///Register `IN_SRAM_SIZE_CH%s` reader
pub type R = crate::R<IN_SRAM_SIZE_CH_SPEC>;
///Register `IN_SRAM_SIZE_CH%s` writer
pub type W = crate::W<IN_SRAM_SIZE_CH_SPEC>;
///Field `IN_SIZE` reader - This register is used to configure the size of L2 Tx FIFO for Rx channel 0. 0:16 bytes. 1:24 bytes. 2:32 bytes. 3: 40 bytes. 4: 48 bytes. 5:56 bytes. 6: 64 bytes. 7: 72 bytes. 8: 80 bytes.
pub type IN_SIZE_R = crate::FieldReader;
///Field `IN_SIZE` writer - This register is used to configure the size of L2 Tx FIFO for Rx channel 0. 0:16 bytes. 1:24 bytes. 2:32 bytes. 3: 40 bytes. 4: 48 bytes. 5:56 bytes. 6: 64 bytes. 7: 72 bytes. 8: 80 bytes.
...
///`read()` method returns [`in_sram_size_ch::R`](R) reader structure
impl crate::Readable for IN_SRAM_SIZE_CH_SPEC {}
///`write(|w| ..)` method takes [`in_sram_size_ch::W`](W) writer structure
