...
impl core::fmt::Debug for R {
    fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
        f.debug_struct("RX_CONF")
            ...
            .field("rx_update", &self.rx_update().bit())
            .field("rx_mono_fst_vld", &self.rx_mono_fst_vld().bit())
            .field("rx_pcm_conf", &self.rx_pcm_conf().bits())
            .field("rx_pcm_bypass", &self.rx_pcm_bypass().bit())
            .field("rx_stop_mode", &self.rx_stop_mode().bits())
            .field("rx_left_align", &self.rx_left_align().bit())
            .field("rx_24_fill_en", &self.rx_24_fill_en().bit())
            .field("rx_ws_idle_pol", &self.rx_ws_idle_pol().bit())
            .field("rx_bit_order", &self.rx_bit_order().bit())
            .field("rx_tdm_en", &self.rx_tdm_en().bit())
            .field("rx_pdm_en", &self.rx_pdm_en().bit())
            .field("rx_pdm2pcm_en", &self.rx_pdm2pcm_en().bit())
            .field("rx_pdm_sinc_dsr_16_en", &self.rx_pdm_sinc_dsr_16_en().bit())
            .finish()
    }
}
#[cfg(feature = "impl-register-debug")]
