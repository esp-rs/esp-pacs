///Register `DCACHE_PRELOAD_ADDR` reader
pub type R = crate::R<DCACHE_PRELOAD_ADDR_SPEC>;
///Register `DCACHE_PRELOAD_ADDR` writer
pub type W = crate::W<DCACHE_PRELOAD_ADDR_SPEC>;
///Field `DCACHE_PRELOAD_ADDR` reader - The bits are used to configure the start virtual address for preload operation. It should be combined with DCACHE_PRELOAD_SIZE_REG.
pub type DCACHE_PRELOAD_ADDR_R = crate::FieldReader<u32>;
///Field `DCACHE_PRELOAD_ADDR` writer - The bits are used to configure the start virtual address for preload operation. It should be combined with DCACHE_PRELOAD_SIZE_REG.
...
///`read()` method returns [`dcache_preload_addr::R`](R) reader structure
impl crate::Readable for DCACHE_PRELOAD_ADDR_SPEC {}
///`write(|w| ..)` method takes [`dcache_preload_addr::W`](W) writer structure
