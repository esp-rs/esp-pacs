///Register `DCACHE_SYNC_SIZE` reader
pub type R = crate::R<DCACHE_SYNC_SIZE_SPEC>;
///Register `DCACHE_SYNC_SIZE` writer
pub type W = crate::W<DCACHE_SYNC_SIZE_SPEC>;
///Field `DCACHE_SYNC_SIZE` reader - The bits are used to configure the length for sync operations. The bits are the counts of cache block. It should be combined with DCACHE_SYNC_ADDR_REG.
pub type DCACHE_SYNC_SIZE_R = crate::FieldReader<u32>;
///Field `DCACHE_SYNC_SIZE` writer - The bits are used to configure the length for sync operations. The bits are the counts of cache block. It should be combined with DCACHE_SYNC_ADDR_REG.
pub type DCACHE_SYNC_SIZE_W<'a, REG> = crate::FieldWriter<'a, REG, 23, u32>;
impl R {
    ///Bits 0:22 - The bits are used to configure the length for sync operations. The bits are the counts of cache block. It should be combined with DCACHE_SYNC_ADDR_REG.
    #[inline(always)]
...
///`read()` method returns [`dcache_sync_size::R`](R) reader structure
impl crate::Readable for DCACHE_SYNC_SIZE_SPEC {}
///`write(|w| ..)` method takes [`dcache_sync_size::W`](W) writer structure
