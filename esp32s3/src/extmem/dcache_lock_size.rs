///Register `DCACHE_LOCK_SIZE` reader
pub type R = crate::R<DCACHE_LOCK_SIZE_SPEC>;
///Register `DCACHE_LOCK_SIZE` writer
pub type W = crate::W<DCACHE_LOCK_SIZE_SPEC>;
///Field `DCACHE_LOCK_SIZE` reader - The bits are used to configure the length for lock operations. The bits are the counts of cache block. It should be combined with DCACHE_LOCK_ADDR_REG.
pub type DCACHE_LOCK_SIZE_R = crate::FieldReader<u16>;
///Field `DCACHE_LOCK_SIZE` writer - The bits are used to configure the length for lock operations. The bits are the counts of cache block. It should be combined with DCACHE_LOCK_ADDR_REG.
pub type DCACHE_LOCK_SIZE_W<'a, REG> = crate::FieldWriter<'a, REG, 16, u16>;
impl R {
    ///Bits 0:15 - The bits are used to configure the length for lock operations. The bits are the counts of cache block. It should be combined with DCACHE_LOCK_ADDR_REG.
    #[inline(always)]
...
///`read()` method returns [`dcache_lock_size::R`](R) reader structure
impl crate::Readable for DCACHE_LOCK_SIZE_SPEC {}
///`write(|w| ..)` method takes [`dcache_lock_size::W`](W) writer structure
