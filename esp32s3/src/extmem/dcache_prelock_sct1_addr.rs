...
///Field `DCACHE_PRELOCK_SCT1_ADDR` reader - The bits are used to configure the second start virtual address of data prelock, which is combined with DCACHE_PRELOCK_SCT1_SIZE_REG
pub type DCACHE_PRELOCK_SCT1_ADDR_R = crate::FieldReader<u32>;
///Field `DCACHE_PRELOCK_SCT1_ADDR` writer - The bits are used to configure the second start virtual address of data prelock, which is combined with DCACHE_PRELOCK_SCT1_SIZE_REG
...
///`read()` method returns [`dcache_prelock_sct1_addr::R`](R) reader structure
impl crate::Readable for DCACHE_PRELOCK_SCT1_ADDR_SPEC {}
///`write(|w| ..)` method takes [`dcache_prelock_sct1_addr::W`](W) writer structure
