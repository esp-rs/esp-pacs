...
///Field `T_STRING` reader - sha t_string(used if and only if mode == sha_256/t)
pub type T_STRING_R = crate::FieldReader<u32>;
///Field `T_STRING` writer - sha t_string(used if and only if mode == sha_256/t)
pub type T_STRING_W<'a, REG> = crate::FieldWriter<'a, REG, 32, u32>;
impl R {
    ///Bits 0:31 - sha t_string(used if and only if mode == sha_256/t)
    #[inline(always)]
