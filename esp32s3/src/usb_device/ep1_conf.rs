...
///Field `SERIAL_IN_EP_DATA_FREE` reader - 1'b1: Indicate UART Tx FIFO is not full and can write data into in. After writing USB_DEVICE_WR_DONE, this bit would be 0 until data in UART Tx FIFO is read by USB Host.
pub type SERIAL_IN_EP_DATA_FREE_R = crate::BitReader;
///Field `SERIAL_OUT_EP_DATA_AVAIL` reader - 1'b1: Indicate there is data in UART Rx FIFO.
...
impl W {
    ///Bit 0 - Set this bit to indicate writing byte data to UART Tx FIFO is done.
    #[inline(always)]
    #[must_use]
...
///`read()` method returns [`ep1_conf::R`](R) reader structure
impl crate::Readable for EP1_CONF_SPEC {}
///`write(|w| ..)` method takes [`ep1_conf::W`](W) writer structure
