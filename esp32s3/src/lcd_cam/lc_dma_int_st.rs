...
///Field `LCD_VSYNC_INT_ST` reader - The status bit for LCD_CAM_LCD_VSYNC_INT interrupt.
pub type LCD_VSYNC_INT_ST_R = crate::BitReader;
///Field `LCD_TRANS_DONE_INT_ST` reader - The status bit for LCD_CAM_LCD_TRANS_DONE_INT interrupt.
pub type LCD_TRANS_DONE_INT_ST_R = crate::BitReader;
///Field `CAM_VSYNC_INT_ST` reader - The status bit for LCD_CAM_CAM_VSYNC_INT interrupt.
pub type CAM_VSYNC_INT_ST_R = crate::BitReader;
///Field `CAM_HS_INT_ST` reader - The status bit for LCD_CAM_CAM_HS_INT interrupt.
pub type CAM_HS_INT_ST_R = crate::BitReader;
impl R {
    ///Bit 0 - The status bit for LCD_CAM_LCD_VSYNC_INT interrupt.
    #[inline(always)]
    pub fn lcd_vsync_int_st(&self) -> LCD_VSYNC_INT_ST_R {
        LCD_VSYNC_INT_ST_R::new((self.bits & 1) != 0)
    }
    ///Bit 1 - The status bit for LCD_CAM_LCD_TRANS_DONE_INT interrupt.
    #[inline(always)]
    pub fn lcd_trans_done_int_st(&self) -> LCD_TRANS_DONE_INT_ST_R {
        LCD_TRANS_DONE_INT_ST_R::new(((self.bits >> 1) & 1) != 0)
    }
    ///Bit 2 - The status bit for LCD_CAM_CAM_VSYNC_INT interrupt.
    #[inline(always)]
    pub fn cam_vsync_int_st(&self) -> CAM_VSYNC_INT_ST_R {
        CAM_VSYNC_INT_ST_R::new(((self.bits >> 2) & 1) != 0)
    }
    ///Bit 3 - The status bit for LCD_CAM_CAM_HS_INT interrupt.
    #[inline(always)]
...
///`read()` method returns [`lc_dma_int_st::R`](R) reader structure
