...
///Field `CARRIER_LOW` reader - This register is used to configure carrier wave 's low level clock period for CHANNEL%s.
pub type CARRIER_LOW_R = crate::FieldReader<u16>;
///Field `CARRIER_LOW` writer - This register is used to configure carrier wave 's low level clock period for CHANNEL%s.
pub type CARRIER_LOW_W<'a, REG> = crate::FieldWriter<'a, REG, 16, u16>;
///Field `CARRIER_HIGH` reader - This register is used to configure carrier wave 's high level clock period for CHANNEL%s.
pub type CARRIER_HIGH_R = crate::FieldReader<u16>;
///Field `CARRIER_HIGH` writer - This register is used to configure carrier wave 's high level clock period for CHANNEL%s.
