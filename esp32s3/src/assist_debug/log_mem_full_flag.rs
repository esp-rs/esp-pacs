...
///Field `LOG_MEM_FULL_FLAG` reader - when it's 1,show that mem write loop morte than one time.
pub type LOG_MEM_FULL_FLAG_R = crate::BitReader;
///Field `LOG_MEM_FULL_FLAG` writer - when it's 1,show that mem write loop morte than one time.
