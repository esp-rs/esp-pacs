...
///Field `IDLE` reader - AES Idle register. Reads ’zero’ while the AES Accelerator is busy processing; reads ’one’ otherwise.
pub type IDLE_R = crate::BitReader;
impl R {
    ///Bit 0 - AES Idle register. Reads ’zero’ while the AES Accelerator is busy processing; reads ’one’ otherwise.
    #[inline(always)]
