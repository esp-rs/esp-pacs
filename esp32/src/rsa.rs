...
impl RegisterBlock {
    ...
    pub fn m_mem_iter(&self) -> impl Iterator<Item = &M_MEM> {
        (0..32).map(|n| unsafe { &*(self as *const Self).cast::<u8>().add(0).add(4 * n).cast() })
    }
    #[doc = "0x200..0x220 - Represents Z"]
    #[inline(always)]
