...
///Field `CH_TX_END(0-7)` reader - The interrupt state bit for channel %s's mt_ch%s_tx_end_int_raw when mt_ch%s_tx_end_int_ena is set to %s.
pub type CH_TX_END_R = crate::BitReader;
///Field `CH_RX_END(0-7)` reader - The interrupt state bit for channel %s's rmt_ch%s_rx_end_int_raw when rmt_ch%s_rx_end_int_ena is set to %s.
pub type CH_RX_END_R = crate::BitReader;
///Field `CH_ERR(0-7)` reader - The interrupt state bit for channel %s's rmt_ch%s_err_int_raw when rmt_ch%s_err_int_ena is set to %s.
pub type CH_ERR_R = crate::BitReader;
///Field `CH_TX_THR_EVENT(0-7)` reader - The interrupt state bit for channel %s's rmt_ch%s_tx_thr_event_int_raw when mt_ch%s_tx_thr_event_int_ena is set to 1.
