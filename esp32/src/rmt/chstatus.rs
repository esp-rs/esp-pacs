///Register `CH%sSTATUS` reader
pub type R = crate::R<CHSTATUS_SPEC>;
///Field `STATUS` reader - The status for channel0
pub type STATUS_R = crate::FieldReader<u32>;
///Field `MEM_WADDR_EX` reader - The current memory read address of channel0.
pub type MEM_WADDR_EX_R = crate::FieldReader<u16>;
///Field `MEM_RADDR_EX` reader - The current memory write address of channel0.
pub type MEM_RADDR_EX_R = crate::FieldReader<u16>;
///Field `STATE` reader - The channel0 state machine status register.3'h0 : idle, 3'h1 : send, 3'h2 : read memory, 3'h3 : receive, 3'h4 : wait.
pub type STATE_R = crate::FieldReader;
///Field `MEM_OWNER_ERR` reader - When channel0 is configured for receive mode, this bit will turn to high level if rmt_mem_owner register is not set to 1.
pub type MEM_OWNER_ERR_R = crate::BitReader;
///Field `MEM_FULL` reader - The memory full status bit for channel0 turns to high level when mem_waddr_ex is greater than or equal to the configuration range.
pub type MEM_FULL_R = crate::BitReader;
///Field `MEM_EMPTY` reader - The memory empty status bit for channel0. in acyclic mode, this bit turns to high level when mem_raddr_ex is greater than or equal to the configured range.
pub type MEM_EMPTY_R = crate::BitReader;
///Field `APB_MEM_WR_ERR` reader - The apb write memory status bit for channel0 turns to high level when the apb write address exceeds the configuration range.
pub type APB_MEM_WR_ERR_R = crate::BitReader;
///Field `APB_MEM_RD_ERR` reader - The apb read memory status bit for channel0 turns to high level when the apb read address exceeds the configuration range.
...
///`read()` method returns [`chstatus::R`](R) reader structure
impl crate::Readable for CHSTATUS_SPEC {}
///`reset()` method sets CH%sSTATUS to value 0
