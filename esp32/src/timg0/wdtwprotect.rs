...
///Field `WDT_WKEY` reader - If change its value from default then write protection is on.
pub type WDT_WKEY_R = crate::FieldReader<u32>;
///Field `WDT_WKEY` writer - If change its value from default then write protection is on.
pub type WDT_WKEY_W<'a, REG> = crate::FieldWriter<'a, REG, 32, u32>;
impl R {
    ///Bits 0:31 - If change its value from default then write protection is on.
    #[inline(always)]
