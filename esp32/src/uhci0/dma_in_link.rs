...
///Field `INLINK_ADDR` reader - This register stores the least 20 bits of the first in link descriptor's address.
pub type INLINK_ADDR_R = crate::FieldReader<u32>;
///Field `INLINK_ADDR` writer - This register stores the least 20 bits of the first in link descriptor's address.
pub type INLINK_ADDR_W<'a, REG> = crate::FieldWriter<'a, REG, 20, u32>;
///Field `INLINK_AUTO_RET` reader - 1:when a packet is wrong in link descriptor returns to the descriptor which is lately used.
pub type INLINK_AUTO_RET_R = crate::BitReader;
///Field `INLINK_AUTO_RET` writer - 1:when a packet is wrong in link descriptor returns to the descriptor which is lately used.
pub type INLINK_AUTO_RET_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `INLINK_STOP` reader - Set this bit to stop dealing with the in link descriptors.
pub type INLINK_STOP_R = crate::BitReader;
///Field `INLINK_STOP` writer - Set this bit to stop dealing with the in link descriptors.
pub type INLINK_STOP_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `INLINK_START` reader - Set this bit to start dealing with the in link descriptors.
pub type INLINK_START_R = crate::BitReader;
///Field `INLINK_START` writer - Set this bit to start dealing with the in link descriptors.
pub type INLINK_START_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `INLINK_RESTART` reader - Set this bit to mount on new in link descriptors
pub type INLINK_RESTART_R = crate::BitReader;
///Field `INLINK_RESTART` writer - Set this bit to mount on new in link descriptors
pub type INLINK_RESTART_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `INLINK_PARK` reader - 1:the in link descriptor's fsm is in idle state. 0:the in link descriptor's fsm is working
