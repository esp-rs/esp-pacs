...
///Field `AHB_TESTMODE` reader - bit2 is ahb bus test enable ,bit1 is used to choose wrtie(1) or read(0) mode. bit0 is used to choose test only once(1) or continue(0)
pub type AHB_TESTMODE_R = crate::FieldReader;
///Field `AHB_TESTMODE` writer - bit2 is ahb bus test enable ,bit1 is used to choose wrtie(1) or read(0) mode. bit0 is used to choose test only once(1) or continue(0)
pub type AHB_TESTMODE_W<'a, REG> = crate::FieldWriter<'a, REG, 3>;
///Field `AHB_TESTADDR` reader - The two bits represent ahb bus address bit\[20:19\]
pub type AHB_TESTADDR_R = crate::FieldReader;
///Field `AHB_TESTADDR` writer - The two bits represent ahb bus address bit\[20:19\]
