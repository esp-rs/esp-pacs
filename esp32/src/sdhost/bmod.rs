///Register `BMOD` reader
pub type R = crate::R<BMOD_SPEC>;
///Register `BMOD` writer
...
///Field `FB` writer - Fixed Burst. Controls whether the AHB Master interface performs fixed burst transfers or not. When set, the AHB will use only SINGLE, INCR4, INCR8 or INCR16 during start of normal burst transfers. When reset, the AHB will use SINGLE and INCR burst transfer operations.
pub type FB_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `DE` reader - IDMAC Enable. When set, the IDMAC is enabled.
pub type DE_R = crate::BitReader;
///Field `DE` writer - IDMAC Enable. When set, the IDMAC is enabled.
...
///Field `PBL` writer - Programmable Burst Length. These bits indicate the maximum number of beats to be performed in one IDMAC???Internal DMA Control???transaction. The IDMAC will always attempt to burst as specified in PBL each time it starts a burst transfer on the host bus. The permissible values are 1, 4, 8, 16, 32, 64, 128 and 256. This value is the mirror of MSIZE of FIFOTH register. In order to change this value, write the required value to FIFOTH register. This is an encode value as follows: 000: 1-byte transfer; 001: 4-byte transfer; 010: 8-byte transfer; 011: 16-byte transfer; 100: 32-byte transfer; 101: 64-byte transfer; 110: 128-byte transfer; 111: 256-byte transfer. PBL is a read-only value and is applicable only for data access, it does not apply to descriptor access.
...
///`read()` method returns [`bmod::R`](R) reader structure
impl crate::Readable for BMOD_SPEC {}
///`write(|w| ..)` method takes [`bmod::W`](W) writer structure
