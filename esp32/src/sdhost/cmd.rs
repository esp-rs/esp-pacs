...
impl core::fmt::Debug for R {
    fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
        f.debug_struct("CMD")
            .field("index", &self.index().bits())
            .field("response_expect", &self.response_expect().bit())
            .field("response_length", &self.response_length().bit())
            .field("check_response_crc", &self.check_response_crc().bit())
            .field("data_expected", &self.data_expected().bit())
            .field("read_write", &self.read_write().bit())
            .field("transfer_mode", &self.transfer_mode().bit())
            .field("send_auto_stop", &self.send_auto_stop().bit())
            .field("wait_prvdata_complete", &self.wait_prvdata_complete().bit())
            .field("stop_abort_cmd", &self.stop_abort_cmd().bit())
            .field("send_initialization", &self.send_initialization().bit())
            .field("card_number", &self.card_number().bits())
            .field(
                "update_clock_registers_only",
                &self.update_clock_registers_only().bit(),
            )
            .field("read_ceata_device", &self.read_ceata_device().bit())
            .field("ccs_expected", &self.ccs_expected().bit())
            .field("use_hole", &self.use_hole().bit())
            .field("start_cmd", &self.start_cmd().bit())
            .finish()
    }
}
#[cfg(feature = "impl-register-debug")]
