...
///Field `SLV_RDBUF_DBITLEN` reader - In the slave mode it is the length in bits for read-buffer operations. The register value shall be (bit_num-1).
pub type SLV_RDBUF_DBITLEN_R = crate::FieldReader<u32>;
///Field `SLV_RDBUF_DBITLEN` writer - In the slave mode it is the length in bits for read-buffer operations. The register value shall be (bit_num-1).
