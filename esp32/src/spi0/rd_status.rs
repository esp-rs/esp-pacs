...
///Field `STATUS` reader - In the slave mode, it is the status for master to read out.
pub type STATUS_R = crate::FieldReader<u16>;
///Field `STATUS` writer - In the slave mode, it is the status for master to read out.
pub type STATUS_W<'a, REG> = crate::FieldWriter<'a, REG, 16, u16>;
///Field `WB_MODE` reader - Mode bits in the flash fast read mode, it is combined with spi_fastrd_mode bit.
pub type WB_MODE_R = crate::FieldReader;
///Field `WB_MODE` writer - Mode bits in the flash fast read mode, it is combined with spi_fastrd_mode bit.
pub type WB_MODE_W<'a, REG> = crate::FieldWriter<'a, REG, 8>;
///Field `STATUS_EXT` reader - In the slave mode,it is the status for master to read out.
pub type STATUS_EXT_R = crate::FieldReader;
///Field `STATUS_EXT` writer - In the slave mode,it is the status for master to read out.
