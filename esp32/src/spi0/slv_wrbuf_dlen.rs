...
///Field `SLV_WRBUF_DBITLEN` reader - In the slave mode it is the length in bits for write-buffer operations. The register value shall be (bit_num-1).
pub type SLV_WRBUF_DBITLEN_R = crate::FieldReader<u32>;
///Field `SLV_WRBUF_DBITLEN` writer - In the slave mode it is the length in bits for write-buffer operations. The register value shall be (bit_num-1).
pub type SLV_WRBUF_DBITLEN_W<'a, REG> = crate::FieldWriter<'a, REG, 24, u32>;
impl R {
    ///Bits 0:23 - In the slave mode it is the length in bits for write-buffer operations. The register value shall be (bit_num-1).
    #[inline(always)]
...
impl core::fmt::Debug for R {
    fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
        f.debug_struct("SLV_WRBUF_DLEN")
            .field("slv_wrbuf_dbitlen", &self.slv_wrbuf_dbitlen())
            .finish()
    }
}
impl W {
