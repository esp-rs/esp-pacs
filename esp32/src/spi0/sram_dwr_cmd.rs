...
///Field `CACHE_SRAM_USR_WR_CMD_VALUE` reader - For SPI0 When cache mode is enable it is the write command value of command phase for SRAM.
pub type CACHE_SRAM_USR_WR_CMD_VALUE_R = crate::FieldReader<u16>;
///Field `CACHE_SRAM_USR_WR_CMD_VALUE` writer - For SPI0 When cache mode is enable it is the write command value of command phase for SRAM.
pub type CACHE_SRAM_USR_WR_CMD_VALUE_W<'a, REG> = crate::FieldWriter<'a, REG, 16, u16>;
///Field `CACHE_SRAM_USR_WR_CMD_BITLEN` reader - For SPI0 When cache mode is enable it is the in bits of command phase for SRAM. The register value shall be (bit_num-1).
pub type CACHE_SRAM_USR_WR_CMD_BITLEN_R = crate::FieldReader;
///Field `CACHE_SRAM_USR_WR_CMD_BITLEN` writer - For SPI0 When cache mode is enable it is the in bits of command phase for SRAM. The register value shall be (bit_num-1).
...
///`read()` method returns [`sram_dwr_cmd::R`](R) reader structure
impl crate::Readable for SRAM_DWR_CMD_SPEC {}
///`write(|w| ..)` method takes [`sram_dwr_cmd::W`](W) writer structure
