...
///Field `RX_MEM_CNT` reader - Refer to the rxfifo_cnt's description.
pub type RX_MEM_CNT_R = crate::FieldReader;
///Field `TX_MEM_CNT` reader - Refer to the txfifo_cnt's description.
pub type TX_MEM_CNT_R = crate::FieldReader;
impl R {
    ///Bits 0:2 - Refer to the rxfifo_cnt's description.
    #[inline(always)]
    pub fn rx_mem_cnt(&self) -> RX_MEM_CNT_R {
        RX_MEM_CNT_R::new((self.bits & 7) as u8)
    }
    ///Bits 3:5 - Refer to the txfifo_cnt's description.
    #[inline(always)]
...
///`read()` method returns [`mem_cnt_status::R`](R) reader structure
