...
///Field `PRE_IDLE_NUM` reader - This register is used to configure the idle duration time before the first at_cmd is received by receiver. when the the duration is less than this register value it will not take the next data received as at_cmd char.
pub type PRE_IDLE_NUM_R = crate::FieldReader<u32>;
///Field `PRE_IDLE_NUM` writer - This register is used to configure the idle duration time before the first at_cmd is received by receiver. when the the duration is less than this register value it will not take the next data received as at_cmd char.
...
///`read()` method returns [`at_cmd_precnt::R`](R) reader structure
impl crate::Readable for AT_CMD_PRECNT_SPEC {}
///`write(|w| ..)` method takes [`at_cmd_precnt::W`](W) writer structure
impl crate::Writable for AT_CMD_PRECNT_SPEC {
    type Safety = crate::Unsafe;
    const ZERO_TO_MODIFY_FIELDS_BITMAP: u32 = 0;
    const ONE_TO_MODIFY_FIELDS_BITMAP: u32 = 0;
}
///`reset()` method sets AT_CMD_PRECNT to value 0x0018_6a00
