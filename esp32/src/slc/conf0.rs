///Register `CONF0` reader
pub type R = crate::R<CONF0_SPEC>;
///Register `CONF0` writer
pub type W = crate::W<CONF0_SPEC>;
///Field `SLC0_TX_RST` reader -
pub type SLC0_TX_RST_R = crate::BitReader;
///Field `SLC0_TX_RST` writer -
pub type SLC0_TX_RST_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `SLC0_RX_RST` reader -
pub type SLC0_RX_RST_R = crate::BitReader;
///Field `SLC0_RX_RST` writer -
pub type SLC0_RX_RST_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `AHBM_FIFO_RST` reader -
pub type AHBM_FIFO_RST_R = crate::BitReader;
///Field `AHBM_FIFO_RST` writer -
pub type AHBM_FIFO_RST_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `AHBM_RST` reader -
pub type AHBM_RST_R = crate::BitReader;
///Field `AHBM_RST` writer -
pub type AHBM_RST_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `SLC0_TX_LOOP_TEST` reader -
pub type SLC0_TX_LOOP_TEST_R = crate::BitReader;
///Field `SLC0_TX_LOOP_TEST` writer -
pub type SLC0_TX_LOOP_TEST_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `SLC0_RX_LOOP_TEST` reader -
pub type SLC0_RX_LOOP_TEST_R = crate::BitReader;
///Field `SLC0_RX_LOOP_TEST` writer -
pub type SLC0_RX_LOOP_TEST_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `SLC0_RX_AUTO_WRBACK` reader -
pub type SLC0_RX_AUTO_WRBACK_R = crate::BitReader;
///Field `SLC0_RX_AUTO_WRBACK` writer -
pub type SLC0_RX_AUTO_WRBACK_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `SLC0_RX_NO_RESTART_CLR` reader -
pub type SLC0_RX_NO_RESTART_CLR_R = crate::BitReader;
///Field `SLC0_RX_NO_RESTART_CLR` writer -
pub type SLC0_RX_NO_RESTART_CLR_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `SLC0_RXDSCR_BURST_EN` reader -
pub type SLC0_RXDSCR_BURST_EN_R = crate::BitReader;
///Field `SLC0_RXDSCR_BURST_EN` writer -
pub type SLC0_RXDSCR_BURST_EN_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `SLC0_RXDATA_BURST_EN` reader -
pub type SLC0_RXDATA_BURST_EN_R = crate::BitReader;
///Field `SLC0_RXDATA_BURST_EN` writer -
pub type SLC0_RXDATA_BURST_EN_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `SLC0_RXLINK_AUTO_RET` reader -
pub type SLC0_RXLINK_AUTO_RET_R = crate::BitReader;
///Field `SLC0_RXLINK_AUTO_RET` writer -
pub type SLC0_RXLINK_AUTO_RET_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `SLC0_TXLINK_AUTO_RET` reader -
pub type SLC0_TXLINK_AUTO_RET_R = crate::BitReader;
///Field `SLC0_TXLINK_AUTO_RET` writer -
pub type SLC0_TXLINK_AUTO_RET_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `SLC0_TXDSCR_BURST_EN` reader -
pub type SLC0_TXDSCR_BURST_EN_R = crate::BitReader;
///Field `SLC0_TXDSCR_BURST_EN` writer -
pub type SLC0_TXDSCR_BURST_EN_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `SLC0_TXDATA_BURST_EN` reader -
pub type SLC0_TXDATA_BURST_EN_R = crate::BitReader;
///Field `SLC0_TXDATA_BURST_EN` writer -
pub type SLC0_TXDATA_BURST_EN_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `SLC0_TOKEN_AUTO_CLR` reader -
pub type SLC0_TOKEN_AUTO_CLR_R = crate::BitReader;
///Field `SLC0_TOKEN_AUTO_CLR` writer -
pub type SLC0_TOKEN_AUTO_CLR_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `SLC0_TOKEN_SEL` reader -
pub type SLC0_TOKEN_SEL_R = crate::BitReader;
///Field `SLC0_TOKEN_SEL` writer -
pub type SLC0_TOKEN_SEL_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `SLC1_TX_RST` reader -
pub type SLC1_TX_RST_R = crate::BitReader;
///Field `SLC1_TX_RST` writer -
pub type SLC1_TX_RST_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `SLC1_RX_RST` reader -
pub type SLC1_RX_RST_R = crate::BitReader;
///Field `SLC1_RX_RST` writer -
pub type SLC1_RX_RST_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `SLC0_WR_RETRY_MASK_EN` reader -
pub type SLC0_WR_RETRY_MASK_EN_R = crate::BitReader;
///Field `SLC0_WR_RETRY_MASK_EN` writer -
pub type SLC0_WR_RETRY_MASK_EN_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `SLC1_WR_RETRY_MASK_EN` reader -
pub type SLC1_WR_RETRY_MASK_EN_R = crate::BitReader;
///Field `SLC1_WR_RETRY_MASK_EN` writer -
pub type SLC1_WR_RETRY_MASK_EN_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `SLC1_TX_LOOP_TEST` reader -
pub type SLC1_TX_LOOP_TEST_R = crate::BitReader;
///Field `SLC1_TX_LOOP_TEST` writer -
pub type SLC1_TX_LOOP_TEST_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `SLC1_RX_LOOP_TEST` reader -
pub type SLC1_RX_LOOP_TEST_R = crate::BitReader;
///Field `SLC1_RX_LOOP_TEST` writer -
pub type SLC1_RX_LOOP_TEST_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `SLC1_RX_AUTO_WRBACK` reader -
pub type SLC1_RX_AUTO_WRBACK_R = crate::BitReader;
///Field `SLC1_RX_AUTO_WRBACK` writer -
pub type SLC1_RX_AUTO_WRBACK_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `SLC1_RX_NO_RESTART_CLR` reader -
pub type SLC1_RX_NO_RESTART_CLR_R = crate::BitReader;
///Field `SLC1_RX_NO_RESTART_CLR` writer -
pub type SLC1_RX_NO_RESTART_CLR_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `SLC1_RXDSCR_BURST_EN` reader -
pub type SLC1_RXDSCR_BURST_EN_R = crate::BitReader;
///Field `SLC1_RXDSCR_BURST_EN` writer -
pub type SLC1_RXDSCR_BURST_EN_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `SLC1_RXDATA_BURST_EN` reader -
pub type SLC1_RXDATA_BURST_EN_R = crate::BitReader;
///Field `SLC1_RXDATA_BURST_EN` writer -
pub type SLC1_RXDATA_BURST_EN_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `SLC1_RXLINK_AUTO_RET` reader -
pub type SLC1_RXLINK_AUTO_RET_R = crate::BitReader;
///Field `SLC1_RXLINK_AUTO_RET` writer -
pub type SLC1_RXLINK_AUTO_RET_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `SLC1_TXLINK_AUTO_RET` reader -
pub type SLC1_TXLINK_AUTO_RET_R = crate::BitReader;
///Field `SLC1_TXLINK_AUTO_RET` writer -
pub type SLC1_TXLINK_AUTO_RET_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `SLC1_TXDSCR_BURST_EN` reader -
pub type SLC1_TXDSCR_BURST_EN_R = crate::BitReader;
///Field `SLC1_TXDSCR_BURST_EN` writer -
pub type SLC1_TXDSCR_BURST_EN_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `SLC1_TXDATA_BURST_EN` reader -
pub type SLC1_TXDATA_BURST_EN_R = crate::BitReader;
///Field `SLC1_TXDATA_BURST_EN` writer -
pub type SLC1_TXDATA_BURST_EN_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `SLC1_TOKEN_AUTO_CLR` reader -
pub type SLC1_TOKEN_AUTO_CLR_R = crate::BitReader;
///Field `SLC1_TOKEN_AUTO_CLR` writer -
pub type SLC1_TOKEN_AUTO_CLR_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `SLC1_TOKEN_SEL` reader -
pub type SLC1_TOKEN_SEL_R = crate::BitReader;
///Field `SLC1_TOKEN_SEL` writer -
pub type SLC1_TOKEN_SEL_W<'a, REG> = crate::BitWriter<'a, REG>;
impl R {
    ///Bit 0
    #[inline(always)]
    pub fn slc0_tx_rst(&self) -> SLC0_TX_RST_R {
        SLC0_TX_RST_R::new((self.bits & 1) != 0)
    }
    ///Bit 1
    #[inline(always)]
    pub fn slc0_rx_rst(&self) -> SLC0_RX_RST_R {
        SLC0_RX_RST_R::new(((self.bits >> 1) & 1) != 0)
    }
    ///Bit 2
    #[inline(always)]
    pub fn ahbm_fifo_rst(&self) -> AHBM_FIFO_RST_R {
        AHBM_FIFO_RST_R::new(((self.bits >> 2) & 1) != 0)
    }
    ///Bit 3
    #[inline(always)]
    pub fn ahbm_rst(&self) -> AHBM_RST_R {
        AHBM_RST_R::new(((self.bits >> 3) & 1) != 0)
    }
    ///Bit 4
    #[inline(always)]
    pub fn slc0_tx_loop_test(&self) -> SLC0_TX_LOOP_TEST_R {
        SLC0_TX_LOOP_TEST_R::new(((self.bits >> 4) & 1) != 0)
    }
    ///Bit 5
    #[inline(always)]
    pub fn slc0_rx_loop_test(&self) -> SLC0_RX_LOOP_TEST_R {
        SLC0_RX_LOOP_TEST_R::new(((self.bits >> 5) & 1) != 0)
    }
    ///Bit 6
    #[inline(always)]
    pub fn slc0_rx_auto_wrback(&self) -> SLC0_RX_AUTO_WRBACK_R {
        SLC0_RX_AUTO_WRBACK_R::new(((self.bits >> 6) & 1) != 0)
    }
    ///Bit 7
    #[inline(always)]
    pub fn slc0_rx_no_restart_clr(&self) -> SLC0_RX_NO_RESTART_CLR_R {
        SLC0_RX_NO_RESTART_CLR_R::new(((self.bits >> 7) & 1) != 0)
    }
    ///Bit 8
    #[inline(always)]
    pub fn slc0_rxdscr_burst_en(&self) -> SLC0_RXDSCR_BURST_EN_R {
        SLC0_RXDSCR_BURST_EN_R::new(((self.bits >> 8) & 1) != 0)
    }
    ///Bit 9
    #[inline(always)]
    pub fn slc0_rxdata_burst_en(&self) -> SLC0_RXDATA_BURST_EN_R {
        SLC0_RXDATA_BURST_EN_R::new(((self.bits >> 9) & 1) != 0)
    }
    ///Bit 10
    #[inline(always)]
    pub fn slc0_rxlink_auto_ret(&self) -> SLC0_RXLINK_AUTO_RET_R {
        SLC0_RXLINK_AUTO_RET_R::new(((self.bits >> 10) & 1) != 0)
    }
    ///Bit 11
    #[inline(always)]
    pub fn slc0_txlink_auto_ret(&self) -> SLC0_TXLINK_AUTO_RET_R {
        SLC0_TXLINK_AUTO_RET_R::new(((self.bits >> 11) & 1) != 0)
    }
    ///Bit 12
    #[inline(always)]
    pub fn slc0_txdscr_burst_en(&self) -> SLC0_TXDSCR_BURST_EN_R {
        SLC0_TXDSCR_BURST_EN_R::new(((self.bits >> 12) & 1) != 0)
    }
    ///Bit 13
    #[inline(always)]
    pub fn slc0_txdata_burst_en(&self) -> SLC0_TXDATA_BURST_EN_R {
        SLC0_TXDATA_BURST_EN_R::new(((self.bits >> 13) & 1) != 0)
    }
    ///Bit 14
    #[inline(always)]
    pub fn slc0_token_auto_clr(&self) -> SLC0_TOKEN_AUTO_CLR_R {
        SLC0_TOKEN_AUTO_CLR_R::new(((self.bits >> 14) & 1) != 0)
    }
    ///Bit 15
    #[inline(always)]
    pub fn slc0_token_sel(&self) -> SLC0_TOKEN_SEL_R {
        SLC0_TOKEN_SEL_R::new(((self.bits >> 15) & 1) != 0)
    }
    ///Bit 16
    #[inline(always)]
    pub fn slc1_tx_rst(&self) -> SLC1_TX_RST_R {
        SLC1_TX_RST_R::new(((self.bits >> 16) & 1) != 0)
    }
    ///Bit 17
    #[inline(always)]
    pub fn slc1_rx_rst(&self) -> SLC1_RX_RST_R {
        SLC1_RX_RST_R::new(((self.bits >> 17) & 1) != 0)
    }
    ///Bit 18
    #[inline(always)]
    pub fn slc0_wr_retry_mask_en(&self) -> SLC0_WR_RETRY_MASK_EN_R {
        SLC0_WR_RETRY_MASK_EN_R::new(((self.bits >> 18) & 1) != 0)
    }
    ///Bit 19
    #[inline(always)]
    pub fn slc1_wr_retry_mask_en(&self) -> SLC1_WR_RETRY_MASK_EN_R {
        SLC1_WR_RETRY_MASK_EN_R::new(((self.bits >> 19) & 1) != 0)
    }
    ///Bit 20
    #[inline(always)]
    pub fn slc1_tx_loop_test(&self) -> SLC1_TX_LOOP_TEST_R {
        SLC1_TX_LOOP_TEST_R::new(((self.bits >> 20) & 1) != 0)
    }
    ///Bit 21
    #[inline(always)]
    pub fn slc1_rx_loop_test(&self) -> SLC1_RX_LOOP_TEST_R {
        SLC1_RX_LOOP_TEST_R::new(((self.bits >> 21) & 1) != 0)
    }
    ///Bit 22
    #[inline(always)]
    pub fn slc1_rx_auto_wrback(&self) -> SLC1_RX_AUTO_WRBACK_R {
        SLC1_RX_AUTO_WRBACK_R::new(((self.bits >> 22) & 1) != 0)
    }
    ///Bit 23
    #[inline(always)]
    pub fn slc1_rx_no_restart_clr(&self) -> SLC1_RX_NO_RESTART_CLR_R {
        SLC1_RX_NO_RESTART_CLR_R::new(((self.bits >> 23) & 1) != 0)
    }
    ///Bit 24
    #[inline(always)]
    pub fn slc1_rxdscr_burst_en(&self) -> SLC1_RXDSCR_BURST_EN_R {
        SLC1_RXDSCR_BURST_EN_R::new(((self.bits >> 24) & 1) != 0)
    }
    ///Bit 25
    #[inline(always)]
    pub fn slc1_rxdata_burst_en(&self) -> SLC1_RXDATA_BURST_EN_R {
        SLC1_RXDATA_BURST_EN_R::new(((self.bits >> 25) & 1) != 0)
    }
    ///Bit 26
    #[inline(always)]
    pub fn slc1_rxlink_auto_ret(&self) -> SLC1_RXLINK_AUTO_RET_R {
        SLC1_RXLINK_AUTO_RET_R::new(((self.bits >> 26) & 1) != 0)
    }
    ///Bit 27
    #[inline(always)]
    pub fn slc1_txlink_auto_ret(&self) -> SLC1_TXLINK_AUTO_RET_R {
        SLC1_TXLINK_AUTO_RET_R::new(((self.bits >> 27) & 1) != 0)
    }
    ///Bit 28
    #[inline(always)]
    pub fn slc1_txdscr_burst_en(&self) -> SLC1_TXDSCR_BURST_EN_R {
        SLC1_TXDSCR_BURST_EN_R::new(((self.bits >> 28) & 1) != 0)
    }
    ///Bit 29
    #[inline(always)]
    pub fn slc1_txdata_burst_en(&self) -> SLC1_TXDATA_BURST_EN_R {
        SLC1_TXDATA_BURST_EN_R::new(((self.bits >> 29) & 1) != 0)
    }
    ///Bit 30
    #[inline(always)]
    pub fn slc1_token_auto_clr(&self) -> SLC1_TOKEN_AUTO_CLR_R {
        SLC1_TOKEN_AUTO_CLR_R::new(((self.bits >> 30) & 1) != 0)
    }
    ///Bit 31
    #[inline(always)]
    pub fn slc1_token_sel(&self) -> SLC1_TOKEN_SEL_R {
        SLC1_TOKEN_SEL_R::new(((self.bits >> 31) & 1) != 0)
    }
}
#[cfg(feature = "impl-register-debug")]
impl core::fmt::Debug for R {
    fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
        f.debug_struct("CONF0")
            .field("slc0_tx_rst", &self.slc0_tx_rst())
            .field("slc0_rx_rst", &self.slc0_rx_rst())
            .field("ahbm_fifo_rst", &self.ahbm_fifo_rst())
            .field("ahbm_rst", &self.ahbm_rst())
            .field("slc0_tx_loop_test", &self.slc0_tx_loop_test())
            .field("slc0_rx_loop_test", &self.slc0_rx_loop_test())
            .field("slc0_rx_auto_wrback", &self.slc0_rx_auto_wrback())
            .field("slc0_rx_no_restart_clr", &self.slc0_rx_no_restart_clr())
            .field("slc0_rxdscr_burst_en", &self.slc0_rxdscr_burst_en())
            .field("slc0_rxdata_burst_en", &self.slc0_rxdata_burst_en())
            .field("slc0_rxlink_auto_ret", &self.slc0_rxlink_auto_ret())
            .field("slc0_txlink_auto_ret", &self.slc0_txlink_auto_ret())
            .field("slc0_txdscr_burst_en", &self.slc0_txdscr_burst_en())
            .field("slc0_txdata_burst_en", &self.slc0_txdata_burst_en())
            .field("slc0_token_auto_clr", &self.slc0_token_auto_clr())
            .field("slc0_token_sel", &self.slc0_token_sel())
            .field("slc1_tx_rst", &self.slc1_tx_rst())
            .field("slc1_rx_rst", &self.slc1_rx_rst())
            .field("slc0_wr_retry_mask_en", &self.slc0_wr_retry_mask_en())
            .field("slc1_wr_retry_mask_en", &self.slc1_wr_retry_mask_en())
            .field("slc1_tx_loop_test", &self.slc1_tx_loop_test())
            .field("slc1_rx_loop_test", &self.slc1_rx_loop_test())
            .field("slc1_rx_auto_wrback", &self.slc1_rx_auto_wrback())
            .field("slc1_rx_no_restart_clr", &self.slc1_rx_no_restart_clr())
            .field("slc1_rxdscr_burst_en", &self.slc1_rxdscr_burst_en())
            .field("slc1_rxdata_burst_en", &self.slc1_rxdata_burst_en())
            .field("slc1_rxlink_auto_ret", &self.slc1_rxlink_auto_ret())
            .field("slc1_txlink_auto_ret", &self.slc1_txlink_auto_ret())
            .field("slc1_txdscr_burst_en", &self.slc1_txdscr_burst_en())
            .field("slc1_txdata_burst_en", &self.slc1_txdata_burst_en())
            .field("slc1_token_auto_clr", &self.slc1_token_auto_clr())
            .field("slc1_token_sel", &self.slc1_token_sel())
            .finish()
    }
}
impl W {
    ///Bit 0
    #[inline(always)]
    #[must_use]
    pub fn slc0_tx_rst(&mut self) -> SLC0_TX_RST_W<CONF0_SPEC> {
        SLC0_TX_RST_W::new(self, 0)
    }
    ///Bit 1
    #[inline(always)]
    #[must_use]
    pub fn slc0_rx_rst(&mut self) -> SLC0_RX_RST_W<CONF0_SPEC> {
        SLC0_RX_RST_W::new(self, 1)
    }
    ///Bit 2
    #[inline(always)]
    #[must_use]
    pub fn ahbm_fifo_rst(&mut self) -> AHBM_FIFO_RST_W<CONF0_SPEC> {
        AHBM_FIFO_RST_W::new(self, 2)
    }
    ///Bit 3
    #[inline(always)]
    #[must_use]
    pub fn ahbm_rst(&mut self) -> AHBM_RST_W<CONF0_SPEC> {
        AHBM_RST_W::new(self, 3)
    }
    ///Bit 4
    #[inline(always)]
    #[must_use]
    pub fn slc0_tx_loop_test(&mut self) -> SLC0_TX_LOOP_TEST_W<CONF0_SPEC> {
        SLC0_TX_LOOP_TEST_W::new(self, 4)
    }
    ///Bit 5
    #[inline(always)]
    #[must_use]
    pub fn slc0_rx_loop_test(&mut self) -> SLC0_RX_LOOP_TEST_W<CONF0_SPEC> {
        SLC0_RX_LOOP_TEST_W::new(self, 5)
    }
    ///Bit 6
    #[inline(always)]
    #[must_use]
    pub fn slc0_rx_auto_wrback(&mut self) -> SLC0_RX_AUTO_WRBACK_W<CONF0_SPEC> {
        SLC0_RX_AUTO_WRBACK_W::new(self, 6)
    }
    ///Bit 7
    #[inline(always)]
    #[must_use]
    pub fn slc0_rx_no_restart_clr(&mut self) -> SLC0_RX_NO_RESTART_CLR_W<CONF0_SPEC> {
        SLC0_RX_NO_RESTART_CLR_W::new(self, 7)
    }
    ///Bit 8
    #[inline(always)]
    #[must_use]
    pub fn slc0_rxdscr_burst_en(&mut self) -> SLC0_RXDSCR_BURST_EN_W<CONF0_SPEC> {
        SLC0_RXDSCR_BURST_EN_W::new(self, 8)
    }
    ///Bit 9
    #[inline(always)]
    #[must_use]
    pub fn slc0_rxdata_burst_en(&mut self) -> SLC0_RXDATA_BURST_EN_W<CONF0_SPEC> {
        SLC0_RXDATA_BURST_EN_W::new(self, 9)
    }
    ///Bit 10
    #[inline(always)]
    #[must_use]
    pub fn slc0_rxlink_auto_ret(&mut self) -> SLC0_RXLINK_AUTO_RET_W<CONF0_SPEC> {
        SLC0_RXLINK_AUTO_RET_W::new(self, 10)
    }
    ///Bit 11
    #[inline(always)]
    #[must_use]
    pub fn slc0_txlink_auto_ret(&mut self) -> SLC0_TXLINK_AUTO_RET_W<CONF0_SPEC> {
        SLC0_TXLINK_AUTO_RET_W::new(self, 11)
    }
    ///Bit 12
    #[inline(always)]
    #[must_use]
    pub fn slc0_txdscr_burst_en(&mut self) -> SLC0_TXDSCR_BURST_EN_W<CONF0_SPEC> {
        SLC0_TXDSCR_BURST_EN_W::new(self, 12)
    }
    ///Bit 13
    #[inline(always)]
    #[must_use]
    pub fn slc0_txdata_burst_en(&mut self) -> SLC0_TXDATA_BURST_EN_W<CONF0_SPEC> {
        SLC0_TXDATA_BURST_EN_W::new(self, 13)
    }
    ///Bit 14
    #[inline(always)]
    #[must_use]
    pub fn slc0_token_auto_clr(&mut self) -> SLC0_TOKEN_AUTO_CLR_W<CONF0_SPEC> {
        SLC0_TOKEN_AUTO_CLR_W::new(self, 14)
    }
    ///Bit 15
    #[inline(always)]
    #[must_use]
    pub fn slc0_token_sel(&mut self) -> SLC0_TOKEN_SEL_W<CONF0_SPEC> {
        SLC0_TOKEN_SEL_W::new(self, 15)
    }
    ///Bit 16
    #[inline(always)]
    #[must_use]
    pub fn slc1_tx_rst(&mut self) -> SLC1_TX_RST_W<CONF0_SPEC> {
        SLC1_TX_RST_W::new(self, 16)
    }
    ///Bit 17
    #[inline(always)]
    #[must_use]
    pub fn slc1_rx_rst(&mut self) -> SLC1_RX_RST_W<CONF0_SPEC> {
        SLC1_RX_RST_W::new(self, 17)
    }
    ///Bit 18
    #[inline(always)]
    #[must_use]
    pub fn slc0_wr_retry_mask_en(&mut self) -> SLC0_WR_RETRY_MASK_EN_W<CONF0_SPEC> {
        SLC0_WR_RETRY_MASK_EN_W::new(self, 18)
    }
    ///Bit 19
    #[inline(always)]
    #[must_use]
    pub fn slc1_wr_retry_mask_en(&mut self) -> SLC1_WR_RETRY_MASK_EN_W<CONF0_SPEC> {
        SLC1_WR_RETRY_MASK_EN_W::new(self, 19)
    }
    ///Bit 20
    #[inline(always)]
    #[must_use]
    pub fn slc1_tx_loop_test(&mut self) -> SLC1_TX_LOOP_TEST_W<CONF0_SPEC> {
        SLC1_TX_LOOP_TEST_W::new(self, 20)
    }
    ///Bit 21
    #[inline(always)]
    #[must_use]
    pub fn slc1_rx_loop_test(&mut self) -> SLC1_RX_LOOP_TEST_W<CONF0_SPEC> {
        SLC1_RX_LOOP_TEST_W::new(self, 21)
    }
    ///Bit 22
    #[inline(always)]
    #[must_use]
    pub fn slc1_rx_auto_wrback(&mut self) -> SLC1_RX_AUTO_WRBACK_W<CONF0_SPEC> {
        SLC1_RX_AUTO_WRBACK_W::new(self, 22)
    }
    ///Bit 23
    #[inline(always)]
    #[must_use]
    pub fn slc1_rx_no_restart_clr(&mut self) -> SLC1_RX_NO_RESTART_CLR_W<CONF0_SPEC> {
        SLC1_RX_NO_RESTART_CLR_W::new(self, 23)
    }
    ///Bit 24
    #[inline(always)]
    #[must_use]
    pub fn slc1_rxdscr_burst_en(&mut self) -> SLC1_RXDSCR_BURST_EN_W<CONF0_SPEC> {
        SLC1_RXDSCR_BURST_EN_W::new(self, 24)
    }
    ///Bit 25
    #[inline(always)]
    #[must_use]
    pub fn slc1_rxdata_burst_en(&mut self) -> SLC1_RXDATA_BURST_EN_W<CONF0_SPEC> {
        SLC1_RXDATA_BURST_EN_W::new(self, 25)
    }
    ///Bit 26
    #[inline(always)]
    #[must_use]
    pub fn slc1_rxlink_auto_ret(&mut self) -> SLC1_RXLINK_AUTO_RET_W<CONF0_SPEC> {
        SLC1_RXLINK_AUTO_RET_W::new(self, 26)
    }
    ///Bit 27
    #[inline(always)]
    #[must_use]
    pub fn slc1_txlink_auto_ret(&mut self) -> SLC1_TXLINK_AUTO_RET_W<CONF0_SPEC> {
        SLC1_TXLINK_AUTO_RET_W::new(self, 27)
    }
    ///Bit 28
    #[inline(always)]
    #[must_use]
    pub fn slc1_txdscr_burst_en(&mut self) -> SLC1_TXDSCR_BURST_EN_W<CONF0_SPEC> {
        SLC1_TXDSCR_BURST_EN_W::new(self, 28)
    }
    ///Bit 29
    #[inline(always)]
    #[must_use]
    pub fn slc1_txdata_burst_en(&mut self) -> SLC1_TXDATA_BURST_EN_W<CONF0_SPEC> {
        SLC1_TXDATA_BURST_EN_W::new(self, 29)
    }
    ///Bit 30
    #[inline(always)]
    #[must_use]
    pub fn slc1_token_auto_clr(&mut self) -> SLC1_TOKEN_AUTO_CLR_W<CONF0_SPEC> {
        SLC1_TOKEN_AUTO_CLR_W::new(self, 30)
    }
    ///Bit 31
    #[inline(always)]
    #[must_use]
    pub fn slc1_token_sel(&mut self) -> SLC1_TOKEN_SEL_W<CONF0_SPEC> {
        SLC1_TOKEN_SEL_W::new(self, 31)
    }
}
/**

You can [`read`](crate::generic::Reg::read) this register and get [`conf0::R`](R).  You can [`reset`](crate::generic::Reg::reset), [`write`](crate::generic::Reg::write), [`write_with_zero`](crate::generic::Reg::write_with_zero) this register using [`conf0::W`](W). You can also [`modify`](crate::generic::Reg::modify) this register. See [API](https://docs.rs/svd2rust/#read--modify--write-api).*/
pub struct CONF0_SPEC;
impl crate::RegisterSpec for CONF0_SPEC {
    type Ux = u32;
}
///`read()` method returns [`conf0::R`](R) reader structure
impl crate::Readable for CONF0_SPEC {}
///`write(|w| ..)` method takes [`conf0::W`](W) writer structure
impl crate::Writable for CONF0_SPEC {
    type Safety = crate::Unsafe;
    const ZERO_TO_MODIFY_FIELDS_BITMAP: u32 = 0;
    const ONE_TO_MODIFY_FIELDS_BITMAP: u32 = 0;
}
///`reset()` method sets CONF0 to value 0xff3c_ff30
impl crate::Resettable for CONF0_SPEC {
    const RESET_VALUE: u32 = 0xff3c_ff30;
}
