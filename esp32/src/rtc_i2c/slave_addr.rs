...
///Field `SLAVE_ADDR` reader - local slave address
pub type SLAVE_ADDR_R = crate::FieldReader<u16>;
///Field `SLAVE_ADDR` writer - local slave address
pub type SLAVE_ADDR_W<'a, REG> = crate::FieldWriter<'a, REG, 15, u16>;
///Field `_10BIT` reader - Set if local slave address is 10-bit
pub type _10BIT_R = crate::BitReader;
///Field `_10BIT` writer - Set if local slave address is 10-bit
pub type _10BIT_W<'a, REG> = crate::BitWriter<'a, REG>;
impl R {
    ///Bits 0:14 - local slave address
    #[inline(always)]
    pub fn slave_addr(&self) -> SLAVE_ADDR_R {
        SLAVE_ADDR_R::new((self.bits & 0x7fff) as u16)
    }
    ///Bit 31 - Set if local slave address is 10-bit
    #[inline(always)]
