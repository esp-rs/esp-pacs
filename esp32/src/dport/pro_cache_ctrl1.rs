...
impl core::fmt::Debug for R {
    fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
        f.debug_struct("PRO_CACHE_CTRL1")
            .field("pro_cache_mask_iram0", &self.pro_cache_mask_iram0().bit())
            .field("pro_cache_mask_iram1", &self.pro_cache_mask_iram1().bit())
            .field("pro_cache_mask_irom0", &self.pro_cache_mask_irom0().bit())
            .field("pro_cache_mask_dram1", &self.pro_cache_mask_dram1().bit())
            .field("pro_cache_mask_drom0", &self.pro_cache_mask_drom0().bit())
            .field(
                "pro_cache_mask_opsdram",
                &self.pro_cache_mask_opsdram().bit(),
            )
            .field(
                "pro_cmmu_sram_page_mode",
                &self.pro_cmmu_sram_page_mode().bits(),
            )
            .field(
                "pro_cmmu_flash_page_mode",
                &self.pro_cmmu_flash_page_mode().bits(),
            )
            .field("pro_cmmu_force_on", &self.pro_cmmu_force_on().bit())
            .field("pro_cmmu_pd", &self.pro_cmmu_pd().bit())
            .field("pro_cache_mmu_ia_clr", &self.pro_cache_mmu_ia_clr().bit())
            .finish()
    }
}
#[cfg(feature = "impl-register-debug")]
