...
#[doc = "Field `DELAY_EVENT_TIME` reader - delay time after zero det int to trigger event for each channel"]
pub type DELAY_EVENT_TIME_R = crate::FieldReader<u16>;
#[doc = "Field `DELAY_EVENT_TIME` writer - delay time after zero det int to trigger event for each channel"]
pub type DELAY_EVENT_TIME_W<'a, REG> = crate::FieldWriter<'a, REG, 16, u16>;
impl R {
    #[doc = "Bits 0:15 - delay time after zero det int to trigger event for each channel"]
    #[inline(always)]
