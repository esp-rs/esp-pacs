...
impl R {
    #[doc = "Bits 0:15 - Transfer Size (XferSize) Indicates the transfer size in bytes for endpoint 0. The core interrupts the application only after it has exhausted the transfer size amount of data. The transfer size can be Set to the maximum packet size of the endpoint, to be interrupted at the end of each packet. The core decrements this field every time a packet is read from the RxFIFO and written to the external memory."]
    #[inline(always)]
    pub fn xfersize(&self) -> XFERSIZE_R {
        XFERSIZE_R::new((self.bits & 0xffff) as u16)
    }
    #[doc = "Bits 19:25 - Packet Count (PktCnt) This field is decremented to zero after a packet is written into the RxFIFO."]
    #[inline(always)]
...
impl W {
    #[doc = "Bits 0:15 - Transfer Size (XferSize) Indicates the transfer size in bytes for endpoint 0. The core interrupts the application only after it has exhausted the transfer size amount of data. The transfer size can be Set to the maximum packet size of the endpoint, to be interrupted at the end of each packet. The core decrements this field every time a packet is read from the RxFIFO and written to the external memory."]
    #[inline(always)]
    pub fn xfersize(&mut self) -> XFERSIZE_W<'_, DOEPTSIZ5_SPEC> {
        XFERSIZE_W::new(self, 0)
    }
    #[doc = "Bits 19:25 - Packet Count (PktCnt) This field is decremented to zero after a packet is written into the RxFIFO."]
    #[inline(always)]
    pub fn pktcnt(&mut self) -> PKTCNT_W<'_, DOEPTSIZ5_SPEC> {
