...
impl W {
    #[doc = "Bits 0:10 - Maximum Packet Size (MPS) The application must program this field with the maximum packet size for the current logical endpoint. This value is in bytes."]
    #[inline(always)]
    pub fn mps(&mut self) -> MPS_W<'_, DIEPCTL2_SPEC> {
        MPS_W::new(self, 0)
    }
    #[doc = "Bit 15 - USB Active Endpoint (USBActEP) Indicates whether this endpoint is active in the current configuration and interface. The core clears this bit for all endpoints (other than EP 0) after detecting a USB reset. After receiving the SetConfiguration and SetInterface commands, the application must program endpoint registers accordingly and set this bit."]
    #[inline(always)]
    pub fn usbactep(&mut self) -> USBACTEP_W<'_, DIEPCTL2_SPEC> {
        USBACTEP_W::new(self, 15)
    }
    #[doc = "Bits 18:19 - Endpoint Type (EPType) This is the transfer type supported by this logical endpoint. - 2'b00: Control - 2'b01: Isochronous - 2'b10: Bulk - 2'b11: Interrupt"]
    #[inline(always)]
    pub fn eptype(&mut self) -> EPTYPE_W<'_, DIEPCTL2_SPEC> {
        EPTYPE_W::new(self, 18)
    }
    #[doc = "Bit 21 - STALL Handshake (Stall) Applies to non-control, non-isochronous IN and OUT endpoints only. The application sets this bit to stall all tokens from the USB host to this endpoint. If a NAK bit, Global Non-periodic IN NAK, or Global OUT NAK is set along with this bit, the STALL bit takes priority. Only the application can clear this bit, never the core. Applies to control endpoints only. The application can only set this bit, and the core clears it, when a SETUP token is received for this endpoint. If a NAK bit, Global Non-periodic IN NAK, or Global OUT NAK is set along with this bit, the STALL bit takes priority. Irrespective of this bit's setting, the core always responds to SETUP data packets with an ACK handshake."]
    #[inline(always)]
    pub fn stall(&mut self) -> STALL_W<'_, DIEPCTL2_SPEC> {
        STALL_W::new(self, 21)
    }
    #[doc = "Bits 22:25 - TxFIFO Number (TxFNum) Shared FIFO Operation non-periodic endpoints must set this bit to zero. Periodic endpoints must map this to the corresponding Periodic TxFIFO number. - 4'h0: Non-Periodic TxFIFO - Others: Specified Periodic TxFIFO.number Note: An interrupt IN endpoint can be configured as a non-periodic endpoint for applications such as mass storage. The core treats an IN endpoint as a non-periodic endpoint if the TxFNum field is set to 0. Otherwise, a separate periodic FIFO must be allocated for an interrupt IN endpoint, and the number of this FIFO must be programmed into the TxFNum field. Configuring an interrupt IN endpoint as a non-periodic endpoint saves the extra periodic FIFO area. Dedicated FIFO Operation: These bits specify the FIFO number associated with this endpoint. Each active IN endpoint must be programmed to a separate FIFO number. This field is valid only for IN endpoints."]
    #[inline(always)]
    pub fn txfnum(&mut self) -> TXFNUM_W<'_, DIEPCTL2_SPEC> {
        TXFNUM_W::new(self, 22)
    }
    #[doc = "Bit 26 - Clear NAK (CNAK) A write to this bit clears the NAK bit for the endpoint."]
    #[inline(always)]
    pub fn cnak(&mut self) -> CNAK_W<'_, DIEPCTL2_SPEC> {
        CNAK_W::new(self, 26)
    }
    #[doc = "Bit 27 - Set NAK (SNAK) A write to this bit sets the NAK bit for the endpoint. Using this bit, the application can control the transmission of NAK handshakes on an endpoint. The core can also Set this bit for an endpoint after a SETUP packet is received on that endpoint."]
    #[inline(always)]
    pub fn snak(&mut self) -> SNAK_W<'_, DIEPCTL2_SPEC> {
        SNAK_W::new(self, 27)
    }
    #[doc = "Bit 28 - SetD0PID - Set DATA0 PID (SetD0PID) -- Applies to interrupt/bulk IN and OUT endpoints only. -- Writing to this field sets the Endpoint Data PID (DPID) field in this register to DATA0. -- This field is applicable both for Scatter/Gather DMA mode and non-Scatter/Gather DMA mode. In non-Scatter/Gather DMA mode: Set Even (micro)Frame (SetEvenFr) -- Applies to isochronous IN and OUT endpoints only. -- Writing to this field sets the Even/Odd (micro)Frame (EO_FrNum) field to even (micro)Frame. When Scatter/Gather DMA mode is enabled, this field is reserved. The frame number in which to send data is in the transmit descriptor structure. The frame in which to receive data is updated in receive descriptor structure."]
    #[inline(always)]
    pub fn setd0pid(&mut self) -> SETD0PID_W<'_, DIEPCTL2_SPEC> {
        SETD0PID_W::new(self, 28)
    }
    #[doc = "Bit 29 - SetD1PID - Set DATA1 PID (SetD1PID) -- Applies to interrupt and bulk IN and OUT endpoints only. -- Writing to this field sets the Endpoint Data PID (DPID) field in this register to DATA1. -- This field is applicable both for Scatter-Gather DMA mode and non Scatter-Gather DMA mode. - Set odd (micro)Frame (SetOddFr) -- Applies to isochronous IN and OUT endpoints only. -- Writing to this field sets the even and odd (micro)Frame (EO_FrNum) field to odd (micro)Frame. -- This field is not applicable for Scatter-Gather DMA mode."]
    #[inline(always)]
    pub fn setd1pid(&mut self) -> SETD1PID_W<'_, DIEPCTL2_SPEC> {
        SETD1PID_W::new(self, 29)
    }
    #[doc = "Bit 30 - Endpoint Disable (EPDis) Applies to IN and OUT endpoints. The application sets this bit to stop transmitting/receiving data on an endpoint, even before the transfer for that endpoint is complete. The application must wait for the Endpoint Disabled interrupt before treating the endpoint as disabled. The core clears this bit before setting the Endpoint Disabled interrupt. The application must set this bit only if Endpoint Enable is already set for this endpoint."]
    #[inline(always)]
    pub fn epdis(&mut self) -> EPDIS_W<'_, DIEPCTL2_SPEC> {
        EPDIS_W::new(self, 30)
    }
    #[doc = "Bit 31 - Endpoint Enable (EPEna) Applies to IN and OUT endpoints. - When Scatter/Gather DMA mode is enabled, -- For IN endpoints this bit indicates that the descriptor structure and data buffer with data ready to transmit is setup. -- For OUT endpoint it indicates that the descriptor structure and data buffer to receive data is setup. - When Scatter/Gather DMA mode is enabled such as for buffer-pointer based DMA mode: -- For IN endpoints, this bit indicates that data is ready to be transmitted on the endpoint. -- For OUT endpoints, this bit indicates that the application has allocated the memory to start receiving data from the USB. - The core clears this bit before setting any of the following interrupts on this endpoint: -- SETUP Phase Done -- Endpoint Disabled -- Transfer Completed Note: For control endpoints in DMA mode, this bit must be set to be able to transfer SETUP data packets in memory."]
    #[inline(always)]
    pub fn epena(&mut self) -> EPENA_W<'_, DIEPCTL2_SPEC> {
        EPENA_W::new(self, 31)
    }
}
