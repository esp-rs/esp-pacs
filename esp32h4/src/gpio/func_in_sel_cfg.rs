...
#[doc = "Field `IN_SEL` reader - Select a pin for input signal %s."]
pub type IN_SEL_R = crate::FieldReader;
#[doc = "Field `IN_SEL` writer - Select a pin for input signal %s."]
...
impl R {
    #[doc = "Bits 0:6 - Select a pin for input signal %s."]
    #[inline(always)]
    pub fn in_sel(&self) -> IN_SEL_R {
        IN_SEL_R::new((self.bits & 0x7f) as u8)
    }
    #[doc = "Bit 7 - Invert the input value. 0: not invert. 1: invert."]
    #[inline(always)]
    pub fn in_inv_sel(&self) -> IN_INV_SEL_R {
        IN_INV_SEL_R::new(((self.bits >> 7) & 1) != 0)
    }
    #[doc = "Bit 8 - Bypass GPIO matrix. 0: bypass. 1: route via GPIO matrix."]
    #[inline(always)]
