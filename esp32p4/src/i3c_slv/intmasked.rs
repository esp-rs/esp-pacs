///Register `INTMASKED` reader
pub type R = crate::R<INTMASKED_SPEC>;
///Field `STOP_MASK` reader - Interrupt on STOP state on the bus. See Start as the preferred interrupt when needed. This interrupt may not trigger for quick STOP/START combination, as it relates to the state of being stopped.
pub type STOP_MASK_R = crate::BitReader;
///Field `RXPEND_MASK` reader - Interrupt when receiving a message from Master, which is not being handled by the block (excludes CCCs being handled automatically). If FIFO, then RX fullness trigger. If DMA, then message end.
pub type RXPEND_MASK_R = crate::BitReader;
///Field `TXSEND_MASK` reader - NA
...
///`read()` method returns [`intmasked::R`](R) reader structure
