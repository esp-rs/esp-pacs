...
///Field `SLV_EVENT` writer - If set to non-0, will request an event. Once requested, STATUS.EVENT and EVDET will show the status as it progresses. Once completed, the field will automatically return to 0. Once non-0, only 0 can be written(to cancel) until done. 0: Normal mode. If set to 0 after was a non-0 value, will cancel if not already in flight. 1: start an IBI. This will try to push through an IBI on the bus. If data associate with the IBI, it will be drawn from the IBIDATA field. Note that if Time control is enabled, this will include anytime control related bytes further, the IBIDATA byte will have bit7 set to 1.
pub type SLV_EVENT_W<'a, REG> = crate::FieldWriter<'a, REG, 2>;
///Field `EXTDATA` reader - reserved
pub type EXTDATA_R = crate::BitReader;
///Field `EXTDATA` writer - reserved
pub type EXTDATA_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `MAPIDX` reader - Index of Dynamic Address that IBI is for. This is 0 for the main or base Dynamic Address, or can be any valid index.
pub type MAPIDX_R = crate::FieldReader;
///Field `MAPIDX` writer - Index of Dynamic Address that IBI is for. This is 0 for the main or base Dynamic Address, or can be any valid index.
pub type MAPIDX_W<'a, REG> = crate::FieldWriter<'a, REG, 4>;
///Field `IBIDATA` reader - Data byte to go with an IBI, if enabled for it. If enabled (was in BCR), then it is required.
pub type IBIDATA_R = crate::FieldReader;
///Field `IBIDATA` writer - Data byte to go with an IBI, if enabled for it. If enabled (was in BCR), then it is required.
pub type IBIDATA_W<'a, REG> = crate::FieldWriter<'a, REG, 8>;
///Field `PENDINT` reader - Should be set to the pending interrupt that GETSTATUS CCC will return. This should be maintained by the application if used and configured, as the Master will read this. If not configured, the GETSTATUS field will return 1 if an IBI is pending, and 0 otherwise.
pub type PENDINT_R = crate::FieldReader;
///Field `PENDINT` writer - Should be set to the pending interrupt that GETSTATUS CCC will return. This should be maintained by the application if used and configured, as the Master will read this. If not configured, the GETSTATUS field will return 1 if an IBI is pending, and 0 otherwise.
pub type PENDINT_W<'a, REG> = crate::FieldWriter<'a, REG, 4>;
///Field `ACTSTATE` reader - NA
pub type ACTSTATE_R = crate::FieldReader;
///Field `ACTSTATE` writer - NA
pub type ACTSTATE_W<'a, REG> = crate::FieldWriter<'a, REG, 2>;
///Field `VENDINFO` reader - NA
pub type VENDINFO_R = crate::FieldReader;
///Field `VENDINFO` writer - NA
pub type VENDINFO_W<'a, REG> = crate::FieldWriter<'a, REG, 8>;
impl R {
    ///Bits 0:1 - If set to non-0, will request an event. Once requested, STATUS.EVENT and EVDET will show the status as it progresses. Once completed, the field will automatically return to 0. Once non-0, only 0 can be written(to cancel) until done. 0: Normal mode. If set to 0 after was a non-0 value, will cancel if not already in flight. 1: start an IBI. This will try to push through an IBI on the bus. If data associate with the IBI, it will be drawn from the IBIDATA field. Note that if Time control is enabled, this will include anytime control related bytes further, the IBIDATA byte will have bit7 set to 1.
    #[inline(always)]
    ...
    ///Bits 4:7 - Index of Dynamic Address that IBI is for. This is 0 for the main or base Dynamic Address, or can be any valid index.
    #[inline(always)]
    pub fn mapidx(&self) -> MAPIDX_R {
        MAPIDX_R::new(((self.bits >> 4) & 0x0f) as u8)
    }
    ///Bits 8:15 - Data byte to go with an IBI, if enabled for it. If enabled (was in BCR), then it is required.
    #[inline(always)]
    pub fn ibidata(&self) -> IBIDATA_R {
        IBIDATA_R::new(((self.bits >> 8) & 0xff) as u8)
    }
    ///Bits 16:19 - Should be set to the pending interrupt that GETSTATUS CCC will return. This should be maintained by the application if used and configured, as the Master will read this. If not configured, the GETSTATUS field will return 1 if an IBI is pending, and 0 otherwise.
    #[inline(always)]
