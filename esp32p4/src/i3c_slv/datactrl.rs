///Register `DATACTRL` reader
pub type R = crate::R<DATACTRL_SPEC>;
///Register `DATACTRL` writer
pub type W = crate::W<DATACTRL_SPEC>;
///Field `FLUSHTB` writer - Flushes the from-bus buffer/FIFO. Not normally used
pub type FLUSHTB_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `FLUSHFB` writer - Flushes the to-bus buffer/FIFO. Used when Master terminates a to-bus (read) message prematurely
pub type FLUSHFB_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `UNLOCK` writer - If this bit is not written 1, the register bits from 7 to 4 are not changed on write.
pub type UNLOCK_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `TXTRIG` reader - Trigger level for tx emptiness when FIFOed, Affects interrupt and DMA(if enabled). The defaults is 3
pub type TXTRIG_R = crate::FieldReader;
///Field `TXTRIG` writer - Trigger level for tx emptiness when FIFOed, Affects interrupt and DMA(if enabled). The defaults is 3
pub type TXTRIG_W<'a, REG> = crate::FieldWriter<'a, REG, 2>;
///Field `RXTRIG` reader - Trigger level for rx fulless when FIFOed, Affects interrupt and DMA(if enabled). The defaults is 3
pub type RXTRIG_R = crate::FieldReader;
///Field `RXTRIG` writer - Trigger level for rx fulless when FIFOed, Affects interrupt and DMA(if enabled). The defaults is 3
pub type RXTRIG_W<'a, REG> = crate::FieldWriter<'a, REG, 2>;
///Field `TXCOUNT` reader - NA
pub type TXCOUNT_R = crate::FieldReader;
///Field `RXCOUNT` reader - NA
pub type RXCOUNT_R = crate::FieldReader;
///Field `TXFULL` reader - NA
pub type TXFULL_R = crate::BitReader;
///Field `RXEMPTY` reader - NA
pub type RXEMPTY_R = crate::BitReader;
impl R {
    ///Bits 4:5 - Trigger level for tx emptiness when FIFOed, Affects interrupt and DMA(if enabled). The defaults is 3
    #[inline(always)]
    pub fn txtrig(&self) -> TXTRIG_R {
        TXTRIG_R::new(((self.bits >> 4) & 3) as u8)
    }
    ///Bits 6:7 - Trigger level for rx fulless when FIFOed, Affects interrupt and DMA(if enabled). The defaults is 3
    #[inline(always)]
...
impl W {
    ///Bit 0 - Flushes the from-bus buffer/FIFO. Not normally used
    #[inline(always)]
    #[must_use]
    pub fn flushtb(&mut self) -> FLUSHTB_W<DATACTRL_SPEC> {
        FLUSHTB_W::new(self, 0)
    }
    ///Bit 1 - Flushes the to-bus buffer/FIFO. Used when Master terminates a to-bus (read) message prematurely
    #[inline(always)]
    #[must_use]
    pub fn flushfb(&mut self) -> FLUSHFB_W<DATACTRL_SPEC> {
        FLUSHFB_W::new(self, 1)
    }
    ///Bit 3 - If this bit is not written 1, the register bits from 7 to 4 are not changed on write.
    #[inline(always)]
    #[must_use]
...
///`read()` method returns [`datactrl::R`](R) reader structure
impl crate::Readable for DATACTRL_SPEC {}
///`write(|w| ..)` method takes [`datactrl::W`](W) writer structure
