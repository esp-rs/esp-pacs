...
///Field `TX_FIFO_RESET` writer - Set this bit to reset Tx AFIFO
...
///Field `TX_STOP_EN` reader - Set this bit to stop disable output BCK signal and WS signal when tx FIFO is emtpy
...
///Field `TX_BIG_ENDIAN` reader - I2S Tx byte endian, 1: low addr value to high addr. 0: low addr with low addr value.
pub type TX_BIG_ENDIAN_R = crate::BitReader;
///Field `TX_BIG_ENDIAN` writer - I2S Tx byte endian, 1: low addr value to high addr. 0: low addr with low addr value.
pub type TX_BIG_ENDIAN_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `TX_UPDATE` reader - Set 1 to update I2S TX registers from APB clock domain to I2S TX clock domain. This bit will be cleared by hardware after update register done.
pub type TX_UPDATE_R = crate::BitReader;
///Field `TX_UPDATE` writer - Set 1 to update I2S TX registers from APB clock domain to I2S TX clock domain. This bit will be cleared by hardware after update register done.
...
///Field `TX_MONO_FST_VLD` writer - 1: The first channel data value is valid in I2S TX mono mode. 0: The second channel data value is valid in I2S TX mono mode.
...
///Field `TX_PCM_CONF` writer - I2S TX compress/decompress configuration bit. &amp; 0 (atol): A-Law decompress, 1 (ltoa) : A-Law compress, 2 (utol) : u-Law decompress, 3 (ltou) : u-Law compress. &amp;
...
///Field `TX_BCK_NO_DLY` reader - 1: BCK is not delayed to generate pos/neg edge in master mode. 0: BCK is delayed to generate pos/neg edge in master mode.
pub type TX_BCK_NO_DLY_R = crate::BitReader;
///Field `TX_BCK_NO_DLY` writer - 1: BCK is not delayed to generate pos/neg edge in master mode. 0: BCK is delayed to generate pos/neg edge in master mode.
pub type TX_BCK_NO_DLY_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `TX_LEFT_ALIGN` reader - 1: I2S TX left alignment mode. 0: I2S TX right alignment mode.
pub type TX_LEFT_ALIGN_R = crate::BitReader;
///Field `TX_LEFT_ALIGN` writer - 1: I2S TX left alignment mode. 0: I2S TX right alignment mode.
pub type TX_LEFT_ALIGN_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `TX_24_FILL_EN` reader - 1: Sent 32 bits in 24 channel bits mode. 0: Sent 24 bits in 24 channel bits mode
pub type TX_24_FILL_EN_R = crate::BitReader;
///Field `TX_24_FILL_EN` writer - 1: Sent 32 bits in 24 channel bits mode. 0: Sent 24 bits in 24 channel bits mode
pub type TX_24_FILL_EN_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `TX_WS_IDLE_POL` reader - 0: WS should be 0 when sending left channel data, and WS is 1in right channel. 1: WS should be 1 when sending left channel data, and WS is 0in right channel.
...
///Field `TX_BIT_ORDER` reader - I2S Tx bit endian. 1:small endian, the LSB is sent first. 0:big endian, the MSB is sent first.
...
///Field `TX_TDM_EN` reader - 1: Enable I2S TDM Tx mode . 0: Disable.
pub type TX_TDM_EN_R = crate::BitReader;
///Field `TX_TDM_EN` writer - 1: Enable I2S TDM Tx mode . 0: Disable.
pub type TX_TDM_EN_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `TX_PDM_EN` reader - 1: Enable I2S PDM Tx mode . 0: Disable.
pub type TX_PDM_EN_R = crate::BitReader;
///Field `TX_PDM_EN` writer - 1: Enable I2S PDM Tx mode . 0: Disable.
...
///Field `TX_CHAN_MOD` reader - I2S transmitter channel mode configuration bits.
pub type TX_CHAN_MOD_R = crate::FieldReader;
///Field `TX_CHAN_MOD` writer - I2S transmitter channel mode configuration bits.
pub type TX_CHAN_MOD_W<'a, REG> = crate::FieldWriter<'a, REG, 3>;
///Field `SIG_LOOPBACK` reader - Enable signal loop back mode with transmitter module and receiver module sharing the same WS and BCK signals.
...
impl R {
    ...
    ///Bit 4 - Set this bit to stop disable output BCK signal and WS signal when tx FIFO is emtpy
    #[inline(always)]
    pub fn tx_stop_en(&self) -> TX_STOP_EN_R {
        TX_STOP_EN_R::new(((self.bits >> 4) & 1) != 0)
    }
    ///Bit 5 - 1: The value of Left channel data is equal to the value of right channel data in I2S TX mono mode or TDM channel select mode. 0: The invalid channel data is reg_i2s_single_data in I2S TX mono mode or TDM channel select mode.
    #[inline(always)]
    pub fn tx_chan_equal(&self) -> TX_CHAN_EQUAL_R {
        TX_CHAN_EQUAL_R::new(((self.bits >> 5) & 1) != 0)
    }
    ///Bit 6 - Set this bit to enable transmitter in mono mode
    #[inline(always)]
    ...
    ///Bit 13 - Set this bit to enable transmitter in Phillips standard mode
    #[inline(always)]
    pub fn tx_msb_shift(&self) -> TX_MSB_SHIFT_R {
        TX_MSB_SHIFT_R::new(((self.bits >> 13) & 1) != 0)
    }
    ///Bit 14 - 1: BCK is not delayed to generate pos/neg edge in master mode. 0: BCK is delayed to generate pos/neg edge in master mode.
    #[inline(always)]
    ...
    ///Bits 21:26 - Bit clock configuration bits in transmitter mode.
    #[inline(always)]
    pub fn tx_bck_div_num(&self) -> TX_BCK_DIV_NUM_R {
        TX_BCK_DIV_NUM_R::new(((self.bits >> 21) & 0x3f) as u8)
    }
    ///Bits 27:29 - I2S transmitter channel mode configuration bits.
    #[inline(always)]
    pub fn tx_chan_mod(&self) -> TX_CHAN_MOD_R {
        TX_CHAN_MOD_R::new(((self.bits >> 27) & 7) as u8)
    }
    ///Bit 30 - Enable signal loop back mode with transmitter module and receiver module sharing the same WS and BCK signals.
    #[inline(always)]
...
impl W {
    ///Bit 0 - Set this bit to reset transmitter
    #[inline(always)]
    #[must_use]
    pub fn tx_reset(&mut self) -> TX_RESET_W<TX_CONF_SPEC> {
        TX_RESET_W::new(self, 0)
    }
    ///Bit 1 - Set this bit to reset Tx AFIFO
    #[inline(always)]
    #[must_use]
...
///`read()` method returns [`tx_conf::R`](R) reader structure
impl crate::Readable for TX_CONF_SPEC {}
///`write(|w| ..)` method takes [`tx_conf::W`](W) writer structure
