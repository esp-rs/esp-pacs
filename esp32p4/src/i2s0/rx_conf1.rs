...
///Field `RX_TDM_WS_WIDTH` reader - The width of rx_ws_out at idle level in TDM mode is (I2S_RX_TDM_WS_WIDTH\[8:0\] +1) * T_bck
pub type RX_TDM_WS_WIDTH_R = crate::FieldReader<u16>;
///Field `RX_TDM_WS_WIDTH` writer - The width of rx_ws_out at idle level in TDM mode is (I2S_RX_TDM_WS_WIDTH\[8:0\] +1) * T_bck
...
impl R {
    ///Bits 0:8 - The width of rx_ws_out at idle level in TDM mode is (I2S_RX_TDM_WS_WIDTH\[8:0\] +1) * T_bck
    #[inline(always)]
    pub fn rx_tdm_ws_width(&self) -> RX_TDM_WS_WIDTH_R {
        RX_TDM_WS_WIDTH_R::new((self.bits & 0x01ff) as u16)
    }
    ///Bits 14:18 - Set the bits to configure the valid data bit length of I2S receiver channel. 7: all the valid channel data is in 8-bit-mode. 15: all the valid channel data is in 16-bit-mode. 23: all the valid channel data is in 24-bit-mode. 31:all the valid channel data is in 32-bit-mode.
    #[inline(always)]
    pub fn rx_bits_mod(&self) -> RX_BITS_MOD_R {
        RX_BITS_MOD_R::new(((self.bits >> 14) & 0x1f) as u8)
    }
    ///Bits 19:26 - I2S Rx half sample bits -1.
    #[inline(always)]
    pub fn rx_half_sample_bits(&self) -> RX_HALF_SAMPLE_BITS_R {
        RX_HALF_SAMPLE_BITS_R::new(((self.bits >> 19) & 0xff) as u8)
    }
    ///Bits 27:31 - The Rx bit number for each channel minus 1in TDM mode.
    #[inline(always)]
...
///`read()` method returns [`rx_conf1::R`](R) reader structure
impl crate::Readable for RX_CONF1_SPEC {}
///`write(|w| ..)` method takes [`rx_conf1::W`](W) writer structure
