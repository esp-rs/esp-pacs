///Register `SYNC_ADDR` reader
pub type R = crate::R<SYNC_ADDR_SPEC>;
///Register `SYNC_ADDR` writer
pub type W = crate::W<SYNC_ADDR_SPEC>;
///Field `SYNC_ADDR` reader - Those bits are used to configure the start virtual address of the sync operation, which should be used together with CACHE_SYNC_SIZE_REG
pub type SYNC_ADDR_R = crate::FieldReader<u32>;
///Field `SYNC_ADDR` writer - Those bits are used to configure the start virtual address of the sync operation, which should be used together with CACHE_SYNC_SIZE_REG
...
///`read()` method returns [`sync_addr::R`](R) reader structure
impl crate::Readable for SYNC_ADDR_SPEC {}
///`write(|w| ..)` method takes [`sync_addr::W`](W) writer structure
