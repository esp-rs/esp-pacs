///Register `SYNC_SIZE` reader
pub type R = crate::R<SYNC_SIZE_SPEC>;
///Register `SYNC_SIZE` writer
pub type W = crate::W<SYNC_SIZE_SPEC>;
///Field `SYNC_SIZE` reader - Those bits are used to configure the size of the sync operation, which should be used together with CACHE_SYNC_ADDR_REG
pub type SYNC_SIZE_R = crate::FieldReader<u32>;
///Field `SYNC_SIZE` writer - Those bits are used to configure the size of the sync operation, which should be used together with CACHE_SYNC_ADDR_REG
pub type SYNC_SIZE_W<'a, REG> = crate::FieldWriter<'a, REG, 28, u32>;
impl R {
    ///Bits 0:27 - Those bits are used to configure the size of the sync operation, which should be used together with CACHE_SYNC_ADDR_REG
    #[inline(always)]
...
///`read()` method returns [`sync_size::R`](R) reader structure
impl crate::Readable for SYNC_SIZE_SPEC {}
///`write(|w| ..)` method takes [`sync_size::W`](W) writer structure
