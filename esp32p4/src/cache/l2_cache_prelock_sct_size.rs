...
///Register `L2_CACHE_PRELOCK_SCT_SIZE` writer
...
///Field `L2_CACHE_PRELOCK_SCT0_SIZE` writer - Those bits are used to configure the size of the first section of prelock on L2-Cache, which should be used together with L2_CACHE_PRELOCK_SCT0_ADDR_REG
...
///Field `L2_CACHE_PRELOCK_SCT1_SIZE` writer - Those bits are used to configure the size of the second section of prelock on L2-Cache, which should be used together with L2_CACHE_PRELOCK_SCT1_ADDR_REG
...
///`write(|w| ..)` method takes [`l2_cache_prelock_sct_size::W`](W) writer structure
