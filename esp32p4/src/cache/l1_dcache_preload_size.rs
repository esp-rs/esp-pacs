///Register `L1_DCACHE_PRELOAD_SIZE` reader
pub type R = crate::R<L1_DCACHE_PRELOAD_SIZE_SPEC>;
///Register `L1_DCACHE_PRELOAD_SIZE` writer
pub type W = crate::W<L1_DCACHE_PRELOAD_SIZE_SPEC>;
///Field `L1_DCACHE_PRELOAD_SIZE` reader - Those bits are used to configure the size of the first section of prelock on L1-DCache, which should be used together with L1_DCACHE_PRELOAD_ADDR_REG
pub type L1_DCACHE_PRELOAD_SIZE_R = crate::FieldReader<u16>;
///Field `L1_DCACHE_PRELOAD_SIZE` writer - Those bits are used to configure the size of the first section of prelock on L1-DCache, which should be used together with L1_DCACHE_PRELOAD_ADDR_REG
pub type L1_DCACHE_PRELOAD_SIZE_W<'a, REG> = crate::FieldWriter<'a, REG, 14, u16>;
impl R {
    ///Bits 0:13 - Those bits are used to configure the size of the first section of prelock on L1-DCache, which should be used together with L1_DCACHE_PRELOAD_ADDR_REG
    #[inline(always)]
...
///`read()` method returns [`l1_dcache_preload_size::R`](R) reader structure
impl crate::Readable for L1_DCACHE_PRELOAD_SIZE_SPEC {}
///`write(|w| ..)` method takes [`l1_dcache_preload_size::W`](W) writer structure
