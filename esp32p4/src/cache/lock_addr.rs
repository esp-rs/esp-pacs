///Register `LOCK_ADDR` reader
pub type R = crate::R<LOCK_ADDR_SPEC>;
///Register `LOCK_ADDR` writer
pub type W = crate::W<LOCK_ADDR_SPEC>;
///Field `LOCK_ADDR` reader - Those bits are used to configure the start virtual address of the lock/unlock operation, which should be used together with CACHE_LOCK_SIZE_REG
pub type LOCK_ADDR_R = crate::FieldReader<u32>;
///Field `LOCK_ADDR` writer - Those bits are used to configure the start virtual address of the lock/unlock operation, which should be used together with CACHE_LOCK_SIZE_REG
...
///`read()` method returns [`lock_addr::R`](R) reader structure
impl crate::Readable for LOCK_ADDR_SPEC {}
///`write(|w| ..)` method takes [`lock_addr::W`](W) writer structure
