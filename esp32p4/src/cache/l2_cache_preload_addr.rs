///Register `L2_CACHE_PRELOAD_ADDR` reader
pub type R = crate::R<L2_CACHE_PRELOAD_ADDR_SPEC>;
///Register `L2_CACHE_PRELOAD_ADDR` writer
pub type W = crate::W<L2_CACHE_PRELOAD_ADDR_SPEC>;
///Field `L2_CACHE_PRELOAD_ADDR` reader - Those bits are used to configure the start virtual address of preload on L2-Cache, which should be used together with L2_CACHE_PRELOAD_SIZE_REG
pub type L2_CACHE_PRELOAD_ADDR_R = crate::FieldReader<u32>;
///Field `L2_CACHE_PRELOAD_ADDR` writer - Those bits are used to configure the start virtual address of preload on L2-Cache, which should be used together with L2_CACHE_PRELOAD_SIZE_REG
pub type L2_CACHE_PRELOAD_ADDR_W<'a, REG> = crate::FieldWriter<'a, REG, 32, u32>;
impl R {
    ///Bits 0:31 - Those bits are used to configure the start virtual address of preload on L2-Cache, which should be used together with L2_CACHE_PRELOAD_SIZE_REG
    #[inline(always)]
...
///`read()` method returns [`l2_cache_preload_addr::R`](R) reader structure
impl crate::Readable for L2_CACHE_PRELOAD_ADDR_SPEC {}
///`write(|w| ..)` method takes [`l2_cache_preload_addr::W`](W) writer structure
