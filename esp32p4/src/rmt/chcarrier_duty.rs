...
///Field `CARRIER_LOW_CH` reader - This register is used to configure carrier wave 's low level clock period for CHANNEL%s.
pub type CARRIER_LOW_CH_R = crate::FieldReader<u16>;
///Field `CARRIER_LOW_CH` writer - This register is used to configure carrier wave 's low level clock period for CHANNEL%s.
pub type CARRIER_LOW_CH_W<'a, REG> = crate::FieldWriter<'a, REG, 16, u16>;
///Field `CARRIER_HIGH_CH` reader - This register is used to configure carrier wave 's high level clock period for CHANNEL%s.
pub type CARRIER_HIGH_CH_R = crate::FieldReader<u16>;
///Field `CARRIER_HIGH_CH` writer - This register is used to configure carrier wave 's high level clock period for CHANNEL%s.
