...
///Field `VADR_NUM_GT` reader - reg_vadr_num is greater than real interrupt st.
pub type VADR_NUM_GT_R = crate::BitReader;
///Field `VADR_NUM_LT` reader - reg_vadr_num is less than real interrupt st.
pub type VADR_NUM_LT_R = crate::BitReader;
///Field `DISCARD` reader - an incomplete frame of data was sent interrupt st.
pub type DISCARD_R = crate::BitReader;
///Field `CSI_BUF_OVERRUN` reader - buffer overrun interrupt st.
pub type CSI_BUF_OVERRUN_R = crate::BitReader;
///Field `CSI_ASYNC_FIFO_OVF` reader - buffer overflow interrupt st.
pub type CSI_ASYNC_FIFO_OVF_R = crate::BitReader;
///Field `DMA_CFG_HAS_UPDATED` reader - dma configuration update complete interrupt st.
pub type DMA_CFG_HAS_UPDATED_R = crate::BitReader;
impl R {
    ///Bit 0 - reg_vadr_num is greater than real interrupt st.
    #[inline(always)]
    pub fn vadr_num_gt(&self) -> VADR_NUM_GT_R {
        VADR_NUM_GT_R::new((self.bits & 1) != 0)
    }
    ///Bit 1 - reg_vadr_num is less than real interrupt st.
    #[inline(always)]
    pub fn vadr_num_lt(&self) -> VADR_NUM_LT_R {
        VADR_NUM_LT_R::new(((self.bits >> 1) & 1) != 0)
    }
    ///Bit 2 - an incomplete frame of data was sent interrupt st.
    #[inline(always)]
    pub fn discard(&self) -> DISCARD_R {
        DISCARD_R::new(((self.bits >> 2) & 1) != 0)
    }
    ///Bit 3 - buffer overrun interrupt st.
    #[inline(always)]
    pub fn csi_buf_overrun(&self) -> CSI_BUF_OVERRUN_R {
        CSI_BUF_OVERRUN_R::new(((self.bits >> 3) & 1) != 0)
    }
    ///Bit 4 - buffer overflow interrupt st.
    #[inline(always)]
    pub fn csi_async_fifo_ovf(&self) -> CSI_ASYNC_FIFO_OVF_R {
        CSI_ASYNC_FIFO_OVF_R::new(((self.bits >> 4) & 1) != 0)
    }
    ///Bit 5 - dma configuration update complete interrupt st.
    #[inline(always)]
