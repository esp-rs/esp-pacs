...
///Field `VADR_NUM` reader - vadr of frame data.
pub type VADR_NUM_R = crate::FieldReader<u16>;
///Field `VADR_NUM` writer - vadr of frame data.
pub type VADR_NUM_W<'a, REG> = crate::FieldWriter<'a, REG, 12, u16>;
///Field `HADR_NUM` reader - hadr of frame data.
pub type HADR_NUM_R = crate::FieldReader<u16>;
///Field `HADR_NUM` writer - hadr of frame data.
pub type HADR_NUM_W<'a, REG> = crate::FieldWriter<'a, REG, 12, u16>;
///Field `HAS_HSYNC_E` reader - 0: frame data doesn't contain hsync. 1: frame data contains hsync.
pub type HAS_HSYNC_E_R = crate::BitReader;
///Field `HAS_HSYNC_E` writer - 0: frame data doesn't contain hsync. 1: frame data contains hsync.
pub type HAS_HSYNC_E_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `VADR_NUM_CHECK` reader - 0: disable vadr check. 1: enable vadr check.
pub type VADR_NUM_CHECK_R = crate::BitReader;
///Field `VADR_NUM_CHECK` writer - 0: disable vadr check. 1: enable vadr check.
pub type VADR_NUM_CHECK_W<'a, REG> = crate::BitWriter<'a, REG>;
impl R {
    ///Bits 0:11 - vadr of frame data.
    #[inline(always)]
    pub fn vadr_num(&self) -> VADR_NUM_R {
        VADR_NUM_R::new((self.bits & 0x0fff) as u16)
    }
    ///Bits 12:23 - hadr of frame data.
    #[inline(always)]
    pub fn hadr_num(&self) -> HADR_NUM_R {
        HADR_NUM_R::new(((self.bits >> 12) & 0x0fff) as u16)
    }
    ///Bit 24 - 0: frame data doesn't contain hsync. 1: frame data contains hsync.
    #[inline(always)]
    pub fn has_hsync_e(&self) -> HAS_HSYNC_E_R {
        HAS_HSYNC_E_R::new(((self.bits >> 24) & 1) != 0)
    }
    ///Bit 25 - 0: disable vadr check. 1: enable vadr check.
    #[inline(always)]
