///Register `COMMAND_BUF_PORT` reader
pub type R = crate::R<COMMAND_BUF_PORT_SPEC>;
///Register `COMMAND_BUF_PORT` writer
pub type W = crate::W<COMMAND_BUF_PORT_SPEC>;
///Field `REG_COMMAND` reader - Contains a Command Descriptor structure that depends on the requested transfer type. Command Descriptor structure is used to schedule the transfers to devices on I3C bus.
pub type REG_COMMAND_R = crate::FieldReader<u32>;
///Field `REG_COMMAND` writer - Contains a Command Descriptor structure that depends on the requested transfer type. Command Descriptor structure is used to schedule the transfers to devices on I3C bus.
pub type REG_COMMAND_W<'a, REG> = crate::FieldWriter<'a, REG, 32, u32>;
impl R {
    ///Bits 0:31 - Contains a Command Descriptor structure that depends on the requested transfer type. Command Descriptor structure is used to schedule the transfers to devices on I3C bus.
    #[inline(always)]
...
///`read()` method returns [`command_buf_port::R`](R) reader structure
impl crate::Readable for COMMAND_BUF_PORT_SPEC {}
///`write(|w| ..)` method takes [`command_buf_port::W`](W) writer structure
