///Register `TX_DATA_PORT` reader
pub type R = crate::R<TX_DATA_PORT_SPEC>;
///Register `TX_DATA_PORT` writer
...
///Field `REG_TX_DATA_PORT` writer - Transmit Data Port. Transmit data is mapped to the Tx-data buffer and transmit data is always packed in 4-byte aligned data words. If the length of data transfer is not aligned to 4-bytes boundary, then there will be extra(unused) bytes(the additional data bytes have to be ignored) at the end of the transferred data. The valid data must be identified using the DATA_LENGTH filed in the Response Descriptor.
pub type REG_TX_DATA_PORT_W<'a, REG> = crate::FieldWriter<'a, REG, 32, u32>;
impl R {
    ///Bits 0:31 - Transmit Data Port. Transmit data is mapped to the Tx-data buffer and transmit data is always packed in 4-byte aligned data words. If the length of data transfer is not aligned to 4-bytes boundary, then there will be extra(unused) bytes(the additional data bytes have to be ignored) at the end of the transferred data. The valid data must be identified using the DATA_LENGTH filed in the Response Descriptor.
    #[inline(always)]
...
///`read()` method returns [`tx_data_port::R`](R) reader structure
impl crate::Readable for TX_DATA_PORT_SPEC {}
///`write(|w| ..)` method takes [`tx_data_port::W`](W) writer structure
