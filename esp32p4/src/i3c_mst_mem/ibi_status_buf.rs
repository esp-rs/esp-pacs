///Register `IBI_STATUS_BUF` reader
pub type R = crate::R<IBI_STATUS_BUF_SPEC>;
///Field `DATA_LENGTH` reader - This field represents the length of data received along with IBI, in bytes.
pub type DATA_LENGTH_R = crate::FieldReader;
///Field `IBI_ID` reader - IBI Identifier. The byte received after START which includes the address the R/W bit: Device address and R/W bit in case of Slave Interrupt or Master Request.
pub type IBI_ID_R = crate::FieldReader;
///Field `IBI_STS` reader - IBI received data/status. IBI Data register is mapped to the IBI Buffer. The IBI Data is always packed in4-byte aligned and put to the IBI Buffer. This register When read from, reads the data from the IBI buffer. IBI Status register when read from, returns the data from the IBI Buffer and indicates how the controller responded to incoming IBI(SIR, MR and HJ).
pub type IBI_STS_R = crate::BitReader;
impl R {
    ///Bits 0:7 - This field represents the length of data received along with IBI, in bytes.
    #[inline(always)]
    pub fn data_length(&self) -> DATA_LENGTH_R {
        DATA_LENGTH_R::new((self.bits & 0xff) as u8)
    }
    ///Bits 8:15 - IBI Identifier. The byte received after START which includes the address the R/W bit: Device address and R/W bit in case of Slave Interrupt or Master Request.
    #[inline(always)]
    pub fn ibi_id(&self) -> IBI_ID_R {
        IBI_ID_R::new(((self.bits >> 8) & 0xff) as u8)
    }
    ///Bit 28 - IBI received data/status. IBI Data register is mapped to the IBI Buffer. The IBI Data is always packed in4-byte aligned and put to the IBI Buffer. This register When read from, reads the data from the IBI buffer. IBI Status register when read from, returns the data from the IBI Buffer and indicates how the controller responded to incoming IBI(SIR, MR and HJ).
    #[inline(always)]
...
///`read()` method returns [`ibi_status_buf::R`](R) reader structure
