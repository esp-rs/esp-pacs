///Register `AT_CMD_POSTCNT_SYNC` reader
pub type R = crate::R<AT_CMD_POSTCNT_SYNC_SPEC>;
///Register `AT_CMD_POSTCNT_SYNC` writer
pub type W = crate::W<AT_CMD_POSTCNT_SYNC_SPEC>;
///Field `POST_IDLE_NUM` reader - This register is used to configure the duration time between the last at_cmd and the next data.
pub type POST_IDLE_NUM_R = crate::FieldReader<u16>;
///Field `POST_IDLE_NUM` writer - This register is used to configure the duration time between the last at_cmd and the next data.
...
///`read()` method returns [`at_cmd_postcnt_sync::R`](R) reader structure
impl crate::Readable for AT_CMD_POSTCNT_SYNC_SPEC {}
///`write(|w| ..)` method takes [`at_cmd_postcnt_sync::W`](W) writer structure
