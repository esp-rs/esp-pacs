///Register `RTCCALICFG1` reader
pub type R = crate::R<RTCCALICFG1_SPEC>;
///Field `RTC_CALI_CYCLING_DATA_VLD` reader - indicate periodic frequency calculation is done.
pub type RTC_CALI_CYCLING_DATA_VLD_R = crate::BitReader;
///Field `RTC_CALI_VALUE` reader - When one-shot or periodic frequency calculation is done, read this value to calculate RTC slow clock's frequency.
pub type RTC_CALI_VALUE_R = crate::FieldReader<u32>;
impl R {
    ///Bit 0 - indicate periodic frequency calculation is done.
    #[inline(always)]
    pub fn rtc_cali_cycling_data_vld(&self) -> RTC_CALI_CYCLING_DATA_VLD_R {
        RTC_CALI_CYCLING_DATA_VLD_R::new((self.bits & 1) != 0)
    }
    ///Bits 7:31 - When one-shot or periodic frequency calculation is done, read this value to calculate RTC slow clock's frequency.
    #[inline(always)]
...
///`read()` method returns [`rtccalicfg1::R`](R) reader structure
