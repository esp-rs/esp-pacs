...
impl R {
    ///Bits 0:2 - Set this bit to control validation of HUK generate mode. Odd of 1 is invalid, even of 1 is valid.
    #[inline(always)]
    pub fn km_huk_gen_state_high(&self) -> KM_HUK_GEN_STATE_HIGH_R {
        KM_HUK_GEN_STATE_HIGH_R::new((self.bits & 7) as u8)
    }
    ///Bits 3:4 - Set bits to control key manager random number switch cycle. 0: control by register. 1: 8 km clk cycles. 2: 16 km cycles. 3: 32 km cycles.
    #[inline(always)]
    pub fn km_rnd_switch_cycle(&self) -> KM_RND_SWITCH_CYCLE_R {
        KM_RND_SWITCH_CYCLE_R::new(((self.bits >> 3) & 3) as u8)
    }
    ///Bits 5:8 - Set each bit to control whether corresponding key can only be deployed once. 1 is true, 0 is false. Bit0: ecdsa. Bit1: xts. Bit2: hmac. Bit3: ds.
    #[inline(always)]
    pub fn km_deploy_only_once(&self) -> KM_DEPLOY_ONLY_ONCE_R {
        KM_DEPLOY_ONLY_ONCE_R::new(((self.bits >> 5) & 0x0f) as u8)
    }
    ///Bits 9:12 - Set each bit to control whether corresponding key must come from key manager.. 1 is true, 0 is false. Bit0: ecdsa. Bit1: xts. Bit2: hmac. Bit3: ds.
    #[inline(always)]
    pub fn force_use_key_manager_key(&self) -> FORCE_USE_KEY_MANAGER_KEY_R {
        FORCE_USE_KEY_MANAGER_KEY_R::new(((self.bits >> 9) & 0x0f) as u8)
    }
    ///Bit 13 - Set this bit to disable software written init key, and force use efuse_init_key.
    #[inline(always)]
    pub fn force_disable_sw_init_key(&self) -> FORCE_DISABLE_SW_INIT_KEY_R {
        FORCE_DISABLE_SW_INIT_KEY_R::new(((self.bits >> 13) & 1) != 0)
    }
    ///Bit 14 - Set this bit to configure flash encryption use xts-128 key, else use xts-256 key.
    #[inline(always)]
