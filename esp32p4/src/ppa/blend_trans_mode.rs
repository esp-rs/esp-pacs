///Register `BLEND_TRANS_MODE` reader
pub type R = crate::R<BLEND_TRANS_MODE_SPEC>;
///Register `BLEND_TRANS_MODE` writer
pub type W = crate::W<BLEND_TRANS_MODE_SPEC>;
///Field `BLEND_EN` reader - Set this bit to enable alpha blending.
pub type BLEND_EN_R = crate::BitReader;
///Field `BLEND_EN` writer - Set this bit to enable alpha blending.
pub type BLEND_EN_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `BLEND_BYPASS` reader - Set this bit to bypass blender. Then background date would be output.
pub type BLEND_BYPASS_R = crate::BitReader;
///Field `BLEND_BYPASS` writer - Set this bit to bypass blender. Then background date would be output.
pub type BLEND_BYPASS_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `BLEND_FIX_PIXEL_FILL_EN` reader - This bit is used to enable fix pixel filling. When this mode is enable only Tx channel is work and the output pixel is configured by PPA_OUT_FIX_PIXEL.
pub type BLEND_FIX_PIXEL_FILL_EN_R = crate::BitReader;
///Field `BLEND_FIX_PIXEL_FILL_EN` writer - This bit is used to enable fix pixel filling. When this mode is enable only Tx channel is work and the output pixel is configured by PPA_OUT_FIX_PIXEL.
pub type BLEND_FIX_PIXEL_FILL_EN_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `UPDATE` writer - Set this bit to update the transfer mode. Only the bit is set the transfer mode is valid.
pub type UPDATE_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `BLEND_RST` reader - write 1 then write 0 to reset blending engine.
pub type BLEND_RST_R = crate::BitReader;
///Field `BLEND_RST` writer - write 1 then write 0 to reset blending engine.
pub type BLEND_RST_W<'a, REG> = crate::BitWriter<'a, REG>;
impl R {
    ///Bit 0 - Set this bit to enable alpha blending.
    #[inline(always)]
    pub fn blend_en(&self) -> BLEND_EN_R {
        BLEND_EN_R::new((self.bits & 1) != 0)
    }
    ///Bit 1 - Set this bit to bypass blender. Then background date would be output.
    #[inline(always)]
    ...
    ///Bit 4 - write 1 then write 0 to reset blending engine.
    #[inline(always)]
...
///`read()` method returns [`blend_trans_mode::R`](R) reader structure
impl crate::Readable for BLEND_TRANS_MODE_SPEC {}
///`write(|w| ..)` method takes [`blend_trans_mode::W`](W) writer structure
