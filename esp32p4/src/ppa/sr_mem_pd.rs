///Register `SR_MEM_PD` reader
pub type R = crate::R<SR_MEM_PD_SPEC>;
///Register `SR_MEM_PD` writer
pub type W = crate::W<SR_MEM_PD_SPEC>;
///Field `SR_MEM_CLK_ENA` reader - Set this bit to force clock enable of scaling and rotating engine's data memory.
pub type SR_MEM_CLK_ENA_R = crate::BitReader;
///Field `SR_MEM_CLK_ENA` writer - Set this bit to force clock enable of scaling and rotating engine's data memory.
pub type SR_MEM_CLK_ENA_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `SR_MEM_FORCE_PD` reader - Set this bit to force power down scaling and rotating engine's data memory.
pub type SR_MEM_FORCE_PD_R = crate::BitReader;
///Field `SR_MEM_FORCE_PD` writer - Set this bit to force power down scaling and rotating engine's data memory.
pub type SR_MEM_FORCE_PD_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `SR_MEM_FORCE_PU` reader - Set this bit to force power up scaling and rotating engine's data memory.
pub type SR_MEM_FORCE_PU_R = crate::BitReader;
///Field `SR_MEM_FORCE_PU` writer - Set this bit to force power up scaling and rotating engine's data memory.
pub type SR_MEM_FORCE_PU_W<'a, REG> = crate::BitWriter<'a, REG>;
impl R {
    ///Bit 0 - Set this bit to force clock enable of scaling and rotating engine's data memory.
    #[inline(always)]
    pub fn sr_mem_clk_ena(&self) -> SR_MEM_CLK_ENA_R {
        SR_MEM_CLK_ENA_R::new((self.bits & 1) != 0)
    }
    ///Bit 1 - Set this bit to force power down scaling and rotating engine's data memory.
    #[inline(always)]
    pub fn sr_mem_force_pd(&self) -> SR_MEM_FORCE_PD_R {
        SR_MEM_FORCE_PD_R::new(((self.bits >> 1) & 1) != 0)
    }
    ///Bit 2 - Set this bit to force power up scaling and rotating engine's data memory.
    #[inline(always)]
...
///`read()` method returns [`sr_mem_pd::R`](R) reader structure
impl crate::Readable for SR_MEM_PD_SPEC {}
///`write(|w| ..)` method takes [`sr_mem_pd::W`](W) writer structure
