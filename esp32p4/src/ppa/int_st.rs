...
///Field `SR_EOF` reader - The raw interrupt status bit for the PPA_SR_EOF_INT interrupt.
pub type SR_EOF_R = crate::BitReader;
///Field `BLEND_EOF` reader - The raw interrupt status bit for the PPA_BLEND_EOF_INT interrupt.
pub type BLEND_EOF_R = crate::BitReader;
///Field `SR_PARAM_CFG_ERR` reader - The raw interrupt status bit for the PPA_SR_RX_YSCAL_ERR_INT interrupt.
pub type SR_PARAM_CFG_ERR_R = crate::BitReader;
impl R {
    ///Bit 0 - The raw interrupt status bit for the PPA_SR_EOF_INT interrupt.
    #[inline(always)]
    pub fn sr_eof(&self) -> SR_EOF_R {
        SR_EOF_R::new((self.bits & 1) != 0)
    }
    ///Bit 1 - The raw interrupt status bit for the PPA_BLEND_EOF_INT interrupt.
    #[inline(always)]
    pub fn blend_eof(&self) -> BLEND_EOF_R {
        BLEND_EOF_R::new(((self.bits >> 1) & 1) != 0)
    }
    ///Bit 2 - The raw interrupt status bit for the PPA_SR_RX_YSCAL_ERR_INT interrupt.
    #[inline(always)]
