///Register `SR_FIX_ALPHA` reader
pub type R = crate::R<SR_FIX_ALPHA_SPEC>;
///Register `SR_FIX_ALPHA` writer
pub type W = crate::W<SR_FIX_ALPHA_SPEC>;
///Field `SR_RX_FIX_ALPHA` reader - The value would replace the alpha value in received pixel for Scaling and Rotating engine when PPA_SR_RX_ALPHA_CONF_EN is enabled.
pub type SR_RX_FIX_ALPHA_R = crate::FieldReader;
///Field `SR_RX_FIX_ALPHA` writer - The value would replace the alpha value in received pixel for Scaling and Rotating engine when PPA_SR_RX_ALPHA_CONF_EN is enabled.
pub type SR_RX_FIX_ALPHA_W<'a, REG> = crate::FieldWriter<'a, REG, 8>;
///Field `SR_RX_ALPHA_MOD` reader - Alpha mode. 0/3: not replace alpha. 1: replace alpha with PPA_SR_FIX_ALPHA. 2: Original alpha multiply with PPA_SR_FIX_ALPHA/256.
pub type SR_RX_ALPHA_MOD_R = crate::FieldReader;
///Field `SR_RX_ALPHA_MOD` writer - Alpha mode. 0/3: not replace alpha. 1: replace alpha with PPA_SR_FIX_ALPHA. 2: Original alpha multiply with PPA_SR_FIX_ALPHA/256.
pub type SR_RX_ALPHA_MOD_W<'a, REG> = crate::FieldWriter<'a, REG, 2>;
///Field `SR_RX_ALPHA_INV` reader - Set this bit to invert the original alpha value. When RX color mode is RGB565/RGB88. The original alpha value is 255.
pub type SR_RX_ALPHA_INV_R = crate::BitReader;
///Field `SR_RX_ALPHA_INV` writer - Set this bit to invert the original alpha value. When RX color mode is RGB565/RGB88. The original alpha value is 255.
pub type SR_RX_ALPHA_INV_W<'a, REG> = crate::BitWriter<'a, REG>;
impl R {
    ///Bits 0:7 - The value would replace the alpha value in received pixel for Scaling and Rotating engine when PPA_SR_RX_ALPHA_CONF_EN is enabled.
    #[inline(always)]
    pub fn sr_rx_fix_alpha(&self) -> SR_RX_FIX_ALPHA_R {
        SR_RX_FIX_ALPHA_R::new((self.bits & 0xff) as u8)
    }
    ///Bits 8:9 - Alpha mode. 0/3: not replace alpha. 1: replace alpha with PPA_SR_FIX_ALPHA. 2: Original alpha multiply with PPA_SR_FIX_ALPHA/256.
    #[inline(always)]
    pub fn sr_rx_alpha_mod(&self) -> SR_RX_ALPHA_MOD_R {
        SR_RX_ALPHA_MOD_R::new(((self.bits >> 8) & 3) as u8)
    }
    ///Bit 10 - Set this bit to invert the original alpha value. When RX color mode is RGB565/RGB88. The original alpha value is 255.
    #[inline(always)]
...
///`read()` method returns [`sr_fix_alpha::R`](R) reader structure
impl crate::Readable for SR_FIX_ALPHA_SPEC {}
///`write(|w| ..)` method takes [`sr_fix_alpha::W`](W) writer structure
