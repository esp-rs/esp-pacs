///Register `RX_MEM_CONF` reader
pub type R = crate::R<RX_MEM_CONF_SPEC>;
///Register `RX_MEM_CONF` writer
pub type W = crate::W<RX_MEM_CONF_SPEC>;
///Field `RX_MEM_FIFO_CNT` reader - The number of data in the rx mem
pub type RX_MEM_FIFO_CNT_R = crate::FieldReader<u16>;
///Field `RX_MEM_THRESHOLD` reader - I2S rx mem will trigger an interrupt when the data in the mem is over(not including equal) reg_rx_mem_threshold
pub type RX_MEM_THRESHOLD_R = crate::FieldReader;
///Field `RX_MEM_THRESHOLD` writer - I2S rx mem will trigger an interrupt when the data in the mem is over(not including equal) reg_rx_mem_threshold
pub type RX_MEM_THRESHOLD_W<'a, REG> = crate::FieldWriter<'a, REG, 8>;
impl R {
    ///Bits 0:8 - The number of data in the rx mem
    #[inline(always)]
    pub fn rx_mem_fifo_cnt(&self) -> RX_MEM_FIFO_CNT_R {
        RX_MEM_FIFO_CNT_R::new((self.bits & 0x01ff) as u16)
    }
    ///Bits 9:16 - I2S rx mem will trigger an interrupt when the data in the mem is over(not including equal) reg_rx_mem_threshold
    #[inline(always)]
...
///`read()` method returns [`rx_mem_conf::R`](R) reader structure
impl crate::Readable for RX_MEM_CONF_SPEC {}
///`write(|w| ..)` method takes [`rx_mem_conf::W`](W) writer structure
