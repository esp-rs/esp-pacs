...
///Field `SPI_SMEM_TIMING_CLK_ENA` writer - For sram, the bit is used to enable timing adjust clock for all reading operations.
...
///Field `SPI_SMEM_TIMING_CALI` writer - For sram, the bit is used to enable timing auto-calibration for all reading operations.
...
///Field `SPI_SMEM_EXTRA_DUMMY_CYCLELEN` writer - For sram, add extra dummy spi clock cycle length for spi clock calibration.
...
///Field `SPI_SMEM_DLL_TIMING_CALI` writer - Set this bit to enable DLL for timing calibration in DDR mode when accessed to EXT_RAM.
...
///`write(|w| ..)` method takes [`spi_smem_timing_cali::W`](W) writer structure
