...
///Field `SPI_FMEM_VAR_DUMMY` writer - Set the bit to enable variable dummy cycle in spi DDR mode.
...
///Field `SPI_FMEM_DDR_RDAT_SWP` writer - Set the bit to reorder rx data of the word in spi DDR mode.
...
///Field `SPI_FMEM_DDR_WDAT_SWP` writer - Set the bit to reorder tx data of the word in spi DDR mode.
...
///Field `SPI_FMEM_DDR_CMD_DIS` writer - the bit is used to disable dual edge in command phase when DDR mode.
...
///Field `SPI_FMEM_TX_DDR_MSK_EN` writer - Set this bit to mask the first or the last byte in SPI0 ECC DDR write mode, when accesses to flash.
...
///Field `SPI_FMEM_RX_DDR_MSK_EN` writer - Set this bit to mask the first or the last byte in SPI0 ECC DDR read mode, when accesses to flash.
...
///Field `SPI_FMEM_USR_DDR_DQS_THD` writer - The delay number of data strobe which from memory based on SPI clock.
...
///Field `SPI_FMEM_DDR_DQS_LOOP` writer - 1: Do not need the input of SPI_DQS signal, SPI0 starts to receive data when spi0_slv_st is in SPI_MEM_DIN state. It is used when there is no SPI_DQS signal or SPI_DQS signal is not stable. 0: SPI0 starts to store data at the positive and negative edge of SPI_DQS.
...
///Field `SPI_FMEM_HYPERBUS_DUMMY_2X` writer - Set this bit to enable the vary dummy function in SPI HyperBus mode, when SPI0 accesses flash or SPI1 accesses flash or sram.
...
impl R {
    ...
    ///Bit 12 - Set this bit to mask the first or the last byte in SPI0 ECC DDR write mode, when accesses to flash.
    #[inline(always)]
    pub fn spi_fmem_tx_ddr_msk_en(&self) -> SPI_FMEM_TX_DDR_MSK_EN_R {
        SPI_FMEM_TX_DDR_MSK_EN_R::new(((self.bits >> 12) & 1) != 0)
    }
    ///Bit 13 - Set this bit to mask the first or the last byte in SPI0 ECC DDR read mode, when accesses to flash.
    #[inline(always)]
...
impl W {
    ...
    #[inline(always)]
    #[must_use]
    pub fn spi_fmem_tx_ddr_msk_en(&mut self) -> SPI_FMEM_TX_DDR_MSK_EN_W<DDR_SPEC> {
        SPI_FMEM_TX_DDR_MSK_EN_W::new(self, 12)
    }
    ///Bit 13 - Set this bit to mask the first or the last byte in SPI0 ECC DDR read mode, when accesses to flash.
    #[inline(always)]
    #[must_use]
    pub fn spi_fmem_rx_ddr_msk_en(&mut self) -> SPI_FMEM_RX_DDR_MSK_EN_W<DDR_SPEC> {
...
///`read()` method returns [`ddr::R`](R) reader structure
impl crate::Readable for DDR_SPEC {}
///`write(|w| ..)` method takes [`ddr::W`](W) writer structure
