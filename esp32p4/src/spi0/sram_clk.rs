...
///Field `SCLKCNT_L` writer - For SPI0 external RAM interface, it must be equal to spi_mem_clkcnt_N.
...
///Field `SCLKCNT_H` writer - For SPI0 external RAM interface, it must be floor((spi_mem_clkcnt_N+1)/2-1).
...
///Field `SCLKCNT_N` writer - For SPI0 external RAM interface, it is the divider of spi_mem_clk. So spi_mem_clk frequency is system/(spi_mem_clkcnt_N+1)
...
///Field `SCLK_EQU_SYSCLK` writer - For SPI0 external RAM interface, 1: spi_mem_clk is eqaul to system 0: spi_mem_clk is divided from system clock.
