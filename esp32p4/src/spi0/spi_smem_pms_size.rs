///Register `SPI_SMEM_PMS%s_SIZE` reader
pub type R = crate::R<SPI_SMEM_PMS_SIZE_SPEC>;
///Register `SPI_SMEM_PMS%s_SIZE` writer
pub type W = crate::W<SPI_SMEM_PMS_SIZE_SPEC>;
///Field `SPI_SMEM_PMS_SIZE` reader - SPI1 external RAM PMS section %s address region is (SPI_SMEM_PMS%s_ADDR_S, SPI_SMEM_PMS%s_ADDR_S + SPI_SMEM_PMS%s_SIZE)
pub type SPI_SMEM_PMS_SIZE_R = crate::FieldReader<u16>;
///Field `SPI_SMEM_PMS_SIZE` writer - SPI1 external RAM PMS section %s address region is (SPI_SMEM_PMS%s_ADDR_S, SPI_SMEM_PMS%s_ADDR_S + SPI_SMEM_PMS%s_SIZE)
pub type SPI_SMEM_PMS_SIZE_W<'a, REG> = crate::FieldWriter<'a, REG, 15, u16>;
impl R {
    ///Bits 0:14 - SPI1 external RAM PMS section %s address region is (SPI_SMEM_PMS%s_ADDR_S, SPI_SMEM_PMS%s_ADDR_S + SPI_SMEM_PMS%s_SIZE)
    #[inline(always)]
    pub fn spi_smem_pms_size(&self) -> SPI_SMEM_PMS_SIZE_R {
        SPI_SMEM_PMS_SIZE_R::new((self.bits & 0x7fff) as u16)
    }
}
...
///`read()` method returns [`spi_smem_pms_size::R`](R) reader structure
impl crate::Readable for SPI_SMEM_PMS_SIZE_SPEC {}
///`write(|w| ..)` method takes [`spi_smem_pms_size::W`](W) writer structure
