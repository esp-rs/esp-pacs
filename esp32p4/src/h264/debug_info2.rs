...
impl core::fmt::Debug for R {
    fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
        f.debug_struct("DEBUG_INFO2")
            .field("p_rc_done_debug_flag", &self.p_rc_done_debug_flag().bit())
            .field(
                "p_p_i_cmp_done_debug_flag",
                &self.p_p_i_cmp_done_debug_flag().bit(),
            )
            .field(
                "p_mv_merge_done_debug_flag",
                &self.p_mv_merge_done_debug_flag().bit(),
            )
            .field(
                "p_move_ori_done_debug_flag",
                &self.p_move_ori_done_debug_flag().bit(),
            )
            .field("p_mc_done_debug_flag", &self.p_mc_done_debug_flag().bit())
            .field("p_ime_done_debug_flag", &self.p_ime_done_debug_flag().bit())
            .field(
                "p_get_ori_done_debug_flag",
                &self.p_get_ori_done_debug_flag().bit(),
            )
            .field("p_fme_done_debug_flag", &self.p_fme_done_debug_flag().bit())
            .field(
                "p_fetch_done_debug_flag",
                &self.p_fetch_done_debug_flag().bit(),
            )
            .field("p_db_done_debug_flag", &self.p_db_done_debug_flag().bit())
            .field(
                "p_bs_buf_done_debug_flag",
                &self.p_bs_buf_done_debug_flag().bit(),
            )
            .field(
                "ref_move_2mb_line_done_debug_flag",
                &self.ref_move_2mb_line_done_debug_flag().bit(),
            )
            .field(
                "i_p_i_cmp_done_debug_flag",
                &self.i_p_i_cmp_done_debug_flag().bit(),
            )
            .field(
                "i_move_ori_done_debug_flag",
                &self.i_move_ori_done_debug_flag().bit(),
            )
            .field(
                "i_get_ori_done_debug_flag",
                &self.i_get_ori_done_debug_flag().bit(),
            )
            .field("i_ec_done_debug_flag", &self.i_ec_done_debug_flag().bit())
            .field("i_db_done_debug_flag", &self.i_db_done_debug_flag().bit())
            .field(
                "i_bs_buf_done_debug_flag",
                &self.i_bs_buf_done_debug_flag().bit(),
            )
            .finish()
    }
}
