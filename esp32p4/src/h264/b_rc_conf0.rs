///Register `B_RC_CONF0` reader
pub type R = crate::R<B_RC_CONF0_SPEC>;
///Register `B_RC_CONF0` writer
pub type W = crate::W<B_RC_CONF0_SPEC>;
///Field `B_QP` reader - Configures video B frame level initial luma QP value.
pub type B_QP_R = crate::FieldReader;
///Field `B_QP` writer - Configures video B frame level initial luma QP value.
pub type B_QP_W<'a, REG> = crate::FieldWriter<'a, REG, 6>;
///Field `B_RATE_CTRL_U` reader - Configures video B parameter U value. U = int((float) u &lt;&lt; 8).
pub type B_RATE_CTRL_U_R = crate::FieldReader<u16>;
///Field `B_RATE_CTRL_U` writer - Configures video B parameter U value. U = int((float) u &lt;&lt; 8).
pub type B_RATE_CTRL_U_W<'a, REG> = crate::FieldWriter<'a, REG, 16, u16>;
///Field `B_MB_RATE_CTRL_EN` reader - Configures video A whether or not to open macro block rate ctrl.\\1:Open the macro block rate ctrl\\1:Close the macro block rate ctrl.
pub type B_MB_RATE_CTRL_EN_R = crate::BitReader;
///Field `B_MB_RATE_CTRL_EN` writer - Configures video A whether or not to open macro block rate ctrl.\\1:Open the macro block rate ctrl\\1:Close the macro block rate ctrl.
pub type B_MB_RATE_CTRL_EN_W<'a, REG> = crate::BitWriter<'a, REG>;
impl R {
    ///Bits 0:5 - Configures video B frame level initial luma QP value.
    #[inline(always)]
    pub fn b_qp(&self) -> B_QP_R {
        B_QP_R::new((self.bits & 0x3f) as u8)
    }
    ///Bits 6:21 - Configures video B parameter U value. U = int((float) u &lt;&lt; 8).
    #[inline(always)]
...
///`read()` method returns [`b_rc_conf0::R`](R) reader structure
impl crate::Readable for B_RC_CONF0_SPEC {}
///`write(|w| ..)` method takes [`b_rc_conf0::W`](W) writer structure
