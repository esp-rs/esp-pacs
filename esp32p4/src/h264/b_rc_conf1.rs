///Register `B_RC_CONF1` reader
pub type R = crate::R<B_RC_CONF1_SPEC>;
///Register `B_RC_CONF1` writer
pub type W = crate::W<B_RC_CONF1_SPEC>;
///Field `B_CHROMA_DC_QP_DELTA` reader - Configures video B chroma DC QP offset based on Chroma QP. Chroma DC QP = Chroma QP(after map) + reg_chroma_dc_qp_delta.
pub type B_CHROMA_DC_QP_DELTA_R = crate::FieldReader;
///Field `B_CHROMA_DC_QP_DELTA` writer - Configures video B chroma DC QP offset based on Chroma QP. Chroma DC QP = Chroma QP(after map) + reg_chroma_dc_qp_delta.
pub type B_CHROMA_DC_QP_DELTA_W<'a, REG> = crate::FieldWriter<'a, REG, 3>;
///Field `B_CHROMA_QP_DELTA` reader - Configures video B chroma QP offset based on luma QP. Chroma QP(before map) = Luma QP + reg_chroma_qp_delta.
pub type B_CHROMA_QP_DELTA_R = crate::FieldReader;
///Field `B_CHROMA_QP_DELTA` writer - Configures video B chroma QP offset based on luma QP. Chroma QP(before map) = Luma QP + reg_chroma_qp_delta.
pub type B_CHROMA_QP_DELTA_W<'a, REG> = crate::FieldWriter<'a, REG, 4>;
///Field `B_QP_MIN` reader - Configures video B allowed luma QP min value.
pub type B_QP_MIN_R = crate::FieldReader;
///Field `B_QP_MIN` writer - Configures video B allowed luma QP min value.
pub type B_QP_MIN_W<'a, REG> = crate::FieldWriter<'a, REG, 6>;
///Field `B_QP_MAX` reader - Configures video B allowed luma QP max value.
pub type B_QP_MAX_R = crate::FieldReader;
///Field `B_QP_MAX` writer - Configures video B allowed luma QP max value.
pub type B_QP_MAX_W<'a, REG> = crate::FieldWriter<'a, REG, 6>;
///Field `B_MAD_FRAME_PRED` reader - Configures vdieo B frame level predicted MB MAD value.
pub type B_MAD_FRAME_PRED_R = crate::FieldReader<u16>;
///Field `B_MAD_FRAME_PRED` writer - Configures vdieo B frame level predicted MB MAD value.
pub type B_MAD_FRAME_PRED_W<'a, REG> = crate::FieldWriter<'a, REG, 12, u16>;
impl R {
    ///Bits 0:2 - Configures video B chroma DC QP offset based on Chroma QP. Chroma DC QP = Chroma QP(after map) + reg_chroma_dc_qp_delta.
    #[inline(always)]
    pub fn b_chroma_dc_qp_delta(&self) -> B_CHROMA_DC_QP_DELTA_R {
        B_CHROMA_DC_QP_DELTA_R::new((self.bits & 7) as u8)
    }
    ///Bits 3:6 - Configures video B chroma QP offset based on luma QP. Chroma QP(before map) = Luma QP + reg_chroma_qp_delta.
    #[inline(always)]
    pub fn b_chroma_qp_delta(&self) -> B_CHROMA_QP_DELTA_R {
        B_CHROMA_QP_DELTA_R::new(((self.bits >> 3) & 0x0f) as u8)
    }
    ///Bits 7:12 - Configures video B allowed luma QP min value.
    #[inline(always)]
    pub fn b_qp_min(&self) -> B_QP_MIN_R {
        B_QP_MIN_R::new(((self.bits >> 7) & 0x3f) as u8)
    }
    ///Bits 13:18 - Configures video B allowed luma QP max value.
    #[inline(always)]
    pub fn b_qp_max(&self) -> B_QP_MAX_R {
        B_QP_MAX_R::new(((self.bits >> 13) & 0x3f) as u8)
    }
    ///Bits 19:30 - Configures vdieo B frame level predicted MB MAD value.
    #[inline(always)]
...
///`read()` method returns [`b_rc_conf1::R`](R) reader structure
impl crate::Readable for B_RC_CONF1_SPEC {}
///`write(|w| ..)` method takes [`b_rc_conf1::W`](W) writer structure
