...
///Field `CBC_ON` reader - Represents whether or not an cycle-by-cycle mode action is on going.\\0:No action\\1: On going
pub type CBC_ON_R = crate::BitReader;
///Field `OST_ON` reader - Represents whether or not an one-shot mode action is on going.\\0:No action\\1: On going
pub type OST_ON_R = crate::BitReader;
impl R {
    ///Bit 0 - Represents whether or not an cycle-by-cycle mode action is on going.\\0:No action\\1: On going
    #[inline(always)]
    pub fn cbc_on(&self) -> CBC_ON_R {
        CBC_ON_R::new((self.bits & 1) != 0)
    }
    ///Bit 1 - Represents whether or not an one-shot mode action is on going.\\0:No action\\1: On going
    #[inline(always)]
