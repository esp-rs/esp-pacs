///Register `BUFFER_STATUS_LEVEL` reader
pub type R = crate::R<BUFFER_STATUS_LEVEL_SPEC>;
///Field `CMD_BUF_EMPTY_CNT` reader - Command Buffer Empty Locations contains the number of empty locations in the command buffer.
pub type CMD_BUF_EMPTY_CNT_R = crate::FieldReader;
///Field `RESP_BUF_CNT` reader - Response Buffer Level Value contains the number of valid data entries in the response buffer.
pub type RESP_BUF_CNT_R = crate::FieldReader;
///Field `IBI_DATA_BUF_CNT` reader - IBI Buffer Level Value contains the number of valid entries in the IBI Buffer. This is field is used in master mode.
pub type IBI_DATA_BUF_CNT_R = crate::FieldReader;
///Field `IBI_STATUS_BUF_CNT` reader - IBI Buffer Status Count contains the number of IBI status entries in the IBI Buffer. This field is used in master mode.
pub type IBI_STATUS_BUF_CNT_R = crate::FieldReader;
impl R {
    ///Bits 0:4 - Command Buffer Empty Locations contains the number of empty locations in the command buffer.
    #[inline(always)]
    pub fn cmd_buf_empty_cnt(&self) -> CMD_BUF_EMPTY_CNT_R {
        CMD_BUF_EMPTY_CNT_R::new((self.bits & 0x1f) as u8)
    }
    ///Bits 8:11 - Response Buffer Level Value contains the number of valid data entries in the response buffer.
    #[inline(always)]
    pub fn resp_buf_cnt(&self) -> RESP_BUF_CNT_R {
        RESP_BUF_CNT_R::new(((self.bits >> 8) & 0x0f) as u8)
    }
    ///Bits 16:19 - IBI Buffer Level Value contains the number of valid entries in the IBI Buffer. This is field is used in master mode.
    #[inline(always)]
    pub fn ibi_data_buf_cnt(&self) -> IBI_DATA_BUF_CNT_R {
        IBI_DATA_BUF_CNT_R::new(((self.bits >> 16) & 0x0f) as u8)
    }
    ///Bits 24:27 - IBI Buffer Status Count contains the number of IBI status entries in the IBI Buffer. This field is used in master mode.
    #[inline(always)]
...
///`read()` method returns [`buffer_status_level::R`](R) reader structure
