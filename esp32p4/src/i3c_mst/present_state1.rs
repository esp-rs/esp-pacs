///Register `PRESENT_STATE1` reader
pub type R = crate::R<PRESENT_STATE1_SPEC>;
///Field `DATA_BYTE_CNT` reader - Present transfer data byte cnt: tx data byte cnt if write rx data byte cnt if read ibi data byte cnt if IBI handle.
pub type DATA_BYTE_CNT_R = crate::FieldReader<u16>;
impl R {
    ///Bits 0:15 - Present transfer data byte cnt: tx data byte cnt if write rx data byte cnt if read ibi data byte cnt if IBI handle.
    #[inline(always)]
...
///`read()` method returns [`present_state1::R`](R) reader structure
