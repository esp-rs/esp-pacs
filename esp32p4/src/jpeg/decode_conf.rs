///Register `DECODE_CONF` reader
pub type R = crate::R<DECODE_CONF_SPEC>;
///Register `DECODE_CONF` writer
pub type W = crate::W<DECODE_CONF_SPEC>;
///Field `RESTART_INTERVAL` reader - configure restart interval in DRI marker when decode
pub type RESTART_INTERVAL_R = crate::FieldReader<u16>;
///Field `RESTART_INTERVAL` writer - configure restart interval in DRI marker when decode
pub type RESTART_INTERVAL_W<'a, REG> = crate::FieldWriter<'a, REG, 16, u16>;
///Field `COMPONENT_NUM` reader - configure number of components in frame when decode
pub type COMPONENT_NUM_R = crate::FieldReader;
///Field `COMPONENT_NUM` writer - configure number of components in frame when decode
pub type COMPONENT_NUM_W<'a, REG> = crate::FieldWriter<'a, REG, 8>;
///Field `SW_DHT_EN` reader - software decode dht table enable
pub type SW_DHT_EN_R = crate::BitReader;
///Field `SOS_CHECK_BYTE_NUM` reader - Configure the byte number to check next sos marker in the multi-scan picture after one scan is decoded down. The real check number is reg_sos_check_byte_num+1
pub type SOS_CHECK_BYTE_NUM_R = crate::FieldReader;
///Field `SOS_CHECK_BYTE_NUM` writer - Configure the byte number to check next sos marker in the multi-scan picture after one scan is decoded down. The real check number is reg_sos_check_byte_num+1
pub type SOS_CHECK_BYTE_NUM_W<'a, REG> = crate::FieldWriter<'a, REG, 2>;
///Field `RST_CHECK_BYTE_NUM` reader - Configure the byte number to check next rst marker after one rst interval is decoded down. The real check number is reg_rst_check_byte_num+1
pub type RST_CHECK_BYTE_NUM_R = crate::FieldReader;
///Field `RST_CHECK_BYTE_NUM` writer - Configure the byte number to check next rst marker after one rst interval is decoded down. The real check number is reg_rst_check_byte_num+1
pub type RST_CHECK_BYTE_NUM_W<'a, REG> = crate::FieldWriter<'a, REG, 2>;
///Field `MULTI_SCAN_ERR_CHECK` reader - reserved for decoder
pub type MULTI_SCAN_ERR_CHECK_R = crate::BitReader;
///Field `MULTI_SCAN_ERR_CHECK` writer - reserved for decoder
...
///Field `DEZIGZAG_READY_CTL` writer - reserved for decoder
pub type DEZIGZAG_READY_CTL_W<'a, REG> = crate::BitWriter<'a, REG>;
impl R {
    ///Bits 0:15 - configure restart interval in DRI marker when decode
    #[inline(always)]
    pub fn restart_interval(&self) -> RESTART_INTERVAL_R {
        RESTART_INTERVAL_R::new((self.bits & 0xffff) as u16)
    }
    ///Bits 16:23 - configure number of components in frame when decode
    #[inline(always)]
    pub fn component_num(&self) -> COMPONENT_NUM_R {
        COMPONENT_NUM_R::new(((self.bits >> 16) & 0xff) as u8)
    }
    ///Bit 24 - software decode dht table enable
    #[inline(always)]
    pub fn sw_dht_en(&self) -> SW_DHT_EN_R {
        SW_DHT_EN_R::new(((self.bits >> 24) & 1) != 0)
    }
    ///Bits 25:26 - Configure the byte number to check next sos marker in the multi-scan picture after one scan is decoded down. The real check number is reg_sos_check_byte_num+1
    #[inline(always)]
    pub fn sos_check_byte_num(&self) -> SOS_CHECK_BYTE_NUM_R {
        SOS_CHECK_BYTE_NUM_R::new(((self.bits >> 25) & 3) as u8)
    }
    ///Bits 27:28 - Configure the byte number to check next rst marker after one rst interval is decoded down. The real check number is reg_rst_check_byte_num+1
    #[inline(always)]
    pub fn rst_check_byte_num(&self) -> RST_CHECK_BYTE_NUM_R {
        RST_CHECK_BYTE_NUM_R::new(((self.bits >> 27) & 3) as u8)
    }
    ///Bit 29 - reserved for decoder
    #[inline(always)]
...
///`read()` method returns [`decode_conf::R`](R) reader structure
impl crate::Readable for DECODE_CONF_SPEC {}
///`write(|w| ..)` method takes [`decode_conf::W`](W) writer structure
