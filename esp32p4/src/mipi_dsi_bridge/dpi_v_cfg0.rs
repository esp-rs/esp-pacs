///Register `DPI_V_CFG0` reader
pub type R = crate::R<DPI_V_CFG0_SPEC>;
///Register `DPI_V_CFG0` writer
pub type W = crate::W<DPI_V_CFG0_SPEC>;
///Field `VTOTAL` reader - this field configures the total length of one frame (by line) for dpi output, must meet: reg_vtotal > reg_vdisp+reg_vsync+reg_vbank
pub type VTOTAL_R = crate::FieldReader<u16>;
///Field `VTOTAL` writer - this field configures the total length of one frame (by line) for dpi output, must meet: reg_vtotal > reg_vdisp+reg_vsync+reg_vbank
pub type VTOTAL_W<'a, REG> = crate::FieldWriter<'a, REG, 12, u16>;
///Field `VDISP` reader - this field configures the length of valid line (by line) for dpi output
pub type VDISP_R = crate::FieldReader<u16>;
///Field `VDISP` writer - this field configures the length of valid line (by line) for dpi output
pub type VDISP_W<'a, REG> = crate::FieldWriter<'a, REG, 12, u16>;
impl R {
    ///Bits 0:11 - this field configures the total length of one frame (by line) for dpi output, must meet: reg_vtotal > reg_vdisp+reg_vsync+reg_vbank
    #[inline(always)]
    pub fn vtotal(&self) -> VTOTAL_R {
        VTOTAL_R::new((self.bits & 0x0fff) as u16)
    }
    ///Bits 16:27 - this field configures the length of valid line (by line) for dpi output
    #[inline(always)]
...
///`read()` method returns [`dpi_v_cfg0::R`](R) reader structure
impl crate::Readable for DPI_V_CFG0_SPEC {}
///`write(|w| ..)` method takes [`dpi_v_cfg0::W`](W) writer structure
