///Register `RAW_BUF_CREDIT_CTL` reader
pub type R = crate::R<RAW_BUF_CREDIT_CTL_SPEC>;
///Register `RAW_BUF_CREDIT_CTL` writer
pub type W = crate::W<RAW_BUF_CREDIT_CTL_SPEC>;
///Field `CREDIT_THRD` reader - this field configures the threshold whether dsi_bridge fifo can receive one more 64-bit, valid only when dsi_bridge as flow controller
pub type CREDIT_THRD_R = crate::FieldReader<u16>;
///Field `CREDIT_THRD` writer - this field configures the threshold whether dsi_bridge fifo can receive one more 64-bit, valid only when dsi_bridge as flow controller
pub type CREDIT_THRD_W<'a, REG> = crate::FieldWriter<'a, REG, 15, u16>;
///Field `CREDIT_BURST_THRD` reader - this field configures the threshold whether dsi_bridge fifo can receive one more dma burst, valid only when dsi_bridge as flow controller
pub type CREDIT_BURST_THRD_R = crate::FieldReader<u16>;
///Field `CREDIT_BURST_THRD` writer - this field configures the threshold whether dsi_bridge fifo can receive one more dma burst, valid only when dsi_bridge as flow controller
pub type CREDIT_BURST_THRD_W<'a, REG> = crate::FieldWriter<'a, REG, 15, u16>;
///Field `CREDIT_RESET` reader - this bit configures internal credit cnt clear, 0: non, 1: reset. valid only when dsi_bridge as flow controller
pub type CREDIT_RESET_R = crate::BitReader;
///Field `CREDIT_RESET` writer - this bit configures internal credit cnt clear, 0: non, 1: reset. valid only when dsi_bridge as flow controller
pub type CREDIT_RESET_W<'a, REG> = crate::BitWriter<'a, REG>;
impl R {
    ///Bits 0:14 - this field configures the threshold whether dsi_bridge fifo can receive one more 64-bit, valid only when dsi_bridge as flow controller
    #[inline(always)]
    pub fn credit_thrd(&self) -> CREDIT_THRD_R {
        CREDIT_THRD_R::new((self.bits & 0x7fff) as u16)
    }
    ///Bits 16:30 - this field configures the threshold whether dsi_bridge fifo can receive one more dma burst, valid only when dsi_bridge as flow controller
    #[inline(always)]
    pub fn credit_burst_thrd(&self) -> CREDIT_BURST_THRD_R {
        CREDIT_BURST_THRD_R::new(((self.bits >> 16) & 0x7fff) as u16)
    }
    ///Bit 31 - this bit configures internal credit cnt clear, 0: non, 1: reset. valid only when dsi_bridge as flow controller
    #[inline(always)]
...
///`read()` method returns [`raw_buf_credit_ctl::R`](R) reader structure
impl crate::Readable for RAW_BUF_CREDIT_CTL_SPEC {}
///`write(|w| ..)` method takes [`raw_buf_credit_ctl::W`](W) writer structure
