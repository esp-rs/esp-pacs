///Register `RX_DATA_CFG` reader
pub type R = crate::R<RX_DATA_CFG_SPEC>;
///Register `RX_DATA_CFG` writer
pub type W = crate::W<RX_DATA_CFG_SPEC>;
///Field `RX_BITLEN` reader - Configures expected byte number of received data.
pub type RX_BITLEN_R = crate::FieldReader<u32>;
///Field `RX_BITLEN` writer - Configures expected byte number of received data.
pub type RX_BITLEN_W<'a, REG> = crate::FieldWriter<'a, REG, 19, u32>;
///Field `RX_DATA_ORDER_INV` reader - Set this bit to invert bit order of one byte sent from RX_FIFO to DMA.
pub type RX_DATA_ORDER_INV_R = crate::BitReader;
///Field `RX_DATA_ORDER_INV` writer - Set this bit to invert bit order of one byte sent from RX_FIFO to DMA.
pub type RX_DATA_ORDER_INV_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `RX_BUS_WID_SEL` reader - Configures the rxd bus width. 3'd0: bus width is 1. 3'd1: bus width is 2. 3'd2: bus width is 4. 3'd3: bus width is 8.
pub type RX_BUS_WID_SEL_R = crate::FieldReader;
///Field `RX_BUS_WID_SEL` writer - Configures the rxd bus width. 3'd0: bus width is 1. 3'd1: bus width is 2. 3'd2: bus width is 4. 3'd3: bus width is 8.
pub type RX_BUS_WID_SEL_W<'a, REG> = crate::FieldWriter<'a, REG, 3>;
impl R {
    ///Bits 9:27 - Configures expected byte number of received data.
    #[inline(always)]
    pub fn rx_bitlen(&self) -> RX_BITLEN_R {
        RX_BITLEN_R::new((self.bits >> 9) & 0x0007_ffff)
    }
    ///Bit 28 - Set this bit to invert bit order of one byte sent from RX_FIFO to DMA.
    #[inline(always)]
    pub fn rx_data_order_inv(&self) -> RX_DATA_ORDER_INV_R {
        RX_DATA_ORDER_INV_R::new(((self.bits >> 28) & 1) != 0)
    }
    ///Bits 29:31 - Configures the rxd bus width. 3'd0: bus width is 1. 3'd1: bus width is 2. 3'd2: bus width is 4. 3'd3: bus width is 8.
    #[inline(always)]
...
///`read()` method returns [`rx_data_cfg::R`](R) reader structure
impl crate::Readable for RX_DATA_CFG_SPEC {}
///`write(|w| ..)` method takes [`rx_data_cfg::W`](W) writer structure
