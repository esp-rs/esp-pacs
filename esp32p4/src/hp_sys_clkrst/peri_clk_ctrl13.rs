...
impl core::fmt::Debug for R {
    fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
        f.debug_struct("PERI_CLK_CTRL13")
            .field("i2s0_rx_div_z", &self.i2s0_rx_div_z().bits())
            .field("i2s0_rx_div_yn1", &self.i2s0_rx_div_yn1().bit())
            .field("i2s0_tx_clk_en", &self.i2s0_tx_clk_en().bit())
            .field("i2s0_tx_clk_src_sel", &self.i2s0_tx_clk_src_sel().bits())
            .field("i2s0_tx_div_n", &self.i2s0_tx_div_n().bits())
            .field("i2s0_tx_div_x", &self.i2s0_tx_div_x().bits())
            .finish()
    }
}
#[cfg(feature = "impl-register-debug")]
