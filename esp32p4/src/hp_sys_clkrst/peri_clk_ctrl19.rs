...
impl core::fmt::Debug for R {
    fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
        f.debug_struct("PERI_CLK_CTRL19")
            .field("i2s2_tx_div_x", &self.i2s2_tx_div_x().bits())
            .field("i2s2_tx_div_y", &self.i2s2_tx_div_y().bits())
            .field("i2s2_tx_div_z", &self.i2s2_tx_div_z().bits())
            .field("i2s2_tx_div_yn1", &self.i2s2_tx_div_yn1().bit())
            .field("i2s2_mst_clk_sel", &self.i2s2_mst_clk_sel().bit())
            .field("lcd_clk_src_sel", &self.lcd_clk_src_sel().bits())
            .field("lcd_clk_en", &self.lcd_clk_en().bit())
            .finish()
    }
}
#[cfg(feature = "impl-register-debug")]
