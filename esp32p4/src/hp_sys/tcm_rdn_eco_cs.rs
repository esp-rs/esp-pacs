...
impl core::fmt::Debug for R {
    fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
        f.debug_struct("TCM_RDN_ECO_CS")
            .field("reg_hp_tcm_rdn_eco_en", &self.reg_hp_tcm_rdn_eco_en())
            .field(
                "reg_hp_tcm_rdn_eco_result",
                &self.reg_hp_tcm_rdn_eco_result(),
            )
            .finish()
    }
}
...
///`read()` method returns [`tcm_rdn_eco_cs::R`](R) reader structure
impl crate::Readable for TCM_RDN_ECO_CS_SPEC {}
///`write(|w| ..)` method takes [`tcm_rdn_eco_cs::W`](W) writer structure
