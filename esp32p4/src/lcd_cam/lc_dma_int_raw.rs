...
///Field `LCD_VSYNC_INT_RAW` reader - The raw bit for LCD frame end interrupt.
pub type LCD_VSYNC_INT_RAW_R = crate::BitReader;
///Field `LCD_TRANS_DONE_INT_RAW` reader - The raw bit for lcd transfer end interrupt.
pub type LCD_TRANS_DONE_INT_RAW_R = crate::BitReader;
///Field `CAM_VSYNC_INT_RAW` reader - The raw bit for Camera frame end interrupt.
pub type CAM_VSYNC_INT_RAW_R = crate::BitReader;
///Field `CAM_HS_INT_RAW` reader - The raw bit for Camera line interrupt.
pub type CAM_HS_INT_RAW_R = crate::BitReader;
impl R {
    ///Bit 0 - The raw bit for LCD frame end interrupt.
    #[inline(always)]
    pub fn lcd_vsync_int_raw(&self) -> LCD_VSYNC_INT_RAW_R {
        LCD_VSYNC_INT_RAW_R::new((self.bits & 1) != 0)
    }
    ///Bit 1 - The raw bit for lcd transfer end interrupt.
    #[inline(always)]
    pub fn lcd_trans_done_int_raw(&self) -> LCD_TRANS_DONE_INT_RAW_R {
        LCD_TRANS_DONE_INT_RAW_R::new(((self.bits >> 1) & 1) != 0)
    }
    ///Bit 2 - The raw bit for Camera frame end interrupt.
    #[inline(always)]
    pub fn cam_vsync_int_raw(&self) -> CAM_VSYNC_INT_RAW_R {
        CAM_VSYNC_INT_RAW_R::new(((self.bits >> 2) & 1) != 0)
    }
    ///Bit 3 - The raw bit for Camera line interrupt.
    #[inline(always)]
