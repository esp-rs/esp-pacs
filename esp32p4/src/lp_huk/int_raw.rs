...
///Field `PREP_DONE` reader - The raw interrupt status bit for the huk_prep_done_int interrupt
pub type PREP_DONE_R = crate::BitReader;
///Field `PROC_DONE` reader - The raw interrupt status bit for the huk_proc_done_int interrupt
pub type PROC_DONE_R = crate::BitReader;
///Field `POST_DONE` reader - The raw interrupt status bit for the huk_post_done_int interrupt
pub type POST_DONE_R = crate::BitReader;
impl R {
    ///Bit 0 - The raw interrupt status bit for the huk_prep_done_int interrupt
    #[inline(always)]
    pub fn prep_done(&self) -> PREP_DONE_R {
        PREP_DONE_R::new((self.bits & 1) != 0)
    }
    ///Bit 1 - The raw interrupt status bit for the huk_proc_done_int interrupt
    #[inline(always)]
    pub fn proc_done(&self) -> PROC_DONE_R {
        PROC_DONE_R::new(((self.bits >> 1) & 1) != 0)
    }
    ///Bit 2 - The raw interrupt status bit for the huk_post_done_int interrupt
    #[inline(always)]
