...
///Field `RECEIVE_INT_ST` reader - 1: this bit is set while the receive FIFO is not empty and the RIE bit is set within the interrupt enable register. 0: reset
pub type RECEIVE_INT_ST_R = crate::BitReader;
///Field `TRANSMIT_INT_ST` reader - 1: this bit is set whenever the transmit buffer status changes from '0-to-1' (released) and the TIE bit is set within the interrupt enable register. 0: reset
pub type TRANSMIT_INT_ST_R = crate::BitReader;
///Field `ERR_WARNING_INT_ST` reader - 1: this bit is set on every change (set and clear) of either the error status or bus status bits and the EIE bit is set within the interrupt enable register. 0: reset
pub type ERR_WARNING_INT_ST_R = crate::BitReader;
///Field `DATA_OVERRUN_INT_ST` reader - 1: this bit is set on a '0-to-1' transition of the data overrun status bit and the DOIE bit is set within the interrupt enable register. 0: reset
pub type DATA_OVERRUN_INT_ST_R = crate::BitReader;
///Field `TS_COUNTER_OVFL_INT_ST` reader - 1: this bit is set then the timestamp counter reaches the maximum value and overflow.
...
///Field `ARBITRATION_LOST_INT_ST` reader - 1: this bit is set when the TWAI controller lost the arbitration and becomes a receiver and the ALIE bit is set within the interrupt enable register. 0: reset
pub type ARBITRATION_LOST_INT_ST_R = crate::BitReader;
///Field `BUS_ERR_INT_ST` reader - 1: this bit is set when the TWAI controller detects an error on the TWAI-bus and the BEIE bit is set within the interrupt enable register. 0: reset
pub type BUS_ERR_INT_ST_R = crate::BitReader;
///Field `IDLE_INT_ST` reader - 1: this bit is set when the TWAI controller detects state of TWAI become IDLE and this interrupt enable bit is set within the interrupt enable register. 0: reset
...
impl R {
    ...
    ///Bit 4 - 1: this bit is set then the timestamp counter reaches the maximum value and overflow.
    #[inline(always)]
...
impl core::fmt::Debug for R {
    fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
        f.debug_struct("INTERRUPT")
            ...
            .field("ts_counter_ovfl_int_st", &self.ts_counter_ovfl_int_st())
...
///`read()` method returns [`interrupt::R`](R) reader structure
