///Register `YUV_FORMAT` reader
pub type R = crate::R<YUV_FORMAT_SPEC>;
///Register `YUV_FORMAT` writer
pub type W = crate::W<YUV_FORMAT_SPEC>;
///Field `YUV_MODE` reader - this bit configures the yuv mode. 0: ITU-R BT.601, 1: ITU-R BT.709
pub type YUV_MODE_R = crate::BitReader;
///Field `YUV_MODE` writer - this bit configures the yuv mode. 0: ITU-R BT.601, 1: ITU-R BT.709
pub type YUV_MODE_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `YUV_RANGE` reader - this bit configures the yuv range. 0: full range, 1: limit range
pub type YUV_RANGE_R = crate::BitReader;
///Field `YUV_RANGE` writer - this bit configures the yuv range. 0: full range, 1: limit range
pub type YUV_RANGE_W<'a, REG> = crate::BitWriter<'a, REG>;
impl R {
    ///Bit 0 - this bit configures the yuv mode. 0: ITU-R BT.601, 1: ITU-R BT.709
    #[inline(always)]
    pub fn yuv_mode(&self) -> YUV_MODE_R {
        YUV_MODE_R::new((self.bits & 1) != 0)
    }
    ///Bit 1 - this bit configures the yuv range. 0: full range, 1: limit range
    #[inline(always)]
...
///`read()` method returns [`yuv_format::R`](R) reader structure
impl crate::Readable for YUV_FORMAT_SPEC {}
///`write(|w| ..)` method takes [`yuv_format::W`](W) writer structure
