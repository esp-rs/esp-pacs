///Register `IN_LINK_ADDR_CH%s` reader
pub type R = crate::R<IN_LINK_ADDR_CH_SPEC>;
///Register `IN_LINK_ADDR_CH%s` writer
pub type W = crate::W<IN_LINK_ADDR_CH_SPEC>;
///Field `INLINK_ADDR` reader - This register stores the 32 least significant bits of the first inlink descriptor's address.
pub type INLINK_ADDR_R = crate::FieldReader<u32>;
///Field `INLINK_ADDR` writer - This register stores the 32 least significant bits of the first inlink descriptor's address.
pub type INLINK_ADDR_W<'a, REG> = crate::FieldWriter<'a, REG, 32, u32>;
impl R {
    ///Bits 0:31 - This register stores the 32 least significant bits of the first inlink descriptor's address.
    #[inline(always)]
...
///`read()` method returns [`in_link_addr_ch::R`](R) reader structure
impl crate::Readable for IN_LINK_ADDR_CH_SPEC {}
///`write(|w| ..)` method takes [`in_link_addr_ch::W`](W) writer structure
