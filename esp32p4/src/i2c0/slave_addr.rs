...
///Register `SLAVE_ADDR` writer
pub type W = crate::W<SLAVE_ADDR_SPEC>;
///Field `SLAVE_ADDR` reader - Configure the slave address of I2C Slave.
pub type SLAVE_ADDR_R = crate::FieldReader<u16>;
///Field `SLAVE_ADDR` writer - Configure the slave address of I2C Slave.
pub type SLAVE_ADDR_W<'a, REG> = crate::FieldWriter<'a, REG, 15, u16>;
///Field `ADDR_10BIT_EN` reader - Configures to enable the slave 10-bit addressing mode in master mode. 0: No effect 1: Enable
pub type ADDR_10BIT_EN_R = crate::BitReader;
///Field `ADDR_10BIT_EN` writer - Configures to enable the slave 10-bit addressing mode in master mode. 0: No effect 1: Enable
pub type ADDR_10BIT_EN_W<'a, REG> = crate::BitWriter<'a, REG>;
impl R {
    ///Bits 0:14 - Configure the slave address of I2C Slave.
    #[inline(always)]
    pub fn slave_addr(&self) -> SLAVE_ADDR_R {
        SLAVE_ADDR_R::new((self.bits & 0x7fff) as u16)
    }
    ///Bit 31 - Configures to enable the slave 10-bit addressing mode in master mode. 0: No effect 1: Enable
    #[inline(always)]
...
///`read()` method returns [`slave_addr::R`](R) reader structure
impl crate::Readable for SLAVE_ADDR_SPEC {}
///`write(|w| ..)` method takes [`slave_addr::W`](W) writer structure
