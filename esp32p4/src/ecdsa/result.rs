...
///Field `OPERATION_RESULT` reader - The operation result bit of ECDSA Accelerator, only valid when ECDSA calculation is done.
pub type OPERATION_RESULT_R = crate::BitReader;
///Field `K_VALUE_WARNING` reader - The k value warning bit of ECDSA Accelerator, valid when k value is bigger than the curve order, then actually taken k = k mod n.
pub type K_VALUE_WARNING_R = crate::BitReader;
impl R {
    ///Bit 0 - The operation result bit of ECDSA Accelerator, only valid when ECDSA calculation is done.
    #[inline(always)]
    pub fn operation_result(&self) -> OPERATION_RESULT_R {
        OPERATION_RESULT_R::new((self.bits & 1) != 0)
    }
    ///Bit 1 - The k value warning bit of ECDSA Accelerator, valid when k value is bigger than the curve order, then actually taken k = k mod n.
    #[inline(always)]
