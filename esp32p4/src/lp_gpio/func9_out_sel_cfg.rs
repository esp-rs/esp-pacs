    ...
    fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
        f.debug_struct("FUNC9_OUT_SEL_CFG")
            .field(
                ...
                &self.reg_gpio_func9_oe_inv_sel().bit(),
            )
            .field("reg_gpio_func9_oe_sel", &self.reg_gpio_func9_oe_sel().bit())
            .field(
                "reg_gpio_func9_out_inv_sel",
                &self.reg_gpio_func9_out_inv_sel().bit(),
            )
            .field(
                "reg_gpio_func9_out_sel",
                &self.reg_gpio_func9_out_sel().bits(),
            )
            .finish()
    }
}
