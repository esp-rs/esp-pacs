...
impl RegisterBlock {
    ...
    ///0x100..0x110 - MSPI flash PMS section %s attribute register
    #[inline(always)]
    ...
    ///0x150 - SPI1 external RAM PMS section 0 start address register
    #[inline(always)]
    pub const fn spi_smem_pms0_size(&self) -> &SPI_SMEM_PMS_SIZE {
        self.spi_smem_pms_size(0)
    }
    ///0x154 - SPI1 external RAM PMS section 1 start address register
    #[inline(always)]
    pub const fn spi_smem_pms1_size(&self) -> &SPI_SMEM_PMS_SIZE {
        self.spi_smem_pms_size(1)
    }
    ///0x158 - SPI1 external RAM PMS section 2 start address register
    #[inline(always)]
    pub const fn spi_smem_pms2_size(&self) -> &SPI_SMEM_PMS_SIZE {
        self.spi_smem_pms_size(2)
    }
    ///0x15c - SPI1 external RAM PMS section 3 start address register
    #[inline(always)]
    ...
    ///0x1a4 - MSPI 16x external RAM input timing delay mode control register
    #[inline(always)]
    pub const fn spi_smem_din_hex_mode(&self) -> &SPI_SMEM_DIN_HEX_MODE {
        &self.spi_smem_din_hex_mode
    }
    ///0x1a8 - MSPI 16x external RAM input timing delay number control register
    #[inline(always)]
    pub const fn spi_smem_din_hex_num(&self) -> &SPI_SMEM_DIN_HEX_NUM {
        &self.spi_smem_din_hex_num
    }
    ///0x1ac - MSPI 16x external RAM output timing adjustment control register
    #[inline(always)]
...
/**SRAM_CLK (rw) register accessor: SPI0 external RAM clock control register
...
pub mod int_ena;
/**INT_CLR (w) register accessor: SPI0 interrupt clear register
...
/**SPI_FMEM_PMS_ATTR (rw) register accessor: MSPI flash PMS section %s attribute register
...
/**SPI_FMEM_PMS_ADDR (rw) register accessor: SPI1 flash PMS section %s start address register
...
/**SPI_FMEM_PMS_SIZE (rw) register accessor: SPI1 flash PMS section %s start address register
...
/**SPI_SMEM_PMS_ATTR (rw) register accessor: SPI1 flash PMS section %s start address register
...
/**SPI_SMEM_PMS_ADDR (rw) register accessor: SPI1 external RAM PMS section %s start address register
...
/**SPI_SMEM_PMS_SIZE (rw) register accessor: SPI1 external RAM PMS section %s start address register
...
/**SPI_SMEM_ECC_CTRL (rw) register accessor: MSPI ECC control register
...
/**SPI_SMEM_AXI_ADDR_CTRL (r) register accessor: SPI0 AXI address control register
...
pub mod dout_mode;
/**SPI_SMEM_TIMING_CALI (rw) register accessor: MSPI external RAM timing calibration register
...
/**SPI_SMEM_DIN_NUM (rw) register accessor: MSPI external RAM input timing delay number control register
...
/**SPI_SMEM_DOUT_MODE (rw) register accessor: MSPI external RAM output timing adjustment control register
...
pub mod spi_smem_ac;
/**SPI_SMEM_DIN_HEX_MODE (rw) register accessor: MSPI 16x external RAM input timing delay mode control register
...
pub mod dpa_ctrl;
/**REGISTERRND_ECO_HIGH (rw) register accessor: MSPI ECO high register
