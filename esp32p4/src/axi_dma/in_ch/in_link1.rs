///Register `IN_LINK1` reader
pub type R = crate::R<IN_LINK1_SPEC>;
///Register `IN_LINK1` writer
...
impl R {
    ///Bit 0 - Set this bit to return to current inlink descriptor's address when there are some errors in current receiving data.
    #[inline(always)]
    pub fn inlink_auto_ret(&self) -> INLINK_AUTO_RET_R {
        INLINK_AUTO_RET_R::new((self.bits & 1) != 0)
    }
    ///Bit 4 - 1: the inlink descriptor's FSM is in idle state. 0: the inlink descriptor's FSM is working.
    #[inline(always)]
...
impl W {
    ...
    ///Bit 1 - Set this bit to stop dealing with the inlink descriptors.
    #[inline(always)]
    #[must_use]
    pub fn inlink_stop(&mut self) -> INLINK_STOP_W<IN_LINK1_SPEC> {
        INLINK_STOP_W::new(self, 1)
    }
    ///Bit 2 - Set this bit to start dealing with the inlink descriptors.
    #[inline(always)]
    #[must_use]
    pub fn inlink_start(&mut self) -> INLINK_START_W<IN_LINK1_SPEC> {
        INLINK_START_W::new(self, 2)
    }
    ///Bit 3 - Set this bit to mount a new inlink descriptor.
    #[inline(always)]
    #[must_use]
...
///`read()` method returns [`in_link1::R`](R) reader structure
impl crate::Readable for IN_LINK1_SPEC {}
///`write(|w| ..)` method takes [`in_link1::W`](W) writer structure
impl crate::Writable for IN_LINK1_SPEC {
    type Safety = crate::Unsafe;
    const ZERO_TO_MODIFY_FIELDS_BITMAP: u32 = 0;
    const ONE_TO_MODIFY_FIELDS_BITMAP: u32 = 0;
}
///`reset()` method sets IN_LINK1 to value 0x11
