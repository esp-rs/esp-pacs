...
///Field `IN_RST` reader - This bit is used to reset AXI_DMA channel 0 Rx FSM and Rx FIFO pointer.
pub type IN_RST_R = crate::BitReader;
///Field `IN_RST` writer - This bit is used to reset AXI_DMA channel 0 Rx FSM and Rx FIFO pointer.
...
///Field `MEM_TRANS_EN` reader - Set this bit 1 to enable automatic transmitting data from memory to memory via AXI_DMA.
pub type MEM_TRANS_EN_R = crate::BitReader;
///Field `MEM_TRANS_EN` writer - Set this bit 1 to enable automatic transmitting data from memory to memory via AXI_DMA.
...
///Field `IN_BURST_SIZE_SEL` reader - 3'b000-3'b100:burst length 8byte~128byte
pub type IN_BURST_SIZE_SEL_R = crate::FieldReader;
///Field `IN_BURST_SIZE_SEL` writer - 3'b000-3'b100:burst length 8byte~128byte
pub type IN_BURST_SIZE_SEL_W<'a, REG> = crate::FieldWriter<'a, REG, 3>;
///Field `IN_CMD_DISABLE` reader - 1:mean disable cmd of this ch0
pub type IN_CMD_DISABLE_R = crate::BitReader;
///Field `IN_CMD_DISABLE` writer - 1:mean disable cmd of this ch0
pub type IN_CMD_DISABLE_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `IN_ECC_AEC_EN` reader - 1: mean access ecc or aes domain,0: mean not
pub type IN_ECC_AEC_EN_R = crate::BitReader;
///Field `IN_ECC_AEC_EN` writer - 1: mean access ecc or aes domain,0: mean not
...
impl R {
    ///Bit 0 - This bit is used to reset AXI_DMA channel 0 Rx FSM and Rx FIFO pointer.
    #[inline(always)]
    ...
    ///Bit 2 - Set this bit 1 to enable automatic transmitting data from memory to memory via AXI_DMA.
    #[inline(always)]
    pub fn mem_trans_en(&self) -> MEM_TRANS_EN_R {
        MEM_TRANS_EN_R::new(((self.bits >> 2) & 1) != 0)
    }
    ///Bit 3 - Set this bit to 1 to enable etm control mode, dma Rx channel 0 is triggered by etm task.
    #[inline(always)]
    pub fn in_etm_en(&self) -> IN_ETM_EN_R {
        IN_ETM_EN_R::new(((self.bits >> 3) & 1) != 0)
    }
    ///Bits 4:6 - 3'b000-3'b100:burst length 8byte~128byte
    #[inline(always)]
    pub fn in_burst_size_sel(&self) -> IN_BURST_SIZE_SEL_R {
        IN_BURST_SIZE_SEL_R::new(((self.bits >> 4) & 7) as u8)
    }
    ///Bit 7 - 1:mean disable cmd of this ch0
    #[inline(always)]
    pub fn in_cmd_disable(&self) -> IN_CMD_DISABLE_R {
        IN_CMD_DISABLE_R::new(((self.bits >> 7) & 1) != 0)
    }
    ///Bit 8 - 1: mean access ecc or aes domain,0: mean not
    #[inline(always)]
    pub fn in_ecc_aec_en(&self) -> IN_ECC_AEC_EN_R {
        IN_ECC_AEC_EN_R::new(((self.bits >> 8) & 1) != 0)
    }
    ///Bit 9 - Set this bit to 1 to enable INCR burst transfer for Rx channel 0 reading link descriptor when accessing internal SRAM.
    #[inline(always)]
...
///`read()` method returns [`in_conf0::R`](R) reader structure
impl crate::Readable for IN_CONF0_SPEC {}
///`write(|w| ..)` method takes [`in_conf0::W`](W) writer structure
