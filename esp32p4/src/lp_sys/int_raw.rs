...
///Field `LP_ADDRHOLE_INT_RAW` reader - the raw interrupt status of lp addrhole(for lp peri and lp ram tee apm, and lp matrix default slave)
pub type LP_ADDRHOLE_INT_RAW_R = crate::BitReader;
///Field `IDBUS_ADDRHOLE_INT_RAW` reader - the raw interrupt status of idbus addrhole(only for lp cpu ibus and dbus)
pub type IDBUS_ADDRHOLE_INT_RAW_R = crate::BitReader;
///Field `LP_CORE_AHB_TIMEOUT_INT_RAW` reader - the raw interrupt status of lp core ahb bus timeout
pub type LP_CORE_AHB_TIMEOUT_INT_RAW_R = crate::BitReader;
///Field `LP_CORE_IBUS_TIMEOUT_INT_RAW` reader - the raw interrupt status of lp core ibus timeout
pub type LP_CORE_IBUS_TIMEOUT_INT_RAW_R = crate::BitReader;
///Field `LP_CORE_DBUS_TIMEOUT_INT_RAW` reader - the raw interrupt status of lp core dbus timeout
pub type LP_CORE_DBUS_TIMEOUT_INT_RAW_R = crate::BitReader;
///Field `ETM_TASK_ULP_INT_RAW` reader - the raw interrupt status of etm task ulp
pub type ETM_TASK_ULP_INT_RAW_R = crate::BitReader;
///Field `SLOW_CLK_TICK_INT_RAW` reader - the raw interrupt status of slow_clk_tick
pub type SLOW_CLK_TICK_INT_RAW_R = crate::BitReader;
impl R {
    ///Bit 0 - the raw interrupt status of lp addrhole(for lp peri and lp ram tee apm, and lp matrix default slave)
    #[inline(always)]
    pub fn lp_addrhole_int_raw(&self) -> LP_ADDRHOLE_INT_RAW_R {
        LP_ADDRHOLE_INT_RAW_R::new((self.bits & 1) != 0)
    }
    ///Bit 1 - the raw interrupt status of idbus addrhole(only for lp cpu ibus and dbus)
    #[inline(always)]
    pub fn idbus_addrhole_int_raw(&self) -> IDBUS_ADDRHOLE_INT_RAW_R {
        IDBUS_ADDRHOLE_INT_RAW_R::new(((self.bits >> 1) & 1) != 0)
    }
    ///Bit 2 - the raw interrupt status of lp core ahb bus timeout
    #[inline(always)]
    pub fn lp_core_ahb_timeout_int_raw(&self) -> LP_CORE_AHB_TIMEOUT_INT_RAW_R {
        LP_CORE_AHB_TIMEOUT_INT_RAW_R::new(((self.bits >> 2) & 1) != 0)
    }
    ///Bit 3 - the raw interrupt status of lp core ibus timeout
    #[inline(always)]
    ...
    ///Bit 4 - the raw interrupt status of lp core dbus timeout
    #[inline(always)]
