...
///Field `MST_ST` reader - The current status of SPI1 master FSM.
pub type MST_ST_R = crate::FieldReader;
///Field `SLV_ST` reader - The current status of SPI1 slave FSM: mspi_st. 0: idle state, 1: preparation state, 2: send command state, 3: send address state, 4: wait state, 5: read data state, 6:write data state, 7: done state, 8: read data end state.
