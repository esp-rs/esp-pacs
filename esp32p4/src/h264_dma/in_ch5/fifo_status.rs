...
///Field `INFIFO_FULL_L1` reader - Tx FIFO full signal for Tx channel 1.
pub type INFIFO_FULL_L1_R = crate::BitReader;
///Field `INFIFO_EMPTY_L1` reader - Tx FIFO empty signal for Tx channel 1.
pub type INFIFO_EMPTY_L1_R = crate::BitReader;
///Field `INFIFO_CNT_L1` reader - The register stores the byte number of the data in Tx FIFO for Tx channel 1.
pub type INFIFO_CNT_L1_R = crate::FieldReader;
impl R {
    ///Bit 0 - Tx FIFO full signal for Tx channel 1.
    #[inline(always)]
    pub fn infifo_full_l1(&self) -> INFIFO_FULL_L1_R {
        INFIFO_FULL_L1_R::new((self.bits & 1) != 0)
    }
    ///Bit 1 - Tx FIFO empty signal for Tx channel 1.
    #[inline(always)]
    pub fn infifo_empty_l1(&self) -> INFIFO_EMPTY_L1_R {
        INFIFO_EMPTY_L1_R::new(((self.bits >> 1) & 1) != 0)
    }
    ///Bits 2:6 - The register stores the byte number of the data in Tx FIFO for Tx channel 1.
    #[inline(always)]
...
///`reset()` method sets FIFO_STATUS to value 0x02
