...
impl R {
    ///Bits 0:2 - This register stores the current control module state machine state.
    #[inline(always)]
    pub fn in_state(&self) -> IN_STATE_R {
        IN_STATE_R::new((self.bits & 7) as u8)
    }
    ///Bit 3 - This register indicate that if the channel reset is safety.
    #[inline(always)]
...
///`reset()` method sets STATE to value 0x08
