...
///Field `OUT_DSCR_STATE` reader - This register stores the current descriptor state machine state.
pub type OUT_DSCR_STATE_R = crate::FieldReader;
///Field `OUT_STATE` reader - This register stores the current control module state machine state.
pub type OUT_STATE_R = crate::FieldReader;
///Field `OUT_RESET_AVAIL` reader - This register indicate that if the channel reset is safety.
...
impl R {
    ...
    ///Bits 20:23 - This register stores the current control module state machine state.
    #[inline(always)]
    pub fn out_state(&self) -> OUT_STATE_R {
        OUT_STATE_R::new(((self.bits >> 20) & 0x0f) as u8)
    }
    ///Bit 24 - This register indicate that if the channel reset is safety.
    #[inline(always)]
...
///`reset()` method sets STATE to value 0x0100_0000
