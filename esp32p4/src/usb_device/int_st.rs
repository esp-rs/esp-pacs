    ...
    fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
        f.debug_struct("INT_ST")
            .field(
                ...
                &format_args!("{}", self.usb_serial_jtag_jtag_in_flush_int_st().bit()),
            )
            .field(
                "usb_serial_jtag_sof_int_st",
                &format_args!("{}", self.usb_serial_jtag_sof_int_st().bit()),
            )
            .field(
                "usb_serial_jtag_serial_out_recv_pkt_int_st",
                &format_args!(
                    "{}",
                    self.usb_serial_jtag_serial_out_recv_pkt_int_st().bit()
                ),
            )
            .field(
                "usb_serial_jtag_serial_in_empty_int_st",
                &format_args!("{}", self.usb_serial_jtag_serial_in_empty_int_st().bit()),
            )
            .field(
                "usb_serial_jtag_pid_err_int_st",
                &format_args!("{}", self.usb_serial_jtag_pid_err_int_st().bit()),
            )
            .field(
                "usb_serial_jtag_crc5_err_int_st",
                &format_args!("{}", self.usb_serial_jtag_crc5_err_int_st().bit()),
            )
            .field(
                "usb_serial_jtag_crc16_err_int_st",
                &format_args!("{}", self.usb_serial_jtag_crc16_err_int_st().bit()),
            )
            .field(
                "usb_serial_jtag_stuff_err_int_st",
                &format_args!("{}", self.usb_serial_jtag_stuff_err_int_st().bit()),
            )
            .field(
                "usb_serial_jtag_in_token_rec_in_ep1_int_st",
                &format_args!(
                    "{}",
                    self.usb_serial_jtag_in_token_rec_in_ep1_int_st().bit()
                ),
            )
            .field(
                "usb_serial_jtag_usb_bus_reset_int_st",
                &format_args!("{}", self.usb_serial_jtag_usb_bus_reset_int_st().bit()),
            )
            .field(
                "usb_serial_jtag_out_ep1_zero_payload_int_st",
                &format_args!(
                    "{}",
                    self.usb_serial_jtag_out_ep1_zero_payload_int_st().bit()
                ),
            )
            .field(
                ...
                &format_args!(
                    "{}",
                    self.usb_serial_jtag_out_ep2_zero_payload_int_st().bit()
                ),
            )
            .field(
                "usb_serial_jtag_rts_chg_int_st",
                &format_args!("{}", self.usb_serial_jtag_rts_chg_int_st().bit()),
            )
            .field(
                "usb_serial_jtag_dtr_chg_int_st",
                &format_args!("{}", self.usb_serial_jtag_dtr_chg_int_st().bit()),
            )
            .field(
                "usb_serial_jtag_get_line_code_int_st",
                &format_args!("{}", self.usb_serial_jtag_get_line_code_int_st().bit()),
            )
            .field(
                "usb_serial_jtag_set_line_code_int_st",
                &format_args!("{}", self.usb_serial_jtag_set_line_code_int_st().bit()),
            )
            .finish()
    }
}
