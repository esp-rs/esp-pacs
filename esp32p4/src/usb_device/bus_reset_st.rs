///Register `BUS_RESET_ST` reader
pub type R = crate::R<BUS_RESET_ST_SPEC>;
///Field `USB_BUS_RESET_ST` reader - USB bus reset status. 0: USB-Serial-JTAG is in usb bus reset status. 1: USB bus reset is released.
pub type USB_BUS_RESET_ST_R = crate::BitReader;
impl R {
    ///Bit 0 - USB bus reset status. 0: USB-Serial-JTAG is in usb bus reset status. 1: USB bus reset is released.
    #[inline(always)]
...
///`read()` method returns [`bus_reset_st::R`](R) reader structure
