    ...
    fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
        f.debug_struct("OUT_EP2_ST")
            .field(
                ...
                &format_args!("{}", self.usb_serial_jtag_out_ep2_state().bits()),
            )
            .field(
                "usb_serial_jtag_out_ep2_wr_addr",
                &format_args!("{}", self.usb_serial_jtag_out_ep2_wr_addr().bits()),
            )
            .field(
                "usb_serial_jtag_out_ep2_rd_addr",
                &format_args!("{}", self.usb_serial_jtag_out_ep2_rd_addr().bits()),
            )
            .finish()
    }
}
