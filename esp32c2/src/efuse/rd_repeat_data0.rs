...
///Field `RD_DIS` reader - The bit be set to disable software read high/low 128-bit of BLK3.
...
///Field `DIS_PAD_JTAG` reader - Set this bit to disable pad jtag.
pub type DIS_PAD_JTAG_R = crate::BitReader;
///Field `DIS_DOWNLOAD_ICACHE` reader - The bit be set to disable icache in download mode.
pub type DIS_DOWNLOAD_ICACHE_R = crate::BitReader;
///Field `DIS_DOWNLOAD_MANUAL_ENCRYPT` reader - The bit be set to disable manual encryption.
pub type DIS_DOWNLOAD_MANUAL_ENCRYPT_R = crate::BitReader;
///Field `SPI_BOOT_ENCRYPT_DECRYPT_CNT` reader - These bits be set to enable SPI boot encrypt/decrypt. Odd number of 1: enable. even number of 1: disable.
pub type SPI_BOOT_ENCRYPT_DECRYPT_CNT_R = crate::FieldReader;
///Field `XTS_KEY_LENGTH_256` reader - The bit be set means XTS_AES use the whole 256-bit efuse data in BLOCK3. Otherwise, XTS_AES use 128-bit eFuse data in BLOCK3.
pub type XTS_KEY_LENGTH_256_R = crate::BitReader;
///Field `UART_PRINT_CONTROL` reader - Set this bit to disable usb printing.
...
///Field `DIS_DOWNLOAD_MODE` reader - Set this bit to disable download mode (boot_mode\[3:0\] = 0, 1, 2, 4, 5, 6, 7).
pub type DIS_DOWNLOAD_MODE_R = crate::BitReader;
///Field `DIS_DIRECT_BOOT` reader - This bit set means disable direct_boot mode.
...
///Field `FLASH_TPUW` reader - Configures flash waiting time after power-up, in unit of ms. If the value is less than 15, the waiting time is the configurable value. Otherwise, the waiting time is twice the configurable value.
pub type FLASH_TPUW_R = crate::FieldReader;
///Field `SECURE_BOOT_EN` reader - The bit be set to enable secure boot.
pub type SECURE_BOOT_EN_R = crate::BitReader;
///Field `RPT4_RESERVED` reader - Reserved (used for four backups method).
