#[doc = "Register `INT_ENA` reader"]
pub type R = crate::R<INT_ENA_SPEC>;
#[doc = "Register `INT_ENA` writer"]
pub type W = crate::W<INT_ENA_SPEC>;
#[doc = "Field `TARGET0_INT_ENA` reader - Write 1 to enable SYSTIMER_TARGET0_INT."]
pub type TARGET0_INT_ENA_R = crate::BitReader;
#[doc = "Field `TARGET0_INT_ENA` writer - Write 1 to enable SYSTIMER_TARGET0_INT."]
pub type TARGET0_INT_ENA_W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `TARGET1_INT_ENA` reader - Write 1 to enable SYSTIMER_TARGET1_INT."]
pub type TARGET1_INT_ENA_R = crate::BitReader;
#[doc = "Field `TARGET1_INT_ENA` writer - Write 1 to enable SYSTIMER_TARGET1_INT."]
pub type TARGET1_INT_ENA_W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `TARGET2_INT_ENA` reader - Write 1 to enable SYSTIMER_TARGET2_INT."]
pub type TARGET2_INT_ENA_R = crate::BitReader;
#[doc = "Field `TARGET2_INT_ENA` writer - Write 1 to enable SYSTIMER_TARGET2_INT."]
pub type TARGET2_INT_ENA_W<'a, REG> = crate::BitWriter<'a, REG>;
impl R {
    #[doc = "Bit 0 - Write 1 to enable SYSTIMER_TARGET0_INT."]
    #[inline(always)]
    pub fn target0_int_ena(&self) -> TARGET0_INT_ENA_R {
        TARGET0_INT_ENA_R::new((self.bits & 1) != 0)
    }
    #[doc = "Bit 1 - Write 1 to enable SYSTIMER_TARGET1_INT."]
    #[inline(always)]
    pub fn target1_int_ena(&self) -> TARGET1_INT_ENA_R {
        TARGET1_INT_ENA_R::new(((self.bits >> 1) & 1) != 0)
    }
    #[doc = "Bit 2 - Write 1 to enable SYSTIMER_TARGET2_INT."]
    #[inline(always)]
    pub fn target2_int_ena(&self) -> TARGET2_INT_ENA_R {
        TARGET2_INT_ENA_R::new(((self.bits >> 2) & 1) != 0)
    }
}
#[cfg(feature = "impl-register-debug")]
impl core::fmt::Debug for R {
    fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
        f.debug_struct("INT_ENA")
            .field("target0_int_ena", &self.target0_int_ena())
            .field("target1_int_ena", &self.target1_int_ena())
            .field("target2_int_ena", &self.target2_int_ena())
            .finish()
    }
}
impl W {
    #[doc = "Bit 0 - Write 1 to enable SYSTIMER_TARGET0_INT."]
    #[inline(always)]
    pub fn target0_int_ena(&mut self) -> TARGET0_INT_ENA_W<'_, INT_ENA_SPEC> {
        TARGET0_INT_ENA_W::new(self, 0)
    }
    #[doc = "Bit 1 - Write 1 to enable SYSTIMER_TARGET1_INT."]
    #[inline(always)]
    pub fn target1_int_ena(&mut self) -> TARGET1_INT_ENA_W<'_, INT_ENA_SPEC> {
        TARGET1_INT_ENA_W::new(self, 1)
    }
    #[doc = "Bit 2 - Write 1 to enable SYSTIMER_TARGET2_INT."]
    #[inline(always)]
    pub fn target2_int_ena(&mut self) -> TARGET2_INT_ENA_W<'_, INT_ENA_SPEC> {
        TARGET2_INT_ENA_W::new(self, 2)
    }
}
#[doc = "Interrupt enable register of system timer\n\nYou can [`read`](crate::Reg::read) this register and get [`int_ena::R`](R). You can [`reset`](crate::Reg::reset), [`write`](crate::Reg::write), [`write_with_zero`](crate::Reg::write_with_zero) this register using [`int_ena::W`](W). You can also [`modify`](crate::Reg::modify) this register. See [API](https://docs.rs/svd2rust/#read--modify--write-api)."]
pub struct INT_ENA_SPEC;
impl crate::RegisterSpec for INT_ENA_SPEC {
    type Ux = u32;
}
#[doc = "`read()` method returns [`int_ena::R`](R) reader structure"]
impl crate::Readable for INT_ENA_SPEC {}
#[doc = "`write(|w| ..)` method takes [`int_ena::W`](W) writer structure"]
impl crate::Writable for INT_ENA_SPEC {
    type Safety = crate::Unsafe;
}
#[doc = "`reset()` method sets INT_ENA to value 0"]
impl crate::Resettable for INT_ENA_SPEC {}
