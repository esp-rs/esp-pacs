...
#[doc = "Field `STATUS_W1TS` writer - Configures whether or not to set the interrupt status register LP_GPIO_STATUS_INTERRUPT of LP_GPIO0 ~ LP_GPIO7. - Bit0 ~ bit7 are corresponding to LP_GPIO0 ~ LP_GPIO7. Bitxx ~ bitxx is invalid. - If the value 1 is written to a bit here, the corresponding bit in LP_GPIO_STATUS_INTERRUPT will be set to 1. \\item Recommended operation: use this register to set LP_GPIO_STATUS_INTERRUPT."]
