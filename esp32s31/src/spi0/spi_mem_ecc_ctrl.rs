...
impl W {
    ...
    pub fn spi_fmem_ecc_err_int_num(
        &mut self,
    ) -> SPI_FMEM_ECC_ERR_INT_NUM_W<'_, SPI_MEM_ECC_CTRL_SPEC> {
    ...
    pub fn spi_fmem_ecc_err_int_en(
        &mut self,
    ) -> SPI_FMEM_ECC_ERR_INT_EN_W<'_, SPI_MEM_ECC_CTRL_SPEC> {
    ...
    #[doc = "Bit 21 - Set this bit to enable MSPI ECC address conversion, no matter MSPI accesses to the ECC region or non-ECC region of flash. If there is no ECC region in flash, this bit should be 0. Otherwise, this bit should be 1."]
    #[inline(always)]
    pub fn spi_fmem_ecc_addr_en(&mut self) -> SPI_FMEM_ECC_ADDR_EN_W<'_, SPI_MEM_ECC_CTRL_SPEC> {
        SPI_FMEM_ECC_ADDR_EN_W::new(self, 21)
    }
    #[doc = "Bit 22 - Set this bit to enable ECC address convert in SPI0/1 USR_CMD transfer."]
    #[inline(always)]
    pub fn spi_mem_usr_ecc_addr_en(
        &mut self,
    ) -> SPI_MEM_USR_ECC_ADDR_EN_W<'_, SPI_MEM_ECC_CTRL_SPEC> {
        SPI_MEM_USR_ECC_ADDR_EN_W::new(self, 22)
    }
    #[doc = "Bit 24 - 1: The error information in SPI_MEM_ECC_ERR_BITS and SPI_MEM_ECC_ERR_ADDR is updated when there is an ECC error. 0: SPI_MEM_ECC_ERR_BITS and SPI_MEM_ECC_ERR_ADDR record the first ECC error information."]
    #[inline(always)]
    pub fn spi_mem_ecc_continue_record_err_en(
        &mut self,
    ) -> SPI_MEM_ECC_CONTINUE_RECORD_ERR_EN_W<'_, SPI_MEM_ECC_CTRL_SPEC> {
