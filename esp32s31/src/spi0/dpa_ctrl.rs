...
#[doc = "Field `XTS_SHADOW_FOR_SEC` reader - 1: shadow core mode for security (against fault injection attack). 0: shadow core mode for performance."]
pub type XTS_SHADOW_FOR_SEC_R = crate::BitReader;
#[doc = "Field `XTS_SHADOW_FOR_SEC` writer - 1: shadow core mode for security (against fault injection attack). 0: shadow core mode for performance."]
pub type XTS_SHADOW_FOR_SEC_W<'a, REG> = crate::BitWriter<'a, REG>;
impl R {
    ...
    #[doc = "Bit 5 - 1: shadow core mode for security (against fault injection attack). 0: shadow core mode for performance."]
    #[inline(always)]
    pub fn xts_shadow_for_sec(&self) -> XTS_SHADOW_FOR_SEC_R {
        XTS_SHADOW_FOR_SEC_R::new(((self.bits >> 5) & 1) != 0)
    }
}
...
impl core::fmt::Debug for R {
    fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
        f.debug_struct("DPA_CTRL")
            ...
            .field("xts_shadow_for_sec", &self.xts_shadow_for_sec())
            .finish()
    }
}
impl W {
    #[doc = "Bits 0:2 - Set the security level of spi mem cryption. 0: Shut off cryption DPA funtion. 1-7: The bigger the number is, the more secure the cryption is. (Note that the performance of cryption will decrease together with this number increasing)"]
    #[inline(always)]
    pub fn crypt_security_level(&mut self) -> CRYPT_SECURITY_LEVEL_W<'_, DPA_CTRL_SPEC> {
        CRYPT_SECURITY_LEVEL_W::new(self, 0)
    }
    #[doc = "Bit 3 - Only available when SPI_CRYPT_SECURITY_LEVEL is not 0. 1: Enable DPA in the calculation that using key 1 or key 2. 0: Enable DPA only in the calculation that using key 1."]
    #[inline(always)]
    pub fn crypt_calc_d_dpa_en(&mut self) -> CRYPT_CALC_D_DPA_EN_W<'_, DPA_CTRL_SPEC> {
        CRYPT_CALC_D_DPA_EN_W::new(self, 3)
    }
    #[doc = "Bit 4 - 1: MSPI XTS DPA clock gate is controlled by SPI_CRYPT_CALC_D_DPA_EN and SPI_CRYPT_SECURITY_LEVEL. 0: Controlled by efuse bits."]
    #[inline(always)]
    pub fn crypt_dpa_select_register(&mut self) -> CRYPT_DPA_SELECT_REGISTER_W<'_, DPA_CTRL_SPEC> {
        CRYPT_DPA_SELECT_REGISTER_W::new(self, 4)
    }
    #[doc = "Bit 5 - 1: shadow core mode for security (against fault injection attack). 0: shadow core mode for performance."]
    #[inline(always)]
    pub fn xts_shadow_for_sec(&mut self) -> XTS_SHADOW_FOR_SEC_W<'_, DPA_CTRL_SPEC> {
        XTS_SHADOW_FOR_SEC_W::new(self, 5)
    }
}
